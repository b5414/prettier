...
import {clearEditorconfigCache, loadEditorconfig as loadEditorconfigForFile} from './editorconfig/index.js';
import {clearPrettierConfigCache, loadPrettierConfig as loadPrettierConfigFile, searchPrettierConfig} from './prettier-config/index.js';
...
async function resolveConfig(fileUrlOrPath, options) {
	...
	const [result, editorConfigured] = await Promise.all([loadPrettierConfig(filePath, options), loadEditorconfig(filePath, options)]);
...
async function resolveConfigFile(fileUrlOrPath) {
	const directory = fileUrlOrPath ? path.dirname(path.resolve(toPath(fileUrlOrPath))) : undefined;
