...
import loaders, {loadConfigFromPackageJson, loadConfigFromPackageYaml} from './loaders.js';

async function loadConfig(configFile) {
	const {base: fileName, ext: extension} = path.parse(configFile);
	const load = fileName === 'package.json' ? loadConfigFromPackageJson : fileName === 'package.yaml' ? loadConfigFromPackageYaml : loaders[extension];
