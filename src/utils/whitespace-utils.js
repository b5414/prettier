...
class WhitespaceUtils {
	...
	constructor(whitespaceCharacters) {
		...
		if (
			...
			(this.#whitespaceCharacters.size === 0 || Array.prototype.some.call(whitespaceCharacters, (character) => !/^\s$/u.test(character)))
		) {
	...
	split(text, captureWhitespace = false) {
		...
		const regexp = new RegExp(captureWhitespace ? `(${pattern})` : pattern, 'u');
