...
function getLanguageByFileName(languages, file) {
	...
	return (
		languages.find(({filenames}) => filenames?.some((name) => name.toLowerCase() === basename)) ??
		languages.find(({extensions}) => extensions?.some((extension) => basename.endsWith(extension)))
	);
}
...
function getLanguageByInterpreter(languages, file) {
	if (process.env.PRETTIER_TARGET === 'universal' || !file || getFileBasename(file).includes('.')) {
		return;
	}
