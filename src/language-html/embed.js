import {breakParent, group, hardline, indent, line} from '../document/builders.js';
...
import {needsToBorrowPrevClosingTagEndMarker, printClosingTag, printClosingTagSuffix, printOpeningTag, printOpeningTagPrefix} from './print/tag.js';
import {dedentString, htmlTrimPreserveIndentation, inferElementParser, isScriptLikeTag, isVueNonHtmlBlock} from './utils/index.js';
import isVueSfcWithTypescriptScript from './utils/is-vue-sfc-with-typescript-script.js';

const embeddedAngularControlFlowBlocks = new Set(['if', 'else if', 'for', 'switch', 'case']);
			...
			if (isScriptLikeTag(node.parent)) {
				...
				if (parser) {
					return async (textToDoc) => {
						...
						return [breakParent, printOpeningTagPrefix(node, options), await textToDoc(value, textToDocOptions), printClosingTagSuffix(node, options)];
					};
				}
				...
				return async (textToDoc) => {
					...
					if (options.parser === 'angular') {
						...
						textToDocOptions.parser = isVueSfcWithTypescriptScript(path, options) ? '__vue_ts_expression' : '__vue_expression';
					} else {
						textToDocOptions.parser = '__js_expression';
					}

					return [indent([line, await textToDoc(node.value, textToDocOptions)]), node.parent.next && needsToBorrowPrevClosingTagEndMarker(node.parent.next) ? ' ' : line];
				};
			}
			break;
