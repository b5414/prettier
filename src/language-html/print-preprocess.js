...
function mergeIfConditionalStartEndCommentIntoElementOpeningTag(ast /*, options */) {
	/**
	 *     <!--[if ...]><!--><target><!--<![endif]-->
	 */
	const isTarget = (node) =>
...
function extractWhitespaces(ast /*, options*/) {
	ast.walk((node) => {
		...
		if (node.children.length === 0 || (node.children.length === 1 && node.children[0].type === 'text' && htmlWhitespaceUtils.trim(node.children[0].value).length === 0)) {
...
function addHasHtmComponentClosingTag(ast, options) {
	ast.walk((node) => {
		...
		node.hasHtmComponentClosingTag =
			node.endSourceSpan && /^<\s*\/\s*\/\s*>$/u.test(options.originalText.slice(node.endSourceSpan.start.offset, node.endSourceSpan.end.offset));
	});
}
