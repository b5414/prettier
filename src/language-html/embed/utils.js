...
function shouldHugJsExpression(ast, options) {
	const rootNode =
		ast.type === 'NGRoot'
			? ast.node.type === 'NGMicrosyntax' && ast.node.body.length === 1 && ast.node.body[0].type === 'NGMicrosyntaxExpression'
	...
	return (
		...
		(rootNode.type === 'ObjectExpression' ||
			...
			((options.parser === '__vue_expression' || options.parser === '__vue_ts_expression') && (rootNode.type === 'TemplateLiteral' || rootNode.type === 'StringLiteral')))
	);
}
 ...
 * @returns {Promise<Doc>}
 */
async function formatAttributeValue(code, textToDoc, options, shouldHugJsExpression) {
	options = {
