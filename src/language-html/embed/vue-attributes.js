import {getUnescapedAttributeValue, isVueScriptTag, isVueSfcBindingsAttribute, isVueSlotAttribute} from '../utils/index.js';
...
import {isVueEventBindingExpression, printVueBindings} from './vue-bindings.js';
...
function printVueVOnDirective(text, textToDoc, {parseWithTs}) {
	...
	return formatAttributeValue(text, textToDoc, {parser: parseWithTs ? '__vue_ts_event_binding' : '__vue_event_binding'}, shouldHugJsExpression);
}
...
function printVueVBindDirective(text, textToDoc, {parseWithTs}) {
	return formatAttributeValue(text, textToDoc, {parser: parseWithTs ? '__vue_ts_expression' : '__vue_expression'}, shouldHugJsExpression);
}
...
function printExpression(text, textToDoc, {parseWithTs}) {
	return formatAttributeValue(text, textToDoc, {parser: parseWithTs ? '__ts_expression' : '__js_expression'}, shouldHugJsExpression);
}
