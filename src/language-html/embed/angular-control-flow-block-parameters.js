...
function printAngularControlFlowBlockParameters(textToDoc, print, path, options) {
	const {node} = path;

	const content = options.originalText.slice(node.sourceSpan.start.offset, node.sourceSpan.end.offset);
