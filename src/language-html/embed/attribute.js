...
function printAttribute(path, options) {
	...
	if (
		...
		/^PRETTIER_HTML_PLACEHOLDER_\d+_\d+_IN_JS$/u.test(options.originalText.slice(node.valueSpan.start.offset, node.valueSpan.end.offset)) || // lwc: html`<my-element data-for={value}></my-element>`
		(options.parser === 'lwc' && node.value.startsWith('{') && node.value.endsWith('}'))
	) {
		return [node.rawName, '=', node.value];
	}

	for (const getValuePrinter of [printSrcset, printStyleAttribute, printClassNames, printVueAttribute, printAngularAttribute]) {
...
function printAttributeWithValuePrinter(printValue) {
	return async (textToDoc, print, path, options) => {
		...
		valueDoc = mapDoc(valueDoc, (doc) => (typeof doc === 'string' ? doc.replaceAll('"', '&quot;') : doc));
