...
import {getTextValueParts, getUnescapedAttributeValue} from '../utils/index.js';
import {interpolationRegex as angularInterpolationRegex, printAngularInterpolation} from './angular-interpolation.js';
import {formatAttributeValue, printExpand, shouldHugJsExpression} from './utils.js';

function createAngularPrinter({parser}) {
	return (textToDoc, print, path /*, options*/) => formatAttributeValue(getUnescapedAttributeValue(path.node), textToDoc, {parser}, shouldHugJsExpression);
}
...
function printAngularAttribute(path, options) {
	...
	if (/^i18n(?:-.+)?$/u.test(attributeName)) {
		return () => printExpand(fill(getTextValueParts(node, value.trim())), !value.includes('@@'));
	}
