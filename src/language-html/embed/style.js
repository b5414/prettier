...
function printStyleAttribute(path, options) {
	...
	if (node.fullName === 'style' && !options.parentParser && !text.includes('{{')) {
		return async (textToDoc) => printExpand(await textToDoc(text, {parser: 'css', __isHTMLStyleAttribute: true}));
	}
}
