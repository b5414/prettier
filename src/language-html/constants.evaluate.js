...
const getCssStyleTags = (property)=>
	fromPairs(
		flat(
			htmlStyles.filter((htmlStyle)=>htmlStyle.style[property]).map((htmlStyle)=>
				htmlStyle.selectorText.split(',').map((selector)=>selector.trim()).filter((selector)=>/^[\dA-Za-z]+$/.test(selector)).map((tagName)=>[tagName, htmlStyle.style[property]]),
			),
		),
	);
