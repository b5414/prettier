import {getHtmlTagDefinition, parse as parseHtml, ParseLocation, ParseSourceFile, ParseSourceSpan, RecursiveVisitor, TagContentType, visitAll} from 'angular-html-parser';
...
function normalizeAngularControlFlowBlock(node) {
	...
	node.parameters = {
		...
		sourceSpan: new ParseSourceSpan(node.parameters[0].sourceSpan.start, node.parameters.at(-1).sourceSpan.end),
	};
}
...
function ngHtmlParser(input, parseOptions, options) {
	...
	let {rootNodes, errors} = parseHtml(input, {
		...
		getTagContentType: shouldParseAsRawText ? (...args) => (shouldParseAsRawText(...args) ? TagContentType.RAW_TEXT : undefined) : undefined,
	...
	if (name === 'vue') {
		...
		const getNodeWithSameLocation = (node) =>
			getHtmlParseResult().rootNodes.find(({startSourceSpan}) => startSourceSpan && startSourceSpan.start.offset === node.startSourceSpan.start.offset) ?? node;
		for (const [index, node] of rootNodes.entries()) {
			...
			if (isVoidElement) {
				...
				const error = getHtmlParseResult().errors.find(
					(error) => error.span.start.offset > startSourceSpan.start.offset && error.span.start.offset < endSourceSpan.end.offset,
				);
				if (error) {
	...
	const normalizeName = (node) => {
		if (node.type === 'element') {
			if (normalizeTagName && (!node.namespace || node.namespace === node.tagDefinition.implicitNamespacePrefix || isUnknownNamespace(node))) {
				node.name = lowerCaseIfFn(node.name, (lowerCasedName) => HTML_TAGS.has(lowerCasedName));
			}
					...
					if (!attr.namespace) {
						attr.name = lowerCaseIfFn(
							...
							(lowerCasedAttrName) =>
								...
								(HTML_ELEMENT_ATTRIBUTES.get('*').has(lowerCasedAttrName) || HTML_ELEMENT_ATTRIBUTES.get(node.name).has(lowerCasedAttrName)),
						);
					}
				}
			}
 ...
 */
function parse(text, parseOptions, options = {}, shouldParseFrontMatter = true) {
	...
	const parseSubHtml = (subContent, startSpan) => {
		...
		const subAst = parse(fakeContent + realContent, parseOptions, options, false);
		// @ts-expect-error
...
export const vue = createParser({
	...
	shouldParseAsRawText(tagName, prefix, hasParent, attrs) {
		return (
			...
			!hasParent &&
			(tagName !== 'template' || attrs.some(({name, value}) => name === 'lang' && value !== 'html' && value !== '' && value !== undefined))
		);
	},
});
