...
const {ParseSourceSpan, ParseLocation, ParseSourceFile} = require('angular-html-parser/lib/compiler/src/parse_util');
const parseFrontMatter = require('../utils/front-matter/parse');
const getLast = require('../utils/get-last');
const createError = require('../common/parser-create-error');
const {inferParserByLanguage} = require('../common/util');
const {HTML_ELEMENT_ATTRIBUTES, HTML_TAGS, isUnknownNamespace} = require('./utils');
const {hasPragma} = require('./pragma');
const {Node} = require('./ast');
const {parseIeConditionalComment} = require('./conditional-comment');
const {locStart, locEnd} = require('./loc');

function ngHtmlParser(input, {recognizeSelfClosing, normalizeTagName, normalizeAttributeName, allowHtmComponentClosingTags, isTagNameCaseSensitive, getTagContentType}, options) {
	const parser = require('angular-html-parser');
	const {RecursiveVisitor, visitAll, Attribute, CDATA, Comment, DocType, Element, Text} = require('angular-html-parser/lib/compiler/src/ml_parser/ast');
	const {ParseSourceSpan} = require('angular-html-parser/lib/compiler/src/parse_util');
	const {getHtmlTagDefinition} = require('angular-html-parser/lib/compiler/src/ml_parser/html_tags');
	...
	const isVueHtml = options.parser === 'vue' && rootNodes.some((node)=>(node instanceof DocType && node.value === 'html') || (node instanceof Element && node.name.toLowerCase() === 'html'));
	...
	if(options.parser === 'vue' && !isVueHtml){
		...
		if(rootNodes.some(shouldParseAsHTML)){
			...
			const getSameLocationNode = (node)=>getSecondParse().rootNodes.find(({startSourceSpan})=>startSourceSpan && startSourceSpan.start.offset === node.startSourceSpan.start.offset);
	...
	const normalizeName = (node)=>{
		if(node instanceof Element){
			...
			if(normalizeAttributeName){
				const CURRENT_HTML_ELEMENT_ATTRIBUTES = HTML_ELEMENT_ATTRIBUTES[node.name] || Object.create(null);
				for(const attr of node.attrs){
					if(!attr.namespace){
						attr.name = lowerCaseIfFn(
							attr.name,
							(lowerCasedAttrName)=>node.name in HTML_ELEMENT_ATTRIBUTES && (lowerCasedAttrName in HTML_ELEMENT_ATTRIBUTES['*'] || lowerCasedAttrName in CURRENT_HTML_ELEMENT_ATTRIBUTES),
						);
					}
				}
			}
...
function _parse(text, options, parserOptions, shouldParseFrontMatter = true) {
	const {frontMatter, content} = shouldParseFrontMatter ? parseFrontMatter(text) : {frontMatter: null, content: text};
	...
	parsers: {
		...
		vue: createParser({
			...
			getTagContentType: (tagName, prefix, hasParent, attrs)=>{
				if(tagName.toLowerCase() !== 'html' && !hasParent && (tagName !== 'template' || attrs.some(({name, value})=>name === 'lang' && value !== 'html'))){
