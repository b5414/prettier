...
import {printAngularControlFlowBlock, printAngularControlFlowBlockParameters} from './print/angular-control-flow-block.js';
import {printAngularIcuCase, printAngularIcuExpression} from './print/angular-icu-expression.js';
import {printChildren} from './print/children.js';
import {printElement} from './print/element.js';
import {printClosingTagEnd, printClosingTagSuffix, printOpeningTagPrefix, printOpeningTagStart} from './print/tag.js';
...
function genericPrint(path, options, print) {
	...
	switch (node.type) {
		...
		case 'docType':
			return [group([printOpeningTagStart(node, options), ' ', node.value.replace(/^html\b/iu, 'html').replaceAll(/\s+/gu, ' ')]), printClosingTagEnd(node, options)];
		case 'comment':
			return [printOpeningTagPrefix(node, options), replaceEndOfLine(options.originalText.slice(locStart(node), locEnd(node))), printClosingTagSuffix(node, options)];
		...
		case 'attribute': {
			...
			return [node.rawName, '=', quote, replaceEndOfLine(quote === '"' ? value.replaceAll('"', '&quot;') : value.replaceAll("'", '&apos;')), quote];
		}
