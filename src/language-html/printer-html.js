...
const {
	builders: {breakParent, dedentToRoot, fill, group, hardline, ifBreak, indentIfBreak, indent, join, line, literalline, softline},
...
const {printVueFor, printVueBindings, isVueEventBindingExpression} = require('./syntax-vue');
	...
	switch(node.type){
		case 'element': {
			...
			if(!node.isSelfClosing && isVueNonHtmlBlock(node, options)){
				...
				let doc = '';
				if(!isEmpty){
					doc = textToDoc(htmlTrimPreserveIndentation(content), {parser, __embeddedInHtml: true}, {stripTrailingHardline: true});
					isEmpty = doc === '';
				}
		...
		case 'attribute': {
			...
			const embeddedAttributeValueDoc = printEmbeddedAttributeValue(
				...
				(code, opts)=>
					...
					textToDoc(code, {__isInHtmlAttribute: true, __embeddedInHtml: true, ...opts}, {stripTrailingHardline: true}),
				options,
			);
...
function genericPrint(path, options, print) {
	...
	switch(node.type){
						...
						: [
								...
								((childrenDoc)=>
									shouldHugContent
										...
										: (isScriptLikeTag(node) || isVueCustomBlock(node, options)) && node.parent.type === 'root' && options.parser === 'vue' && !options.vueIndentScriptAndStyle
										? childrenDoc
										: indent(childrenDoc))([
									shouldHugContent
								...
								(node.next ? needsToBorrowPrevClosingTagEndMarker(node.next) : needsToBorrowLastChildClosingTagEndMarker(node.parent))
									...
									: node.lastChild.hasTrailingSpaces && node.lastChild.isTrailingSpaceSensitive
									? line
									: (node.lastChild.type === 'comment' || (node.lastChild.type === 'text' && node.isWhitespaceSensitive && node.isIndentationSensitive)) &&
		...
		case 'comment': {
			return [printOpeningTagPrefix(node, options), ...replaceEndOfLineWith(options.originalText.slice(locStart(node), locEnd(node)), literalline), printClosingTagSuffix(node, options)];
		}
		case 'attribute': {
			if(node.value === null){
				return node.rawName;
			...
			return [node.rawName, '=', quote, ...replaceEndOfLineWith(quote === '"' ? value.replace(/"/g, '&quot;') : value.replace(/'/g, '&apos;'), literalline), quote];
		}
		default:
			/* istanbul ignore next */
	...
	if(forceBreakChildren(node)){
		return [
			...
			...path.map((childPath)=>{
				...
				return [!prevBetweenLine ? '' : [prevBetweenLine, forceNextEmptyLine(childNode.prev) ? hardline : ''], printChild(childPath)];
			}, 'children'),
		];
	}
			...
			return [
				...
				...replaceEndOfLineWith(
					options.originalText.slice(
						...
						locEnd(child) - (child.next && needsToBorrowPrevClosingTagEndMarker(child.next) ? printClosingTagEndMarker(child, options).length : 0),
					),
					literalline,
				),
...
function printAttributes(path, options, print) {
	...
	const hasPrettierIgnoreAttribute =
		typeof ignoreAttributeData === 'boolean' ? ()=>ignoreAttributeData : Array.isArray(ignoreAttributeData) ? (attribute)=>ignoreAttributeData.includes(attribute.rawName) : ()=>false;

	const printedAttributes = path.map((attributePath)=>{
		const attribute = attributePath.getValue();
		return hasPrettierIgnoreAttribute(attribute) ? replaceEndOfLineWith(options.originalText.slice(locStart(attribute), locEnd(attribute)), literalline) : print();
	}, 'attrs');
...
function printEmbeddedAttributeValue(node, originalTextToDoc, options) {
	...
	const __onHtmlBindingRoot = (root, options)=>{
		const rootNode =
			root.type === 'NGRoot'
				? root.node.type === 'NGMicrosyntax' && root.node.body.length === 1 && root.node.body[0].type === 'NGMicrosyntaxExpression'
		...
		if(
			...
			(rootNode.type === 'ObjectExpression' ||
				...
				(options.parser === '__vue_expression' && (rootNode.type === 'TemplateLiteral' || rootNode.type === 'StringLiteral')))
		){
			shouldHug = true;
		}
	};
	...
	const printExpand = (doc, canHaveTrailingWhitespace = true)=>group([indent([softline, doc]), canHaveTrailingWhitespace ? softline : '']);
	const printMaybeHug = (doc)=>(shouldHug ? printHug(doc) : printExpand(doc));

	const textToDoc = (code, opts)=>originalTextToDoc(code, {__onHtmlBindingRoot, __embeddedInHtml: true, ...opts}, {stripTrailingHardline: true});
	...
	if(options.parser === 'vue'){
		...
		if(isKeyMatched(vueExpressionBindingPatterns)){
			return printMaybeHug(textToDoc(getValue(), {parser: '__vue_expression'}, {stripTrailingHardline: true}));
		}

		if(isKeyMatched(jsExpressionBindingPatterns)){
			return printMaybeHug(textToDoc(getValue(), {parser: '__js_expression'}, {stripTrailingHardline: true}));
		}
	}
	...
	if(options.parser === 'angular'){
		const ngTextToDoc = (code, opts)=>
			...
			textToDoc(code, {...opts, trailingComma: 'none'}, {stripTrailingHardline: true});
		...
		if(isKeyMatched(ngI18nPatterns)){
			...
			return printExpand(fill(getTextValueParts(node, value)), !value.includes('@@'));
		}

		if(isKeyMatched(ngDirectiveBindingPatterns)){
			return printMaybeHug(ngTextToDoc(getValue(), {parser: '__ng_directive'}));
		}
