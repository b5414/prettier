...
import {hardline, indent, join, line, softline} from '../../document/builders.js';
import {replaceEndOfLine} from '../../document/utils.js';
import isNonEmptyArray from '../../utils/is-non-empty-array.js';
import {locEnd, locStart} from '../loc.js';
import {getLastDescendant, hasPrettierIgnore, isPreLikeNode, isTextLikeNode, isVueSfcBlock, shouldPreserveContent} from '../utils/index.js';
...
function needsToBorrowPrevClosingTagEndMarker(node) {
	 ...
	 */
	return (
		node.prev && node.prev.type !== 'docType' && node.type !== 'angularControlFlowBlock' && !isTextLikeNode(node.prev) && node.isLeadingSpaceSensitive && !node.hasLeadingSpaces
	);
}
...
function printAttributes(path, options, print) {
	...
	const ignoreAttributeData = node.prev?.type === 'comment' && getPrettierIgnoreAttributeCommentData(node.prev.value);
	...
	const printedAttributes = path.map(
		({node: attribute}) => (hasPrettierIgnoreAttribute(attribute) ? replaceEndOfLine(options.originalText.slice(locStart(attribute), locEnd(attribute))) : print()),
		'attrs',
	);

	const forceNotToBreakAttrContent =
		node.type === 'element' && node.fullName === 'script' && node.attrs.length === 1 && node.attrs[0].fullName === 'src' && node.children.length === 0;
