import {breakParent, group, hardline, ifBreak, line, softline} from '../../document/builders.js';
import {replaceEndOfLine} from '../../document/utils.js';
import htmlWhitespaceUtils from '../../utils/html-whitespace-utils.js';
import isNonEmptyArray from '../../utils/is-non-empty-array.js';
import {locEnd, locStart} from '../loc.js';
import {forceBreakChildren, forceNextEmptyLine, hasPrettierIgnore, isTextLikeNode, preferHardlineAsLeadingSpaces} from '../utils/index.js';
import {
...
function getEndLocation(node) {
	...
	if (node.type === 'element' && !node.endSourceSpan && isNonEmptyArray(node.children)) {
			...
			replaceEndOfLine(
				htmlWhitespaceUtils.trimEnd(
					options.originalText.slice(
						...
						endLocation - (child.next && needsToBorrowPrevClosingTagEndMarker(child.next) ? printClosingTagEndMarker(child, options).length : 0),
					),
				),
			),
