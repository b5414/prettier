import {breakParent, dedentToRoot, group, ifBreak, indent, indentIfBreak, line, softline} from '../../document/builders.js';
import {replaceEndOfLine} from '../../document/utils.js';
import getNodeContent from '../get-node-content.js';
import {forceBreakContent, isScriptLikeTag, isVueCustomBlock, shouldPreserveContent} from '../utils/index.js';
...
function printElement(path, options, print) {
	...
	const shouldHugContent =
		...
		(node.firstChild.type === 'interpolation' || node.firstChild.type === 'angularIcuExpression') &&
	...
	const printTag = (doc) => group([group(printOpeningTag(path, options, print), {id: attrGroupId}), doc, printClosingTag(node, options)]);
	...
	const printLineAfterChildren = () => {
		...
		if (
			...
			new RegExp(`\\n[\\t ]{${options.tabWidth * (path.ancestors.length - 1)}}$`, 'u').test(node.lastChild.value)
		) {
			return '';
		}
		return softline;
