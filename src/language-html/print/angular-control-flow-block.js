...
function shouldCloseBlock(node) {
	return !(node.next?.type === 'angularControlFlowBlock' && ANGULAR_CONTROL_FLOW_BLOCK_SETTINGS.get(node.name)?.has(node.next.name));
}

function isPreviousBlockUnClosed(path) {
	const {previous} = path;
	return previous?.type === 'angularControlFlowBlock' && !hasPrettierIgnore(previous) && !shouldCloseBlock(previous);
}

function printAngularControlFlowBlockParameters(path, options, print) {
	return [indent([softline, join([';', line], path.map(print, 'children'))]), softline];
}
