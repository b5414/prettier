...
import {CSS_DISPLAY_DEFAULT, CSS_DISPLAY_TAGS, CSS_WHITE_SPACE_DEFAULT, CSS_WHITE_SPACE_TAGS} from '../constants.evaluate.js';
import isUnknownNamespace from './is-unknown-namespace.js';

const htmlTrimLeadingBlankLines = (string) => string.replaceAll(/^[\t\f\r ]*\n/gu, '');
const htmlTrimPreserveIndentation = (string) => htmlTrimLeadingBlankLines(htmlWhitespaceUtils.trimEnd(string));
...
function isScriptLikeTag(node) {
	return (
		...
		(node.fullName === 'script' ||
			...
			(isUnknownNamespace(node) && (node.name === 'script' || node.name === 'style')))
	);
}
...
function forceNextEmptyLine(node) {
	return isFrontMatter(node) || (node.next && node.sourceSpan.end && node.sourceSpan.end.line + 1 < node.next.sourceSpan.start.line);
}
...
function forceBreakContent(node) {
	return (
		...
		(node.type === 'element' && node.children.length > 0 && (['body', 'script', 'style'].includes(node.name) || node.children.some((child) => hasNonTextChild(child)))) ||
		(node.firstChild &&
			node.firstChild === node.lastChild &&
			node.firstChild.type !== 'text' &&
			hasLeadingLineBreak(node.firstChild) &&
			(!node.lastChild.isTrailingSpaceSensitive || hasTrailingLineBreak(node.lastChild)))
	);
}
...
function forceBreakChildren(node) {
	return (
		...
		(['html', 'head', 'ul', 'ol', 'select'].includes(node.name) || (node.cssDisplay.startsWith('table') && node.cssDisplay !== 'table-cell'))
	);
}
...
function hasLeadingLineBreak(node) {
	return (
		...
		(node.prev ? node.prev.sourceSpan.end.line < node.sourceSpan.start.line : node.parent.type === 'root' || node.parent.startSourceSpan.end.line < node.sourceSpan.start.line)
	);
}
...
function hasTrailingLineBreak(node) {
	return (
		...
		(node.next
			...
			: node.parent.type === 'root' || (node.parent.endSourceSpan && node.parent.endSourceSpan.start.line > node.sourceSpan.end.line))
	);
}
...
function inferScriptParser(node, options) {
	...
	return inferParser(options, {language: lang}) ?? inferParserByTypeAttribute(type);
}
...
function inferElementParser(node, options) {
	return inferScriptParser(node, options) ?? inferStyleParser(node, options) ?? inferVueSfcBlockParser(node, options);
}
...
function getTextValueParts(node, value = node.value) {
	return node.parent.isWhitespaceSensitive
		? node.parent.isIndentationSensitive
			...
			: replaceEndOfLine(dedentString(htmlTrimPreserveIndentation(value)), hardline)
