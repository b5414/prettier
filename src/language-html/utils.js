...
const {CSS_DISPLAY_TAGS, CSS_DISPLAY_DEFAULT, CSS_WHITE_SPACE_TAGS, CSS_WHITE_SPACE_DEFAULT} = require('./constants.evaluate');
...
function isScriptLikeTag(node) {
	return(
		node.type === 'element' &&
		(node.fullName === 'script' || node.fullName === 'style' || node.fullName === 'svg:style' || (isUnknownNamespace(node) && (node.name === 'script' || node.name === 'style')))
	);
}
...
function forceBreakChildren(node) {
	return(
		node.type === 'element' && node.children.length > 0 && (['html', 'head', 'ul', 'ol', 'select'].includes(node.name) || (node.cssDisplay.startsWith('table') && node.cssDisplay !== 'table-cell'))
	);
}
