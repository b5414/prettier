		...
		if(nodes !== this[key]){
			...
			if(key === 'attrs'){
				setNonEnumerableProperties(this, {
					attrMap: fromPairs(this[key].map((attr)=>[attr.fullName, attr.value])),
				});
			}
		}
	}
...
function cloneAndUpdateNodes(nodes, parent) {
	const siblings = nodes.map((node)=>(node instanceof Node ? node.clone() : new Node(node)));
...
function setNonEnumerableProperties(obj, props) {
	const descriptors = fromPairs(Object.entries(props).map(([key, value])=>[key, {value, enumerable: false}]));
