	...
	while (width >= 0) {
		...
		switch (docType) {
			...
			case DOC_TYPE_GROUP: {
				...
				const contents = doc.expandedStates && groupMode === MODE_BREAK ? doc.expandedStates.at(-1) : doc.contents;
		...
		switch (getDocType(doc)) {
			...
			case DOC_TYPE_INDENT_IF_BREAK: {
				...
				if (groupMode === MODE_BREAK) {
					const breakContents = doc.type === DOC_TYPE_IF_BREAK ? doc.breakContents : doc.negate ? doc.contents : indent(doc.contents);
					if (breakContents) {
						cmds.push({ind, mode, doc: breakContents});
					}
				}
				if (groupMode === MODE_FLAT) {
					const flatContents = doc.type === DOC_TYPE_IF_BREAK ? doc.flatContents : doc.negate ? indent(doc.contents) : doc.contents;
					if (flatContents) {
