...
function propagateBreaks(doc) {
	...
	traverseDoc(doc, propagateBreaksOnEnterFn, propagateBreaksOnExitFn, /* shouldTraverseConditionalGroups */ true);
}
...
function stripTrailingHardlineFromParts(parts) {
	...
	while (parts.length >= 2 && parts.at(-2).type === DOC_TYPE_LINE && parts.at(-1).type === DOC_TYPE_BREAK_PARENT) {
		parts.length -= 2;
	}
...
function normalizeParts(parts) {
	...
	while (restParts.length > 0) {
		...
		if (newParts.length > 0 && typeof newParts.at(-1) === 'string' && typeof part === 'string') {
