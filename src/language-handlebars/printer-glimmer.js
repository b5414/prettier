import {dedent, fill, group, hardline, ifBreak, indent, join, line, softline} from '../document/builders.js';
...
function print(path, options, print) {
	...
	switch (node.type) {
		...
		case 'ElementNode': {
			...
			const escapeNextElementNode = options.htmlWhitespaceSensitivity === 'ignore' && path.next?.type === 'ElementNode' ? softline : '';
			...
			if (options.htmlWhitespaceSensitivity === 'ignore') {
				return [startingTag, indent(printChildren(path, options, print)), hardline, indent(endingTag), escapeNextElementNode];
			}

			return [startingTag, indent(group(printChildren(path, options, print))), indent(endingTag), escapeNextElementNode];
		}

		case 'BlockStatement':
			if (isElseIfLike(path)) {
				return [printElseIfLikeBlock(path, print), printProgram(path, print, options), printInverse(path, print, options)];
			}

			return [printOpenBlock(path, print), group([printProgram(path, print, options), printInverse(path, print, options), printCloseBlock(path, print, options)])];
		...
		case 'AttrNode': {
			...
			const quote = isText
				...
				: value.type === 'ConcatStatement'
					? getPreferredQuote(value.parts.map((part) => (part.type === 'TextNode' ? part.chars : '')).join(''), options.singleQuote)
					: '';
		...
		case 'TextNode': {
			...
			if (attrName) {
				...
				if (attrName === 'class') {
					...
					if (path.parent.type === 'ConcatStatement') {
						...
						if (path.next?.type === 'MustacheStatement' && /\s$/u.test(text) && formattedClasses !== '') {
							trailingSpace = true;
						}
					}
			...
			if (options.htmlWhitespaceSensitivity !== 'ignore') {
				...
				const shouldTrimLeadingNewlines = isFirst && path.parent.type === 'Template';
			...
			if ((isFirst || isLast) && isWhitespaceOnly && (path.parent.type === 'Block' || path.parent.type === 'ElementNode' || path.parent.type === 'Template')) {
				return '';
			}
			...
			if (isWhitespaceOnly && lineBreaksCount) {
				...
				if (path.next?.type === 'BlockStatement' || path.next?.type === 'ElementNode') {
					trailingLineBreaksCount = Math.max(trailingLineBreaksCount, 1);
				}

				if (path.previous?.type === 'BlockStatement' || path.previous?.type === 'ElementNode') {
			...
			if (trailingLineBreaksCount === 0 && path.next?.type === 'MustacheStatement') {
				trailingSpace = ' ';
			}

			if (leadingLineBreaksCount === 0 && path.previous?.type === 'MustacheStatement') {
				leadingSpace = ' ';
			}
...
const isPathWithSameHead = (pathA, pathB) => pathA.head.type === 'VarHead' && pathB.head.type === 'VarHead' && pathA.head.name === pathB.head.name;

function isElseIfLike(path) {
	const {grandparent, node} = path;
	return grandparent?.inverse?.body.length === 1 && grandparent.inverse.body[0] === node && isPathWithSameHead(grandparent.inverse.body[0].path, grandparent.path);
}
...
function printElseIfLikeBlock(path, print) {
	...
	return group([
		...
		indent([line, group(printParams(path, print)), ...(isNonEmptyArray(node.program.blockParams) ? [line, printBlockParams(node.program)] : [])]),
		softline,
...
function blockStatementHasOnlyWhitespaceInProgram(node) {
	return node.type === 'BlockStatement' && node.program.body.every((node) => isWhitespaceNode(node));
}

function blockStatementHasElseIfLike(node) {
	return (
		blockStatementHasElse(node) && node.inverse.body.length === 1 && node.inverse.body[0].type === 'BlockStatement' && isPathWithSameHead(node.inverse.body[0].path, node.path)
	);
}
...
function printStringLiteral(path, options) {
	...
	const quote = getPreferredQuote(value, needsOppositeQuote(path) ? !options.singleQuote : options.singleQuote);
...
const PATH_EXPRESSION_FORBIDDEN_IN_FIRST_PART = new Set(['true', 'false', 'null', 'undefined']);
const isPathExpressionPartNeedBrackets = (part, index) => {
	...
	return (
		...
		/\s/u.test(part) ||
		/^\d/u.test(part) ||
		Array.prototype.some.call(part, (character) => PATH_EXPRESSION_FORBIDDEN_CHARACTERS.has(character))
	);
};
function printPathExpression(node) {
	...
	return parts.map((part, index) => (isPathExpressionPartNeedBrackets(part, index) ? `[${part}]` : part)).join('.');
}
