...
const {
	builders: {dedent, fill, group, hardline, ifBreak, indent, join, line, softline, literalline},
	utils: {getDocParts},
} = require('../document');
...
function print(path, options, print) {
	...
	switch(node.type){
		...
		case 'MustacheStatement': {
			const isParentOfSpecifiedTypes = isParentOfSomeType(path, ['AttrNode', 'ConcatStatement']);

			const isChildOfElementNodeAndDoesNotHaveParams = isParentOfSomeType(path, ['ElementNode']) && doesNotHaveHashParams(node) && doesNotHavePositionalParams(node);

			const shouldBreakOpeningMustache = isParentOfSpecifiedTypes || isChildOfElementNodeAndDoesNotHaveParams;

			return group([printOpeningMustache(node), shouldBreakOpeningMustache ? indent(softline) : '', printPathAndParams(path, print), softline, printClosingMustache(node)]);
		}
		...
		case 'AttrNode': {
			...
			const quote = isText
				...
				? chooseEnclosingQuote(options, node.value.parts.filter((part)=>part.type === 'TextNode').map((part)=>part.chars).join('')).quote
				: '';
...
function printStartingTag(path, print) {
	...
	const attributesLike = ['attributes', 'modifiers', 'comments', 'blockParams'].filter((property)=>isNonEmptyArray(node[property])).map((property)=>[
		line,
		property === 'blockParams' ? printBlockParams(node) : join(line, path.map(print, property)),
	]);

	return ['<', node.tag, indent(attributesLike), printStartingTagEndMarker(node)];
}
...
function printOpenBlock(path, print) {
	...
	return group([openingMustache, indent(group(attributes)), softline, closingMustache]);
}
...
function printElseIfBlock(path, print) {
	...
	return [printInverseBlockOpeningMustache(parentNode), 'else ', printPathAndParams(path, print), printInverseBlockClosingMustache(parentNode)];
}
