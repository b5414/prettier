...
function isVoidElement(node) {
	return node.selfClosing === true || isVoidTag(node.tag) || (isGlimmerComponent(node) && node.children.every((node) => isWhitespaceNode(node)));
}
...
function isPrettierIgnoreNode(node) {
	return node?.type === 'MustacheCommentStatement' && typeof node.value === 'string' && node.value.trim() === 'prettier-ignore';
}

function hasPrettierIgnore(path) {
	return (
		isPrettierIgnoreNode(path.node) ||
		(path.isInArray && (path.key === 'children' || path.key === 'body' || path.key === 'parts') && isPrettierIgnoreNode(path.siblings[path.index - 2]))
	);
}
