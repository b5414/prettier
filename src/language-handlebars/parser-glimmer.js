	...
	if (childrenOrBody) {
		for (let i = 0; i < childrenOrBody.length - 1; i++) {
			if (childrenOrBody[i].type === 'TextNode' && childrenOrBody[i + 1].type === 'MustacheStatement') {
				childrenOrBody[i].chars = childrenOrBody[i].chars.replace(/\\$/u, '\\\\');
			}
		}
	}
}
...
function createPlugin(text) {
	...
	const calculateOffset = ({line, column}) => lines.indexForLocation({line: line - 1, column});
...
function getErrorMessage(error) {
  ...
  ```
  */
	if (lines.length >= 4 && /^Parse error on line \d+:$/u.test(lines[0]) && /^-*\^$/u.test(lines.at(-2))) {
		return lines.at(-1);
	}
  ...
  */
	if (
		lines.length >= 4 &&
		/:\s?$/u.test(lines[0]) &&
		/^\(error occurred in '.*?' @ line \d+ : column \d+\)$/u.test(lines.at(-1)) &&
		lines[1] === '' &&
		lines.at(-2) === '' &&
