...
function clean(original, cloned, parent) {
	...
	if (original.type === 'css-comment' && parent.type === 'css-root' && parent.nodes.length > 0) {
		...
		if (parent.nodes[0] === original || (isFrontMatter(parent.nodes[0]) && parent.nodes[1] === original)) {
			/**
			 * something
			 *
			 * @format
	...
	if (
		(original.type === 'value-word' && ((original.isColor && original.isHex) || ['initial', 'inherit', 'unset', 'revert'].includes(original.value.toLowerCase()))) ||
		original.type === 'media-feature' ||
	...
	if (
		...
		cloned.value = cloned.value.replaceAll(/([\d+.e-]+)([a-z]*)/giu, (match, numStr, unit) => {
	...
	if (
		...
		original.groups.some((node) => (node.type === 'value-atword' && node.value.endsWith('[')) || (node.type === 'value-word' && node.value.startsWith(']')))
	) {
		return {
			type: 'value-atword',
