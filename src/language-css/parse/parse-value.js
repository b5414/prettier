...
function parseValueNode(valueNode, options) {
	...
	for (let i = 0; i < nodes.length; ++i) {
		...
		if (options.parser === 'scss' && node.type === 'number' && node.unit === '..' && node.value.endsWith('.')) {
		...
		if (node.type === 'func' && node.value === 'selector') {
			node.group.groups = [parseSelector(getValueRoot(valueNode).text.slice(node.group.open.sourceIndex + 1, node.group.close.sourceIndex))];
		}
		...
		if (node.type === 'func' && node.value === 'url') {
			...
			if (hasSCSSInterpolation(groupList) || (!hasStringOrFunction(groupList) && !isSCSSVariable(groupList[0], options))) {
