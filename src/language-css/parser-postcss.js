...
import {calculateLoc, locEnd, locStart, replaceQuotesInInlineComments} from './loc.js';
	...
	if (node && typeof node === 'object') {
		...
		if (node.type === 'css-decl' && typeof node.prop === 'string' && node.prop.startsWith('--') && typeof node.value === 'string' && node.value.startsWith('{')) {
			...
			if (node.value.trimEnd().endsWith('}')) {
				const textBefore = options.originalText.slice(0, node.source.start.offset);
				const nodeText = 'a'.repeat(node.prop.length) + options.originalText.slice(node.source.start.offset + node.prop.length, node.source.end.offset);
		...
		if (options.parser === 'less' && node.type === 'css-decl' && value.startsWith('extend(')) {
			// extend is missing
		...
		if (node.type === 'css-atrule') {
			...
			if (options.parser === 'less') {
				...
				// `@color :blue;`
				if (!['page', 'nest', 'keyframes'].includes(node.name) && node.params?.[0] === ':') {
					node.variable = true;
