...
const {hasSCSSInterpolation, hasStringOrFunction, isLessParser, isSCSS, isSCSSNestedPropertyNode, isSCSSVariable, stringifyNode} = require('./utils');
	...
	if(node && typeof node === 'object'){
		...
		if(node.type === 'css-atrule'){
			...
			if(isLessParser(options)){
				...
				// `@color :blue;`
				if(!['page', 'nest'].includes(node.name) && node.params && node.params[0] === ':'){
					node.variable = true;
