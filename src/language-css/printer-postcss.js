...
const {printNumber, printString, hasNewline, isFrontMatterNode, isNextLineEmpty, isNonEmptyArray} = require('../common/util');
const {
	builders: {join, line, hardline, softline, group, fill, indent, dedent, ifBreak, breakParent},
	...
	switch(node.type){
		...
		case 'css-rule': {
			return [
				...
				node.nodes
					? [
							node.selector && node.selector.type === 'selector-unknown' && lastLineHasInlineComment(node.selector.value) ? line : ' ',
							'{',
							node.nodes.length > 0 ? indent([hardline, printNodeSequence(path, options, print)]) : '',
							hardline,
							'}',
		...
		case 'css-atrule': {
			...
			if(isLessParser(options)){
				...
				if(node.variable){
					return [
						...
						node.nodes ? ['{', indent([node.nodes.length > 0 ? softline : '', printNodeSequence(path, options, print)]), softline, '}'] : '',
			...
			return [
				...
				node.value ? group([' ', print('value'), isSCSSControlDirectiveNode(node) ? (hasParensAroundNode(node) ? ' ' : line) : '']) : node.name === 'else' ? ' ' : '',
				node.nodes
					? [
							isSCSSControlDirectiveNode(node)
								...
								: ' ',
							'{',
							indent([node.nodes.length > 0 ? softline : '', printNodeSequence(path, options, print)]),
							softline,
							'}',
					  ]
					: isTemplatePlaceholderNodeWithoutSemiColon
		...
		case 'selector-root': {
			return group([
				insideAtRuleNode(path, 'custom-selector') ? [getAncestorNode(path, 'css-atrule').customSelector, line] : '',
		...
		case 'selector-tag': {
			...
			return [
				...
				prevNode.type === 'selector-nesting' ? node.value : adjustNumbers(isKeyframeAtRuleKeywords(path, node.value) ? node.value.toLowerCase() : node.value),
			];
		}
		case 'selector-id': {
			return ['#', node.value];
		...
		case 'value-comma_group': {
			...
			for(let i = 0; i < node.groups.length; ++i){
				...
				if(
					...
					(isAdditionNode(iNode) || isAdditionNode(iNextNode) || isSubtractionNode(iNode) || isSubtractionNode(iNextNode)) &&
				...
				const requireSpaceBeforeOperator =
					(iNextNextNode && iNextNextNode.type === 'value-func') || (iNextNextNode && isWordNode(iNextNextNode)) || iNode.type === 'value-func' || isWordNode(iNode);
				...
				if(
					...
					(hasEmptyRawBefore(iNextNode) || (isMathOperator && (!iPrevNode || (iPrevNode && isMathOperatorNode(iPrevNode)))))
				){
					continue;
				}
				...
				if(
					isControlDirective &&
					(isEqualityOperatorNode(iNextNode) || isRelationalOperatorNode(iNextNode) || isIfElseKeywordNode(iNextNode) || isEachKeywordNode(iNode) || isForKeywordNode(iNode))
				){
					parts.push(' ');
		...
		case 'value-paren_group': {
			...
			const printed = group(
				[
					...
					ifBreak(!isLastItemComment && isSCSS(options.parser, options.originalText) && isSCSSMapItem && shouldPrintComma(options) ? ',' : ''),
					softline,
		...
		case 'value-colon': {
			...
			return [
				...
				(prevNode && typeof prevNode.value === 'string' && getLast(prevNode.value) === '\\') ||
	...
	path.each((pathChild, i, nodes)=>{
		...
		if(i !== nodes.length - 1){
			if(
				...
				(nodes[i + 1].type === 'css-atrule' && nodes[i + 1].name === 'else' && nodes[i].type !== 'css-comment')
			){
				parts.push(' ');
			}else{
