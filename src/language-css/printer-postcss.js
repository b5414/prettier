import {breakParent, dedent, group, hardline, ifBreak, indent, join, line, softline} from '../document/builders.js';
...
import {adjustNumbers, adjustStrings, printCssNumber, printUnit, quoteAttributeValue} from './print/misc.js';
import {printParenthesizedValueGroup, shouldBreakList} from './print/parenthesized-value-group.js';
...
function genericPrint(path, options, print) {
	...
	switch (node.type) {
		...
		case 'css-root': {
			...
			return [node.frontMatter ? [print('frontMatter'), hardline] : '', nodes, after ? ` ${after}` : '', node.nodes.length > 0 ? hardline : ''];
		}
		case 'css-comment': {
		...
		case 'css-rule':
			return [
				...
				node.nodes
					? [
							node.selector?.type === 'selector-unknown' && lastLineHasInlineComment(node.selector.value) ? line : node.selector ? ' ' : '',
							'{',
							node.nodes.length > 0 ? indent([hardline, printSequence(path, options, print)]) : '',
							hardline,
							'}',
		...
		case 'css-decl': {
			...
			if (!isColon && lastLineHasInlineComment(trimmedBetween) && !(node.value?.group?.group && path.call(() => shouldBreakList(path), 'value', 'group', 'group'))) {
			...
			return [
				...
				options.parser === 'less' && node.extend && node.selector ? ['extend(', print('selector'), ')'] : '',
				value,
				...
				node.nodes
					? [' {', indent([softline, printSequence(path, options, print)]), softline, '}']
					: isTemplatePropNode(node) && !parentNode.raws.semicolon && options.originalText[locEnd(node) - 1] !== ';'
						? ''
		...
		case 'css-atrule': {
			...
			if (options.parser === 'less') {
				...
				if (node.function) {
					return [node.name, typeof node.params === 'string' ? node.params : print('params'), isTemplatePlaceholderNodeWithoutSemiColon ? '' : ';'];
				}
				...
				if (node.variable) {
					return [
						...
						node.nodes ? ['{', indent([node.nodes.length > 0 ? softline : '', printSequence(path, options, print)]), softline, '}'] : '',
						isTemplatePlaceholderNodeWithoutSemiColon ? '' : ';',
					];
				}
			}
			const isImportUnknownValueEndsWithSemiColon = node.name === 'import' && node.params?.type === 'value-unknown' && node.params.value.endsWith(';');
			...
			return [
				...
				node.value
					? group([' ', print('value'), isSCSSControlDirectiveNode(node, options) ? (hasParensAroundNode(node) ? ' ' : line) : ''])
					: node.name === 'else'
						? ' '
						: '',
				node.nodes
					? [
							isSCSSControlDirectiveNode(node, options)
								? ''
								: (node.selector && !node.selector.nodes && typeof node.selector.value === 'string' && lastLineHasInlineComment(node.selector.value)) ||
									  (!node.selector && typeof node.params === 'string' && lastLineHasInlineComment(node.params))
									? line
									: ' ',
							'{',
							indent([node.nodes.length > 0 ? softline : '', printSequence(path, options, print)]),
							softline,
							'}',
						]
		...
		case 'media-feature':
			return maybeToLowerCase(adjustStrings(node.value.replaceAll(/ +/gu, ' '), options));
		...
		case 'media-url':
			return adjustStrings(node.value.replaceAll(/^url\(\s+/giu, 'url(').replaceAll(/\s+\)$/gu, ')'), options);
		...
		case 'selector-root':
			return group([
				insideAtRuleNode(path, 'custom-selector') ? [path.findAncestor((node) => node.type === 'css-atrule').customSelector, line] : '',
				join([',', insideAtRuleNode(path, ['extend', 'custom-selector', 'nest']) ? line : hardline], path.map(print, 'nodes')),
			]);
		...
		case 'selector-tag':
			return [
				...
				path.previous?.type === 'selector-nesting' ? node.value : adjustNumbers(isKeyframeAtRuleKeywords(path, node.value) ? node.value.toLowerCase() : node.value),
			];
		...
		case 'selector-attribute':
			return [
				...
				node.attribute.trim(),
				node.operator ?? '',
				node.value ? quoteAttributeValue(adjustStrings(node.value.trim(), options), options) : '',
				node.insensitive ? ' i' : '',
				']',
			];
		...
		case 'selector-combinator': {
			if (node.value === '+' || node.value === '>' || node.value === '~' || node.value === '>>>') {
				const parentNode = path.parent;
				const leading = parentNode.type === 'selector-selector' && parentNode.nodes[0] === node ? '' : line;
		...
		case 'selector-unknown': {
			const ruleAncestorNode = path.findAncestor((node) => node.type === 'css-rule');
			...
			if (parentNode.type === 'value-paren_group' && grandParent?.type === 'value-func' && grandParent.value === 'selector') {
		...
		case 'value-colon': {
			...
			return [
				...
				(typeof previous?.value === 'string' && previous.value.endsWith('\\')) ||
