import {breakParent, dedent, fill, group, hardline, indent, line, softline} from '../../document/builders.js';
...
function printCommaSeparatedValueGroup(path, options, print) {
	...
	const isControlDirective = atRuleAncestorNode && isSCSSControlDirectiveNode(atRuleAncestorNode, options);
    ...
    */
		...
		// Ignore escaped `/`
		if (iPrevNode?.value && iPrevNode.value.indexOf('\\') === iPrevNode.value.length - 1 && iNode.type === 'value-operator' && iNode.value === '/') {
			continue;
		}
		...
		const requireSpaceBeforeOperator = iNextNextNode?.type === 'value-func' || (iNextNextNode && isWordNode(iNextNextNode)) || iNode.type === 'value-func' || isWordNode(iNode);
		const requireSpaceAfterOperator = iNextNode.type === 'value-func' || isWordNode(iNextNode) || iPrevNode?.type === 'value-func' || (iPrevNode && isWordNode(iPrevNode));

		// Space before unary minus followed by a function call.
		if (options.parser === 'scss' && isMathOperator && iNode.value === '-' && iNextNode.type === 'value-func' && locEnd(iNode) !== locStart(iNextNode)) {
			parts.push(' ');
			continue;
		}
		...
		if (iNode.value?.endsWith('#') && iNextNode.value === '{' && isParenGroupNode(iNextNode.group)) {
			continue;
		}
