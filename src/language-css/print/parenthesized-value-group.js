...
import {isConfigurationNode, isKeyInValuePairNode, isKeyValuePairNode, isSCSSMapItemNode, isURLFunctionNode, isVarFunctionNode} from '../utils/index.js';
import {shouldPrintTrailingComma} from './misc.js';

function hasComma({node, parent}, options) {
	return Boolean(node.source && options.originalText.slice(locStart(node), locStart(parent.close)).trimEnd().endsWith(','));
}
...
function printTrailingComma(path, options) {
	...
	if (
		...
		!(path.node.type === 'value-comma_group' && path.node.groups.every((group) => group.type === 'value-comment')) &&
...
function printParenthesizedValueGroup(path, options, print) {
	...
	const groupDocs = path.map(({node}) => (typeof node === 'string' ? node : print()), 'groups');
	...
	if (
		...
		return [node.open ? print('open') : '', join(',', groupDocs), node.close ? print('close') : ''];
	}
	...
	const doc = group([node.open ? print('open') : '', indent([softline, join(line, parts)]), softline, node.close ? print('close') : ''], {
		shouldBreak,
	});
...
function shouldBreakList(path) {
	return path.match(
		(node) => node.type === 'value-paren_group' && !node.open && node.groups.some((node) => node.type === 'value-comma_group'),
