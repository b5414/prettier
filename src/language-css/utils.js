...
function insideICSSRuleNode(path) {
	...
	return(
		ruleAncestorNode && ruleAncestorNode.raws && ruleAncestorNode.raws.selector && (ruleAncestorNode.raws.selector.startsWith(':import') || ruleAncestorNode.raws.selector.startsWith(':export'))
	);
}
...
function hasParensAroundNode(node) {
	return(
		node.value && node.value.group && node.value.group.group && node.value.group.group.type === 'value-paren_group' && node.value.group.group.open !== null && node.value.group.group.close !== null
	);
}
...
function stringifyNode(node) {
	if(node.groups){
		...
		const groups = node.groups.reduce(
			(previousValue, currentValue, index)=>previousValue + stringifyNode(currentValue) + (node.groups[0].type === 'comma_group' && index !== node.groups.length - 1 ? ',' : ''),
			'',
		);
