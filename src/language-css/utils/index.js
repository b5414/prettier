...
function getPropOfDeclNode(path) {
	return path.findAncestor((node) => node.type === 'css-decl')?.prop?.toLowerCase();
}
...
function isKeyframeAtRuleKeywords(path, value) {
	const atRuleAncestorNode = path.findAncestor((node) => node.type === 'css-atrule');
	return atRuleAncestorNode?.name?.toLowerCase().endsWith('keyframes') && ['from', 'to'].includes(value.toLowerCase());
}
...
function insideValueFunctionNode(path, functionName) {
	const funcAncestorNode = path.findAncestor((node) => node.type === 'value-func');
...
function insideURLFunctionInImportAtRuleNode(path) {
	const {node} = path;
	return node.groups[0].value === 'url' && node.groups.length === 2 && path.findAncestor((node) => node.type === 'css-atrule')?.name === 'import';
}
...
function isSCSSControlDirectiveNode(node, options) {
	return options.parser === 'scss' && node.type === 'css-atrule' && ['if', 'else', 'for', 'each', 'while'].includes(node.name);
}
...
function isPostcssSimpleVarNode(currentNode, nextNode) {
	return currentNode.value === '$$' && currentNode.type === 'value-func' && nextNode?.type === 'value-word' && !nextNode.raws.before;
}

function hasComposesNode(node) {
	return node.value?.type === 'value-root' && node.value.group?.type === 'value-value' && node.prop.toLowerCase() === 'composes';
}

function hasParensAroundNode(node) {
	return node.value?.group?.group?.type === 'value-paren_group' && node.value.group.group.open !== null && node.value.group.group.close !== null;
}
...
function isKeyValuePairNode(node) {
	return node.type === 'value-comma_group' && node.groups?.[1]?.type === 'value-colon';
}

function isKeyValuePairInParenGroupNode(node) {
	return node.type === 'value-paren_group' && node.groups?.[0] && isKeyValuePairNode(node.groups[0]);
}
...
function isAtWordPlaceholderNode(node) {
	return node?.type === 'value-atword' && node.value.startsWith('prettier-placeholder-');
}

function isConfigurationNode(node, parentNode) {
	if (node.open?.value !== '(' || node.close?.value !== ')' || node.groups.some((group) => group.type !== 'value-comma_group')) {
		return false;
	}
	if (parentNode.type === 'value-comma_group') {
		...
		if (maybeWithNode?.type === 'value-word' && maybeWithNode.value === 'with') {
			return true;
		}
	}
	return false;
...
function isParenGroupNode(node) {
	return node.type === 'value-paren_group' && node.open?.value === '(' && node.close?.value === ')';
}
