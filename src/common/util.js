...
function printString(raw, options) {
	...
	const enclosingQuote =
		options.parser === 'json' || (options.parser === 'json5' && options.quoteProps === 'preserve' && !options.singleQuote)
			? '"'
...
function inferParserByLanguage(language, options) {
	...
	const matched =
		...
		languages.find(({extensions})=>Array.isArray(extensions) && extensions.includes(`.${language}`));
