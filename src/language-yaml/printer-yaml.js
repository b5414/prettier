...
const {
	builders: {breakParent, fill, group, hardline, join, line, lineSuffix, literalline},
	utils: {getDocParts},
} = require('../document');
...
const {alignWithSpaces, printNextEmptyLine, shouldPrintEndComments} = require('./print/misc');
const {printFlowMapping, printFlowSequence} = require('./print/flow-mapping-sequence');
...
function genericPrint(path, options, print) {
	...
	if(hasPrettierIgnore(path)){
		parts.push(replaceEndOfLineWith(options.originalText.slice(node.position.start.offset, node.position.end.offset).trimEnd(), literalline));
	}else{
	...
	if(shouldPrintEndComments(node)){
		parts.push(
			alignWithSpaces(node.type === 'sequenceItem' ? 2 : 0, [
				...
				join(
					hardline,
					path.map((path)=>[isPreviousLineEmpty(options.originalText, path.getValue(), locStart) ? hardline : '', print()], 'endComments'),
				),
			]),
		);
	}
...
function printFlowScalarContent(nodeType, content, options) {
	...
	return join(
		hardline,
		lineContents.map((lineContentWords)=>fill(getDocParts(join(line, lineContentWords)))),
	);
}
