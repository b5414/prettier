...
import {breakParent, fill, group, hardline, join, line, lineSuffix} from '../document/builders.js';
...
import {printFlowMapping, printFlowSequence} from './print/flow-mapping-sequence.js';
import printMappingItem from './print/mapping-item.js';
import {alignWithSpaces, printNextEmptyLine, shouldPrintEndComments} from './print/misc.js';
...
function genericPrint(path, options, print) {
	...
	if (hasPrettierIgnore(path)) {
		parts.push(replaceEndOfLine(options.originalText.slice(node.position.start.offset, node.position.end.offset).trimEnd()));
	} else {
	...
	if (hasTrailingComment(node) && !isNode(node, ['document', 'documentHead'])) {
		parts.push(
			lineSuffix([
				...
				path.parent.type === 'mappingKey' && path.getParentNode(2).type === 'mapping' && isInlineNode(node) ? '' : breakParent,
	...
	if (shouldPrintEndComments(node)) {
		parts.push(
			alignWithSpaces(node.type === 'sequenceItem' ? 2 : 0, [
				...
				join(
					hardline,
					path.map(({node}) => [isPreviousLineEmpty(options.originalText, locStart(node)) ? hardline : '', print()], 'endComments'),
				),
			]),
		);
	}
...
function printNode(path, options, print) {
	...
	switch (node.type) {
		...
		case 'quoteSingle': {
			...
			if ((node.type === 'quoteSingle' && raw.includes('\\')) || (node.type === 'quoteDouble' && /\\[^"]/u.test(raw))) {
...
function shouldPrintDocumentHeadEndMarker(path, options) {
	...
	if (
		 ...
		 */
		(path.isFirst && /---(?:\s|$)/u.test(options.originalText.slice(locStart(document), locStart(document) + 4))) ||
		/**
		 * %DIRECTIVE
		 * ---
		 */
