...
function isNode(value, types) {
	return typeof value?.type === 'string' && (!types || types.includes(value.type));
}
...
function getLastDescendantNode(node) {
	return isNonEmptyArray(node.children) ? getLastDescendantNode(node.children.at(-1)) : node;
}
...
function hasPrettierIgnore(path) {
	...
	if (node.type === 'documentBody') {
		...
		return hasEndComments(documentHead) && isPrettierIgnore(documentHead.endComments.at(-1));
	}

	return hasLeadingComments(node) && isPrettierIgnore(node.leadingComments.at(-1));
}
...
function getFlowScalarLineContents(nodeType, content, options) {
	...
	return rawLineContents
		.map((lineContent) => (lineContent.length === 0 ? [] : splitWithSingleSpace(lineContent)))
		.reduce(
		...
		.map((lineContentWords) => (options.proseWrap === 'never' ? [lineContentWords.join(' ')] : lineContentWords));
}

function getBlockValueLineContents(node, {parentIndent, isLastDescendant, options}) {
	const content =
	...
	return removeUnnecessaryTrailingNewlines(
		rawLineContents
			...
			.reduce(
				(reduced, lineContentWords, index) =>
					index !== 0 && rawLineContents[index - 1].length > 0 && lineContentWords.length > 0 && !/^\s/u.test(lineContentWords[0]) && !/^\s|\s$/u.test(reduced.at(-1))
						? [...reduced.slice(0, -1), [...reduced.at(-1), ...lineContentWords]]
						: [...reduced, lineContentWords],
				[],
			)
			.map((lineContentWords) =>
				lineContentWords.reduce(
					(reduced, word) =>
						...
						reduced.length > 0 && /\s$/u.test(reduced.at(-1)) ? [...reduced.slice(0, -1), reduced.at(-1) + ' ' + word] : [...reduced, word],
					[],
				),
			)
			.map((lineContentWords) => (options.proseWrap === 'never' ? [lineContentWords.join(' ')] : lineContentWords)),
	...
	function removeUnnecessaryTrailingNewlines(lineContents) {
		if (node.chomping === 'keep') {
			return lineContents.at(-1).length === 0 ? lineContents.slice(0, -1) : lineContents;
		}
