...
function getFlowScalarLineContents(nodeType, content, options) {
	...
	return rawLineContents.map((lineContent)=>(lineContent.length === 0 ? [] : splitWithSingleSpace(lineContent))).reduce(
		(reduced, lineContentWords, index)=>
			...
			!(
				...
				? [...reduced.slice(0, -1), [...getLast(reduced), ...lineContentWords]]
...
function getBlockValueLineContents(node, {parentIndent, isLastDescendant, options}) {
	...
	return removeUnnecessaryTrailingNewlines(
		rawLineContents.map((lineContent)=>(lineContent.length === 0 ? [] : splitWithSingleSpace(lineContent))).reduce(
			(reduced, lineContentWords, index)=>
				index !== 0 && rawLineContents[index - 1].length > 0 && lineContentWords.length > 0 && !/^\s/.test(lineContentWords[0]) && !/^\s|\s$/.test(getLast(reduced))
					? [...reduced.slice(0, -1), [...getLast(reduced), ...lineContentWords]]
					: [...reduced, lineContentWords],
			[],
			...
			lineContentWords.reduce(
				(reduced, word)=>
					...
					reduced.length > 0 && /\s$/.test(getLast(reduced)) ? [...reduced.slice(0, -1), getLast(reduced) + ' ' + word] : [...reduced, word],
				[],
			),
		).map((lineContentWords)=>(options.proseWrap === 'never' ? [lineContentWords.join(' ')] : lineContentWords)),
	);
