...
const {hasLeadingComments, hasMiddleComments, hasTrailingComment, hasEndComments, isNode, isEmptyNode, isInlineNode} = require('../utils');
...
function printMappingItem(node, parentNode, path, print, options) {
	...
	if(hasLeadingComments(value) || !isInlineNode(key.content)){
		return [
			...
			join(
				'',
				path.map(print, 'value', 'leadingComments').map((comment)=>[comment, hardline]),
			),
			': ',
	...
	if(
		...
		(parentNode.type === 'mapping' && hasTrailingComment(key.content) && isInlineNode(value.content)) ||
	...
	return conditionalGroup([[groupedKey, ifBreak(breakValue, flatValue, {groupId})]]);
}
