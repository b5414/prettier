...
import {conditionalGroup, group, hardline, ifBreak, line} from '../../document/builders.js';
import {hasEndComments, hasLeadingComments, hasMiddleComments, hasTrailingComment, isEmptyNode, isInlineNode, isNode} from '../utils.js';
...
function printMappingItem(path, print, options) {
	...
	if (hasLeadingComments(value) || !isInlineNode(key.content)) {
		return ['? ', alignWithSpaces(2, printedKey), hardline, ...path.map(() => [print(), hardline], 'value', 'leadingComments'), ': ', alignWithSpaces(2, printedValue)];
	}
	...
	const groupedKey = group([ifBreak('? '), group(alignWithSpaces(2, printedKey), {id: groupId})]);
	...
	if (
		hasLeadingComments(value.content) ||
		(hasEndComments(value) && value.content && !isNode(value.content, ['mapping', 'sequence'])) ||
		(parent.type === 'mapping' && hasTrailingComment(key.content) && isInlineNode(value.content)) ||
		(isNode(value.content, ['mapping', 'sequence']) && value.content.tag === null && value.content.anchor === null)
	) {
	...
	return conditionalGroup([[groupedKey, ifBreak(explicitMappingValue, implicitMappingValue, {groupId})]]);
}
...
function isAbsolutelyPrintedAsSingleLineNode(node, options) {
	...
	if (
		// backslash-newline
		/\\$/mu.test(options.originalText.slice(node.position.start.offset, node.position.end.offset))
	) {
		return false;
	}
