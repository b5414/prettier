...
import {dedent, dedentToRoot, fill, hardline, join, line, literalline, markAsRoot} from '../../document/builders.js';
import {getBlockValueLineContents, hasIndicatorComment, isLastDescendantNode} from '../utils.js';
import {alignWithSpaces} from './misc.js';

function printBlock(path, print, options) {
	const {node} = path;
	const parentIndent = path.ancestors.filter((node) => node.type === 'sequence' || node.type === 'mapping').length;
