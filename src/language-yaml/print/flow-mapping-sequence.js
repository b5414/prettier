import {hardline, ifBreak, join, line, softline} from '../../document/builders.js';
...
function printFlowMapping(path, print, options) {
	...
	return [
		...
		alignWithSpaces(options.tabWidth, [
			...
			hasEndComments(node) ? [hardline, join(hardline, path.map(print, 'endComments'))] : '',
		]),
...
function printChildren(path, print, options) {
	return path.map(
		({isLast, node, next}) => [print(), isLast ? '' : [',', line, node.position.start.line !== next.position.start.line ? printNextEmptyLine(path, options.originalText) : '']],
		'children',
	);
}
