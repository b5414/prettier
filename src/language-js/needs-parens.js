...
function needsParens(path, options) {
	...
	if (node.type === 'Identifier') {
		...
		// `for ((let.a) of []);`
		if (node.name === 'let') {
			const expression = path.findAncestor((node) => node.type === 'ForOfStatement')?.left;
		...
		if (key === 'object' && node.name === 'let' && parent.type === 'MemberExpression' && parent.computed && !parent.optional) {
			const statement = path.findAncestor((node) => node.type === 'ExpressionStatement' || node.type === 'ForStatement' || node.type === 'ForInStatement');
		...
		if (key === 'expression') {
			switch (node.name) {
				...
				case 'hook':
				case 'type': {
					const ancestorNeitherAsNorSatisfies = path.findAncestor((node) => !isBinaryCastExpression(node));
	...
	if (node.type === 'ObjectExpression' || node.type === 'FunctionExpression' || node.type === 'ClassExpression' || node.type === 'DoExpression') {
		const expression = path.findAncestor((node) => node.type === 'ExpressionStatement')?.expression;
		if (expression && startsWithNoLookaheadToken(expression, (leftmostNode) => leftmostNode === node)) {
			return true;
		}
	}
	...
	if (node.type === 'ObjectExpression') {
		const arrowFunctionBody = path.findAncestor((node) => node.type === 'ArrowFunctionExpression')?.body;
		if (
			arrowFunctionBody &&
			arrowFunctionBody.type !== 'SequenceExpression' && // these have parens added anyway
			arrowFunctionBody.type !== 'AssignmentExpression' &&
			startsWithNoLookaheadToken(arrowFunctionBody, (leftmostNode) => leftmostNode === node)
		) {
			return true;
		}
	}
	...
	switch (parent.type) {
		...
		case 'TypeAnnotation':
			if (path.match(undefined, undefined, (node, key) => key === 'returnType' && node.type === 'ArrowFunctionExpression') && includesFunctionTypeInObjectType(node)) {
				return true;
			}
			break;
		...
		case 'VariableDeclarator':
			...
			if (
				...
				path.match(
					undefined,
					undefined,
					(node, key) => key === 'declarations' && node.type === 'VariableDeclaration',
	...
	switch (node.type) {
		...
		case 'TSFunctionType':
			if (
				path.match(
					...
					(node, key) => key === 'typeAnnotation' && node.type === 'TSTypeAnnotation',
					(node, key) => key === 'returnType' && node.type === 'ArrowFunctionExpression',
				)
			) {
				return true;
			}
		...
		case 'TSTypeOperator':
			return (
				...
				(parent.type === 'TSTypeAnnotation' && path.grandparent.type.startsWith('TSJSDoc'))
			);
		case 'TSTypeQuery':
		...
		case 'TypeOperator':
			return (
				...
				(key === 'objectType' && (parent.type === 'IndexedAccessType' || parent.type === 'OptionalIndexedAccessType')) ||
		...
		case 'FunctionTypeAnnotation': {
			if (node.type === 'ComponentTypeAnnotation' && (node.rendersType === null || node.rendersType === undefined)) {
				return false;
			}
			...
			if (
				path.match(
					undefined,
					(node, key) => key === 'typeAnnotation' && node.type === 'TypeAnnotation',
					(node, key) => key === 'returnType' && node.type === 'ArrowFunctionExpression',
				)
			) {
				return true;
      ...
      */
			if (
				path.match(
					undefined,
					(node, key) => key === 'typeAnnotation' && node.type === 'TypePredicate',
					(node, key) => key === 'typeAnnotation' && node.type === 'TypeAnnotation',
					(node, key) => key === 'returnType' && node.type === 'ArrowFunctionExpression',
				)
			) {
			...
			return (
				...
				(key === 'objectType' && (ancestor.type === 'IndexedAccessType' || ancestor.type === 'OptionalIndexedAccessType')) ||
				(key === 'checkType' && parent.type === 'ConditionalTypeAnnotation') ||
				(key === 'extendsType' && parent.type === 'ConditionalTypeAnnotation' && node.returnType?.type === 'InferTypeAnnotation' && node.returnType?.typeParameter.bound) ||
				...
				(parent.type === 'FunctionTypeParam' &&
					parent.name === null &&
					getFunctionParameters(node).some((param) => param.typeAnnotation?.type === 'NullableTypeAnnotation'))
			);
		}
...
function includesFunctionTypeInObjectType(node) {
	return hasNode(node, (node) => node.type === 'ObjectTypeAnnotation' && hasNode(node, (node) => node.type === 'FunctionTypeAnnotation'));
}
...
function shouldWrapFunctionForExportDefault(path, options) {
	...
	return path.call(() => shouldWrapFunctionForExportDefault(path, options), ...getLeftSidePathName(node));
}
...
function shouldAddParenthesesToChainElement(path) {
	...
	if (
		...
		((key === 'object' && parent.type === 'MemberExpression') ||
			(key === 'callee' && (parent.type === 'CallExpression' || parent.type === 'NewExpression')) ||
			(parent.type === 'TSNonNullExpression' && grandparent.type === 'MemberExpression' && grandparent.object === parent))
	) {
		return true;
	}
	...
	if (
		...
		(path.match(
			...
			undefined,
			(node, name) =>
				(name === 'callee' && ((node.type === 'CallExpression' && !node.optional) || node.type === 'NewExpression')) ||
				(name === 'object' && node.type === 'MemberExpression' && !node.optional),
		) ||
			path.match(
				undefined,
				undefined,
