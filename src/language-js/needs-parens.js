...
function needsParens(path, options) {
	...
	switch(parent.type){
		...
		case 'ExpressionStatement': {
			if(startsWithNoLookaheadToken(node, /* forbidFunctionClassAndDoExpr */ true)){
				return true;
			}
			break;
		}
		case 'ArrowFunctionExpression': {
			if(
				...
				startsWithNoLookaheadToken(node, /* forbidFunctionClassAndDoExpr */ false)
			){
				return true;
			}
			break;
	...
	switch(node.type){
		...
		case 'YieldExpression':
			...
			if(name === 'expression' && node.argument && node.argument.type === 'PipelinePrimaryTopicReference' && parent.type === 'PipelineTopicExpression'){
				return true;
			}
		...
		case 'TSTypeOperator':
			return(
				...
				(parent.type === 'TSTypeAnnotation' && /^TSJSDoc/.test(path.getParentNode(1).type))
			);
		...
		case 'FunctionTypeAnnotation': {
			...
			return(
				...
				(parent.type === 'FunctionTypeParam' &&
					parent.name === null &&
					getFunctionParameters(node).some((param)=>param.typeAnnotation && param.typeAnnotation.type === 'NullableTypeAnnotation'))
			);
		}
		...
		case 'OptionalCallExpression': {
			...
			if(
				...
				(name === 'callee' && (parent.type === 'CallExpression' || parent.type === 'NewExpression')) ||
				(parent.type === 'TSNonNullExpression' && parentParent.type === 'MemberExpression' && parentParent.object === parent)
			){
				return true;
			}
		}
		...
		case 'JSXElement':
			return(
				name === 'callee' ||
				(name === 'left' && parent.type === 'BinaryExpression' && parent.operator === '<') ||
				(parent.type !== 'ArrayExpression' &&
...
function includesFunctionTypeInObjectType(node) {
	return hasNode(node, (n1)=>(n1.type === 'ObjectTypeAnnotation' && hasNode(n1, (n2)=>n2.type === 'FunctionTypeAnnotation' || undefined)) || undefined);
}
