import {isArrayOrTupleExpression, isNumericLiteral, isStringLiteral} from './utils/index.js';
...
function clean(original, cloned, parent) {
	...
	if (
		...
		original.openingElement.attributes.some((attr) => attr.type === 'JSXAttribute' && attr.name.name === 'jsx')
	) {
	...
	if (original.type === 'JSXAttribute' && original.value?.type === 'Literal' && /["']|&quot;|&apos;/u.test(original.value.value)) {
		cloned.value.value = original.value.value.replaceAll(/["']|&quot;|&apos;/gu, '"');
	}
	...
	if (original.type === 'TemplateLiteral') {
		...
		const hasLanguageComment = original.leadingComments?.some(
			(comment) => isBlockComment(comment) && ['GraphQL', 'HTML'].some((languageName) => comment.value === ` ${languageName} `),
		);
		if (
			hasLanguageComment ||
