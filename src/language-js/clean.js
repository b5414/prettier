...
const ignoredProperties = new Set(['range', 'raw', 'comments', 'leadingComments', 'trailingComments', 'innerComments', 'extra', 'start', 'end', 'loc', 'flags', 'errors', 'tokens']);
...
function clean(ast, newObj, parent) {
	...
	if(
		...
		ast.key &&
		(ast.key.type === 'Literal' || ast.key.type === 'NumericLiteral' || ast.key.type === 'StringLiteral' || ast.key.type === 'Identifier')
	){
		delete newObj.key;
	}
	...
	if(
		...
		(ast.tag.type === 'MemberExpression' ||
			(ast.tag.type === 'Identifier' &&
				(ast.tag.name === 'gql' || ast.tag.name === 'graphql' || ast.tag.name === 'css' || ast.tag.name === 'md' || ast.tag.name === 'markdown' || ast.tag.name === 'html')) ||
	...
	if(ast.type === 'TemplateLiteral'){
		...
		const hasLanguageComment =
			ast.leadingComments && ast.leadingComments.some((comment)=>isBlockComment(comment) && ['GraphQL', 'HTML'].some((languageName)=>comment.value === ` ${languageName} `));
		if(
			hasLanguageComment ||
