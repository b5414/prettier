import {group, hardline, indent, label, line} from '../../document/builders.js';
import {mapDoc} from '../../document/utils.js';
import {printTemplateExpressions, uncookTemplateElementValue} from '../print/template-literal.js';
...
async function printEmbedHtmlLike(parser, textToDoc, print, path, options) {
	...
	const placeholderRegex = new RegExp(composePlaceholder(String.raw`(\d+)`), 'gu');
	let topLevelCount = 0;
			...
			if (i % 2 === 0) {
				if (component) {
					...
					if (options.__embeddedInHtml) {
						component = component.replaceAll(/<\/(?=script\b)/giu, String.raw`<\/`);
					}
	...
	return label({hug: false}, group(['`', leadingWhitespace, topLevelCount > 1 ? indent(group(contentDoc)) : group(contentDoc), trailingWhitespace, '`']));
}
