...
const {getLast, getNextNonSpaceNonCommentCharacter} = require('../common/util');
...
function postprocess(ast, options) {
	...
	if(options.parser === 'typescript' && options.originalText.includes('@')){
		const {esTreeNodeToTSNodeMap, tsNodeToESTreeNodeMap} = options.tsParseResult;
		ast = visitNode(ast, (node)=>{
			...
			if(
				...
				throw createError('Leading decorators must be attached to a class declaration', {
	...
	if(options.parser !== 'typescript' && options.parser !== 'flow' && options.parser !== 'espree' && options.parser !== 'meriyah'){
	...
	ast = visitNode(ast, (node)=>{
		switch(node.type){
			...
			case 'ClassProperty':
				...
				if(node.key && node.key.type === 'TSPrivateIdentifier' && getNextNonSpaceNonCommentCharacter(options.originalText, node.key, locEnd) === '?'){
					node.optional = true;
				}
				break;
		}
