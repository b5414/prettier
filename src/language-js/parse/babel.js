...
function isFlowFile(text, options) {
	...
	const firstNonSpaceNonCommentCharacterIndex = getNextNonSpaceNonCommentCharacterIndex(text, 0);
...
function parseWithOptions(parse, text, options) {
	...
	const error = ast.errors.find((error) => !allowedReasonCodes.has(error.reasonCode));
	if (error) {
		throw error;
	}
	return ast;
...
function createParse({isExpression = false, optionsCombinations}) {
	return (text, options = {}) => {
		if ((options.parser === 'babel' || options.parser === '__babel_estree') && isFlowFile(text, options)) {
		...
		let ast;
		try {
			ast = tryCombinations(combinations.map((options) => () => parseWithOptions(parseFunction, text, options)));
...
const babelTs = createBabelParser({
	optionsCombinations: [appendPlugins(['jsx', 'typescript']), appendPlugins(['typescript'])],
});
...
const babelFlow = createBabelParser({
	optionsCombinations: [appendPlugins(['jsx', ['flow', {all: true, enums: true}], 'flowComments'])],
});
const babelEstree = createBabelParser({
	optionsCombinations: babelParserOptionsCombinations.map((options) => appendPlugins(['estree'], options)),
});
