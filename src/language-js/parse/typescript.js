...
const isKnownFileType = (filepath) => /\.(?:js|mjs|cjs|jsx|ts|mts|cts|tsx)$/iu.test(filepath);
...
function parse(text, options) {
	...
	let ast;
	try {
		ast = tryCombinations(parseOptionsCombinations.map((parseOptions) => () => parseTypeScript(textToParse, parseOptions)));
	} catch ({
