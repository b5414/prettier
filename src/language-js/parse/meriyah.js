...
function createParseError(error) {
	...
	const matches = message.match(/^\[(?<line>\d+):(?<column>\d+)\]: (?<message>.*)$/u)?.groups;
...
function parse(text, options = {}) {
	...
	const combinations = (sourceType ? [sourceType] : ['module', 'script']).map((sourceType) => () => parseWithOptions(text, sourceType));
