...
function isStyledJsx(path) {
	...
	return(
		...
		(parent && parent.type === 'TaggedTemplateExpression' && parent.tag.type === 'Identifier' && parent.tag.name === 'css') ||
		(parent &&
			parent.type === 'TaggedTemplateExpression' &&
			parent.tag.type === 'MemberExpression' &&
			parent.tag.object.name === 'css' &&
			(parent.tag.property.name === 'global' || parent.tag.property.name === 'resolve'))
	);
}
...
function isAngularComponentStyles(path) {
	return path.match(
		...
		(node, name)=>(node.type === 'Property' || node.type === 'ObjectProperty') && node.key.type === 'Identifier' && node.key.name === 'styles' && name === 'value',
...
function isAngularComponentTemplate(path) {
	return path.match(
		(node)=>node.type === 'TemplateLiteral',
		(node, name)=>(node.type === 'Property' || node.type === 'ObjectProperty') && node.key.type === 'Identifier' && node.key.name === 'template' && name === 'value',
		...angularComponentObjectExpressionPredicates,
	);
}
...
function isGraphQL(path) {
	...
	return(
		...
		(parent &&
			((parent.type === 'TaggedTemplateExpression' &&
				((parent.tag.type === 'MemberExpression' && parent.tag.object.name === 'graphql' && parent.tag.property.name === 'experimental') ||
					(parent.tag.type === 'Identifier' && (parent.tag.name === 'gql' || parent.tag.name === 'graphql')))) ||
...
function hasLanguageComment(node, languageName) {
	...
	// Also see ./clean.js
	return hasComment(node, CommentCheckFlags.Block | CommentCheckFlags.Leading, ({value})=>value === ` ${languageName} `);
}
