...
const {printHtmlBinding, isVueEventBindingExpression} = require('./print/html-binding');
...
const {printOptionalToken, printBindExpressionCallee, printTypeAnnotation, adjustClause} = require('./print/misc');
const {printImportDeclaration, printExportDeclaration, printExportAllDeclaration, printModuleSpecifier} = require('./print/module');
...
const {printClass, printClassMethod, printClassProperty} = require('./print/class');
const {printTypeParameters} = require('./print/type-parameters');
const {printPropertyKey, printProperty} = require('./print/property');
const {printFunctionDeclaration, printArrowFunctionExpression, printMethod, printReturnStatement, printThrowStatement} = require('./print/function');
const {printCallExpression} = require('./print/call-expression');
const {printInterface} = require('./print/interface');
const {printVariableDeclarator, printAssignmentExpression} = require('./print/assignment');
...
function printPathNoParens(path, options, print, args) {
	...
	switch(node.type){
		...
		case 'ForStatement': {
			...
			const dangling = printDanglingComments(path, options, /* sameLine */ true);
		...
		case 'EnumSymbolBody': {
			...
			if(node.members.length === 0 && !node.hasUnknownMembers){
				...
				parts.push(group(['{', indent([...members, ...(node.hasUnknownMembers ? [hardline, '...'] : [])]), printDanglingComments(path, options, /* sameIndent */ true), hardline, '}']));
			}
			return parts;
		}
