import {addDanglingComment, addLeadingComment, addTrailingComment} from '../../main/comments/utils.js';
...
function addBlockStatementFirstComment(node, comment) {
	// @ts-expect-error
	const firstNonEmptyNode = (node.body || node.properties).find(({type}) => type !== 'EmptyStatement');
...
function handleNestedConditionalExpressionComments({comment, enclosingNode, followingNode, options}) {
	if (!options.experimentalTernaries) {
		return false;
	}

	const enclosingIsCond = enclosingNode?.type === 'ConditionalExpression' || enclosingNode?.type === 'ConditionalTypeAnnotation' || enclosingNode?.type === 'TSConditionalType';

	if (!enclosingIsCond) {
		return false;
	}

	const followingIsCond = followingNode?.type === 'ConditionalExpression' || followingNode?.type === 'ConditionalTypeAnnotation' || followingNode?.type === 'TSConditionalType';
...
function handleConditionalExpressionComments({comment, precedingNode, enclosingNode, followingNode, text, options}) {
	const isSameLineAsPrecedingNode = precedingNode && !hasNewlineInRange(text, locEnd(precedingNode), locStart(comment));

	if (
		(!precedingNode || !isSameLineAsPrecedingNode) &&
		(enclosingNode?.type === 'ConditionalExpression' || enclosingNode?.type === 'ConditionalTypeAnnotation' || enclosingNode?.type === 'TSConditionalType') &&
		followingNode
	) {
		if (
			options.experimentalTernaries &&
			enclosingNode.alternate === followingNode &&
			!(isBlockComment(comment) && !hasNewlineInRange(options.originalText, locStart(comment), locEnd(comment)))
		) {
			addDanglingComment(enclosingNode, comment);
			return true;
		}
...
const classLikeNodeTypes = new Set(['ClassDeclaration', 'ClassExpression', 'DeclareClass', 'DeclareInterface', 'InterfaceDeclaration', 'TSInterfaceDeclaration']);
function handleClassComments({comment, precedingNode, enclosingNode, followingNode}) {
	if (classLikeNodeTypes.has(enclosingNode?.type)) {
		if (isNonEmptyArray(enclosingNode.decorators) && !(followingNode?.type === 'Decorator')) {
		...
		if (followingNode) {
			if (
				...
				precedingNode &&
				(precedingNode === enclosingNode.id || precedingNode === enclosingNode.typeParameters)
			) {
				addTrailingComment(precedingNode, comment);
				return true;
			}
...
function handleMethodNameComments({comment, precedingNode, enclosingNode, text}) {
	...
	if (
		...
		(enclosingNode.type === 'Property' || enclosingNode.type === 'TSDeclareMethod' || enclosingNode.type === 'TSAbstractMethodDefinition') &&
	...
	if (precedingNode?.type === 'Decorator' && propertyLikeNodeTypes.has(enclosingNode?.type)) {
		addTrailingComment(precedingNode, comment);
		return true;
	}
...
const functionLikeNodeTypes = new Set(['FunctionDeclaration', 'FunctionExpression', 'ClassMethod', 'MethodDefinition', 'ObjectMethod']);
...
function handleLastComponentArgComments({comment, precedingNode, enclosingNode, followingNode, text}) {
	// "DeclareComponent" and "ComponentTypeAnnotation" definitions
	if (
		precedingNode?.type === 'ComponentTypeParameter' &&
		(enclosingNode?.type === 'DeclareComponent' || enclosingNode?.type === 'ComponentTypeAnnotation') &&
	...
	if (
		(precedingNode?.type === 'ComponentParameter' || precedingNode?.type === 'RestElement') &&
...
function handleLastFunctionArgComments({comment, precedingNode, enclosingNode, followingNode, text}) {
	...
	if (
		...
		(enclosingNode?.type === 'FunctionDeclaration' || enclosingNode?.type === 'FunctionExpression' || enclosingNode?.type === 'ObjectMethod') &&
		followingNode?.type === 'BlockStatement' &&
		enclosingNode.body === followingNode
	) {
		const characterAfterCommentIndex = getNextNonSpaceNonCommentCharacterIndex(text, locEnd(comment));
...
function handleOnlyComments({comment, enclosingNode, ast, isLastComment}) {
	...
	if (enclosingNode?.type === 'Program' && enclosingNode.body.length === 0 && !isNonEmptyArray(enclosingNode.directives)) {
		if (isLastComment) {
			addDanglingComment(enclosingNode, comment);
		} else {
			addLeadingComment(enclosingNode, comment);
...
function handleModuleSpecifiersComments({comment, precedingNode, enclosingNode, text}) {
	...
	const isExportDeclaration = precedingNode?.type === 'ExportSpecifier' && enclosingNode?.type === 'ExportNamedDeclaration';
...
const assignmentLikeNodeTypes = new Set(['VariableDeclarator', 'AssignmentExpression', 'TypeAlias', 'TSTypeAliasDeclaration']);
...
function handleVariableDeclaratorComments({comment, enclosingNode, followingNode}) {
	if (assignmentLikeNodeTypes.has(enclosingNode?.type) && followingNode && (complexExprNodeTypes.has(followingNode.type) || isBlockComment(comment))) {
		addLeadingComment(followingNode, comment);
		return true;
	}
	return false;
...
function handleTSFunctionTrailingComments({comment, enclosingNode, followingNode, text}) {
	if (
		!followingNode &&
		(enclosingNode?.type === 'TSMethodSignature' || enclosingNode?.type === 'TSDeclareFunction' || enclosingNode?.type === 'TSAbstractMethodDefinition') &&
		getNextNonSpaceNonCommentCharacter(text, locEnd(comment)) === ';'
	) {
		addTrailingComment(enclosingNode, comment);
		return true;
...
function handleIgnoreComments({comment, enclosingNode, followingNode}) {
	if (isPrettierIgnoreComment(comment) && enclosingNode?.type === 'TSMappedType' && followingNode?.type === 'TSTypeParameter' && followingNode.constraint) {
...
function handleSwitchDefaultCaseComments({comment, enclosingNode, followingNode}) {
 ...
 * @returns {boolean}
 */
function handleLastUnionElementInExpression({comment, precedingNode, enclosingNode, followingNode}) {
	if (
		isUnionType(precedingNode) &&
		(((enclosingNode.type === 'TSArrayType' || enclosingNode.type === 'ArrayTypeAnnotation') && !followingNode) || isIntersectionType(enclosingNode))
	) {
 ...
 *
 */
function handleCommentsInDestructuringPattern({comment, enclosingNode, precedingNode, followingNode}) {
...
export {handleEndOfLineComment as endOfLine, handleOwnLineComment as ownLine, handleRemainingComment as remaining};
