...
const {getNextNonSpaceNonCommentCharacterIndexWithStartIndex, getShebang} = require('../common/util');
...
function createParse(parseMethod, ...optionsCombinations) {
	return (text, parsers, opts = {})=>{
		...
		const {result: ast, error} = tryCombinations(...combinations.map((options)=>()=>parseWithOptions(parseMethod, text, options)));
...
const parseFlow = createParse('parse', appendPlugins(['jsx', ['flow', {all: true, enums: true}]]));
const parseTypeScript = createParse('parse', appendPlugins(['jsx', 'typescript']), appendPlugins(['typescript']));
