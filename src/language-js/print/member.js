...
function printMemberExpression(path, options, print) {
	...
	const shouldInline =
		...
		node.computed ||
		(node.object.type === 'Identifier' && node.property.type === 'Identifier' && !isMemberExpression(parent));

	return [print('object'), shouldInline ? printMemberLookup(path, options, print) : group(indent([softline, printMemberLookup(path, options, print)]))];
}
