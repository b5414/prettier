...
const {
	builders: {line, hardline, softline, group, indent, conditionalGroup, ifBreak, breakParent},
	utils: {willBreak},
} = require('../../document');
...
function printCallArguments(path, options, print) {
	...
	if(args.length === 0){
		return ['(', printDanglingComments(path, options, /* sameIndent */ true), ')'];
	}
	...
	function shouldBreakForArrowFunctionInArguments(arg, argPath) {
		if(!arg || arg.type !== 'ArrowFunctionExpression' || !arg.body || arg.body.type !== 'BlockStatement' || getFunctionParameters(arg).length === 0){
			return false;
		}
	...
	const maybeTrailingComma =
		...
		!(isDynamicImport || (node.callee && node.callee.type === 'Import')) && shouldPrintComma(options, 'all') ? ',' : '';

	function allArgsBrokenOut() {
		return group(['(', indent([line, ...printedArguments]), maybeTrailingComma, line, ')'], {shouldBreak: true});
	}

	if(path.getParentNode().type !== 'Decorator' && isFunctionCompositionArgs(args)){
	...
	if(shouldGroupFirst || shouldGroupLast){
		const shouldBreak = (shouldGroupFirst ? printedArguments.slice(1).some(willBreak) : printedArguments.slice(0, -1).some(willBreak)) || anyArgEmptyLine || shouldBreakForArrowFunction;
		...
		iterateCallArgumentsPath(path, (argPath, i)=>{
			if(shouldGroupFirst && i === 0){
				printedExpanded = [
					[print([], {expandFirstArg: true}), printedArguments.length > 1 ? ',' : '', hasEmptyLineFollowingFirstArg ? hardline : line, hasEmptyLineFollowingFirstArg ? hardline : ''],
		...
		return [
			...
			conditionalGroup(
				[
					!somePrintedArgumentsWillBreak && !node.typeArguments && !node.typeParameters ? simpleConcat : ifBreak(allArgsBrokenOut(), simpleConcat),
					shouldGroupFirst
...
function couldGroupArg(arg, arrowChainRecursion = false) {
	return(
		(arg.type === 'ObjectExpression' && (arg.properties.length > 0 || hasComment(arg))) ||
		(arg.type === 'ArrayExpression' && (arg.elements.length > 0 || hasComment(arg))) ||
		...
		(arg.type === 'ArrowFunctionExpression' &&
			...
			(arg.body.type === 'BlockStatement' ||
				(arg.body.type === 'ArrowFunctionExpression' && couldGroupArg(arg.body, true)) ||
				arg.body.type === 'ObjectExpression' ||
				arg.body.type === 'ArrayExpression' ||
				(!arrowChainRecursion && (isCallExpression(arg.body) || arg.body.type === 'ConditionalExpression')) ||
				isJsxNode(arg.body)))
	);
}
...
function shouldGroupLastArg(args, options) {
	...
	return(
		...
		(args.length !== 2 || penultimateArg.type !== 'ArrowFunctionExpression' || lastArg.type !== 'ArrayExpression') &&
		!(args.length > 1 && lastArg.type === 'ArrayExpression' && isConciselyPrintedArray(lastArg, options))
	);
}
...
function shouldGroupFirstArg(args) {
	...
	return(
		!hasComment(firstArg) &&
		(firstArg.type === 'FunctionExpression' || (firstArg.type === 'ArrowFunctionExpression' && firstArg.body.type === 'BlockStatement')) &&
...
function isNonEmptyBlockStatement(node) {
	return node.type === 'BlockStatement' && (node.body.some((node)=>node.type !== 'EmptyStatement') || hasComment(node, CommentCheckFlags.Dangling));
}
