...
function couldExpandArg(arg, arrowChainRecursion = false) {
	return (
		(isObjectOrRecordExpression(arg) && (arg.properties.length > 0 || hasComment(arg))) ||
		(isArrayOrTupleExpression(arg) && (arg.elements.length > 0 || hasComment(arg))) ||
		...
		(arg.type === 'ArrowFunctionExpression' &&
			...
			(arg.body.type === 'BlockStatement' ||
				(arg.body.type === 'ArrowFunctionExpression' && couldExpandArg(arg.body, true)) ||
...
function shouldExpandLastArg(args, argDocs, options) {
	...
	return (
		...
		(args.length !== 2 || penultimateArg.type !== 'ArrowFunctionExpression' || !isArrayOrTupleExpression(lastArg)) &&
...
function isReactHookCallWithDepsArray(args) {
	...
	if (args.length === 3) {
		 ...
		 * }, [dep1, dep2, dep2]);
		 */
		return args[0].type === 'Identifier' && isValidHookCallbackAndDepsFormat(args, /* baseIndex */ 1);
	}
	return false;
}
