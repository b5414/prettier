import {group, hardline, ifBreak, indent, join, line, softline} from '../../document/builders.js';
...
import {CommentCheckFlags, createTypeCheckFunction, hasComment, isStringLiteral, needsHardlineAfterDanglingComment, rawText, shouldPrintComma} from '../utils/index.js';
...
const isDefaultExport = (node) => node.type === 'ExportDefaultDeclaration' || (node.type === 'DeclareExportDeclaration' && node.default);
...
function printExportDeclaration(path, options, print) {
	...
	if (declaration) {
		...
		parts.push(printModuleSource(path, options, print), printImportAttributes(path, options, print));
	}
...
function printSemicolonAfterExportDeclaration(node, options) {
	if (options.semi && (!node.declaration || (isDefaultExport(node) && !shouldOmitSemicolon(node.declaration)))) {
		return ';';
	}
...
function printModuleSpecifiers(path, options, print) {
	...
	if (isNonEmptyArray(node.specifiers)) {
		...
		if (groupedSpecifiers.length > 0) {
			...
			const canBreak = groupedSpecifiers.length > 1 || standaloneSpecifiers.length > 0 || node.specifiers.some((node) => hasComment(node));

			if (canBreak) {
				parts.push(
					group([
						'{',
						indent([options.bracketSpacing ? line : softline, join([',', line], groupedSpecifiers)]),
...
function shouldPrintSpecifiers(node, options) {
	if (node.type !== 'ImportDeclaration' || isNonEmptyArray(node.specifiers) || node.importKind === 'type') {
		return true;
	}

	const text = getTextWithoutComments(options, locStart(node), locStart(node.source));
...
function getImportAttributesKeyword(node, options) {
	...
	const textBetweenSourceAndAttributes = getTextWithoutComments(options, locEnd(node.source), node.attributes?.[0] ? locStart(node.attributes[0]) : locEnd(node)).trimStart();
...
function isShorthandSpecifier(specifier) {
	...
	const {local, [specifier.type === 'ImportSpecifier' ? 'imported' : 'exported']: importedOrExported} = specifier;
...
export {printExportDeclaration, printImportDeclaration, printImportKind, printModuleSpecifier};
