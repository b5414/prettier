...
import {getCallArguments, isCallExpression, isMemberish, isStringLiteral, isTemplateOnItsOwnLine, isTestCall, iterateCallArgumentsPath} from '../utils/index.js';
...
function printCallExpression(path, options, print) {
	...
	const isTemplateLiteralSingleArg = args.length === 1 && isTemplateOnItsOwnLine(args[0], options.originalText);
	...
	if (
		...
		!path.call((path) => pathNeedsParens(path, options), 'callee', ...(node.callee.type === 'ChainExpression' ? ['expression'] : []))
	) {
