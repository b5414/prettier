...
const {getCallArguments, hasFlowAnnotationComment, isCallExpression, isMemberish, isStringLiteral, isTemplateOnItsOwnLine, isTestCall, iterateCallArgumentsPath} = require('../utils');
...
function printCallExpression(path, options, print) {
	...
	const isIdentifierWithFlowAnnotation =
		(options.parser === 'babel' || options.parser === 'babel-flow') && node.callee && node.callee.type === 'Identifier' && hasFlowAnnotationComment(node.callee.trailingComments);
	...
	if(!isDynamicImport && !isNew && isMemberish(node.callee) && !path.call((path)=>pathNeedsParens(path, options), 'callee')){
	...
	const contents = [
		...
		optional,
		isIdentifierWithFlowAnnotation ? `/*:: ${node.callee.trailingComments[0].value.slice(2).trim()} */` : '',
...
function isCommonsJsOrAmdCall(node, parentNode) {
	...
	if(node.callee.name === 'define'){
		...
		return(
			...
			(args.length === 1 || (args.length === 2 && args[0].type === 'ArrayExpression') || (args.length === 3 && isStringLiteral(args[0]) && args[1].type === 'ArrayExpression'))
		);
	}
