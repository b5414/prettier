import {group, hardline, indent, join, line, softline} from '../../document/builders.js';
...
import {printAssignmentExpression, printVariableDeclarator} from './assignment.js';
...
import {adjustClause, printBindExpressionCallee, printDeclareToken, printDefiniteToken, printOptionalToken, printRestSpread} from './misc.js';
import {printExportDeclaration, printImportDeclaration, printModuleSpecifier} from './module.js';
import {printObject} from './object.js';
import {printProperty} from './property.js';
import {printStatementSequence} from './statement.js';
import {printTaggedTemplateLiteral, printTemplateLiteral} from './template-literal.js';
...
function printEstree(path, options, print, args) {
	...
	switch (node.type) {
		...
		case 'BindExpression':
			...
			return parts;
		case 'Identifier':
			return [node.name, printOptionalToken(path), printDefiniteToken(path), printTypeAnnotationProperty(path, print)];
		...
		case 'AwaitExpression':
			...
			if (node.argument) {
				...
				if ((isCallExpression(parent) && parent.callee === node) || (isMemberExpression(parent) && parent.object === node)) {
					...
					if (parentAwaitOrBlock?.type !== 'AwaitExpression' || !startsWithNoLookaheadToken(parentAwaitOrBlock.argument, (leftmostNode) => leftmostNode === node)) {
						return group(parts);
					}
				}
			}
		...
		case 'UpdateExpression':
			return [node.prefix ? node.operator : '', print('argument'), node.prefix ? '' : node.operator];
			...
			if (node.param) {
				const parameterHasComments = hasComment(
					...
					(comment) =>
						...
						(comment.leading && hasNewline(options.originalText, locEnd(comment))) ||
						(comment.trailing &&
			...
			return [
				...
				node.cases.length > 0
					? indent([
							...
							join(
								hardline,
								path.map(({node, isLast}) => [print(), !isLast && isNextLineEmpty(node, options) ? hardline : ''], 'cases'),
							),
						])
					: '',
				hardline,
