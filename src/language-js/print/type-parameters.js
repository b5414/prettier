...
const {isTestCall, hasComment, CommentCheckFlags, isTSXFile, shouldPrintComma, getFunctionParameters} = require('../utils');
...
function printTypeParameters(path, options, print, paramsKey) {
	...
	const shouldInline =
		isParameterInTestCall || node[paramsKey].length === 0 || (node[paramsKey].length === 1 && (shouldHugType(node[paramsKey][0]) || node[paramsKey][0].type === 'NullableTypeAnnotation'));
	...
	const trailingComma =
		node.type === 'TSTypeParameterInstantiation' // https://github.com/microsoft/TypeScript/issues/21984
			? ''
			: getFunctionParameters(node).length === 1 && isTSXFile(options) && !node[paramsKey][0].constraint && path.getParentNode().type === 'ArrowFunctionExpression'
			? ','
...
function printDanglingCommentsForInline(path, options) {
	...
	const printed = printDanglingComments(path, options, /* sameIndent */ hasOnlyBlockComments);
