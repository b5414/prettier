import {group, hardline, ifBreak, indent, indentIfBreak, join, line, lineSuffixBoundary, softline} from '../../document/builders.js';
import {printDanglingComments} from '../../main/comments/print.js';
import createGroupIdMapper from '../../utils/create-group-id-mapper.js';
import {CommentCheckFlags, getFunctionParameters, hasComment, isObjectType, isTestCall, shouldPrintComma} from '../utils/index.js';
import {isArrowFunctionVariableDeclarator} from './assignment.js';
import {printTypeScriptMappedTypeModifier} from './mapped-type.js';
import {printTypeAnnotationProperty, shouldHugType} from './type-annotation.js';
...
function printTypeParameters(path, options, print, paramsKey) {
	...
	const isArrowFunctionVariable = path.match(
		(node) => !(node[paramsKey].length === 1 && isObjectType(node[paramsKey][0])),
		undefined,
	...
	const shouldInline =
		...
		(!isArrowFunctionVariable &&
			(isParameterInTestCall || (node[paramsKey].length === 1 && (node[paramsKey][0].type === 'NullableTypeAnnotation' || shouldHugType(node[paramsKey][0])))));
	...
	return group(['<', indent([softline, join([',', line], path.map(print, paramsKey))]), trailingComma, softline, '>'], {id: getTypeParametersGroupId(node)});
}
...
function printTypeParameter(path, options, print) {
	...
	if (node.constraint) {
		...
		parts.push(' extends', group(indent(line), {id: groupId}), lineSuffixBoundary, indentIfBreak(print('constraint'), {groupId}));
	}
