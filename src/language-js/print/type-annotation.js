...
const {isSimpleType, isObjectType, hasLeadingOwnLineComment, isObjectTypePropertyAFunction, shouldPrintComma} = require('../utils');
const {printAssignment} = require('./assignment');
const {printFunctionParameters, shouldGroupFunctionParameters} = require('./function-parameters');
...
function shouldHugType(node) {
	...
	if(node.type === 'UnionTypeAnnotation' || node.type === 'TSUnionType'){
		const voidCount = node.types.filter((node)=>node.type === 'VoidTypeAnnotation' || node.type === 'TSVoidKeyword' || node.type === 'NullLiteralTypeAnnotation' || node.type === 'TSNullKeyword')
			.length;
...
function printUnionType(path, options, print) {
	...
	const shouldIndent =
		...
		!(parent.type === 'FunctionTypeParam' && !parent.name && path.getParentNode(1).this !== parent) &&
		!((parent.type === 'TypeAlias' || parent.type === 'VariableDeclarator' || parent.type === 'TSTypeAliasDeclaration') && hasLeadingOwnLineComment(options.originalText, node));
...
function printFunctionType(path, options, print) {
	...
	let isArrowFunctionTypeAnnotation =
		...
		!(
			((parent.type === 'ObjectTypeProperty' || parent.type === 'ObjectTypeInternalSlot') && !parent.variance && !parent.optional && locStart(parent) === locStart(node)) ||
	...
	const parametersDoc = printFunctionParameters(path, print, options, /* expandArg */ false, /* printTypeParams */ true);

	// The returnType is not wrapped in a TypeAnnotation, so the colon
	// needs to be added separately.
	const returnTypeDoc =
		node.returnType || node.predicate || node.typeAnnotation ? [isArrowFunctionTypeAnnotation ? ' => ' : ': ', print('returnType'), print('predicate'), print('typeAnnotation')] : '';
