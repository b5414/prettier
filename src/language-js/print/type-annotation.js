import {align, group, ifBreak, indent, join, line, softline} from '../../document/builders.js';
...
import {printAbstractToken, printDeclareToken, printOptionalToken} from './misc.js';

/**
 * @typedef {import("../../document/builders.js").Doc} Doc
 */

const isVoidType = createTypeCheckFunction(['VoidTypeAnnotation', 'TSVoidKeyword', 'NullLiteralTypeAnnotation', 'TSNullKeyword']);
...
function printOpaqueType(path, options, print) {
	...
	const {node} = path;
	const parts = [printDeclareToken(path), 'opaque type ', print('id'), print('typeParameters')];
...
function printUnionType(path, options, print) {
	...
	const shouldIndent =
		...
		(parent.type !== 'ConditionalTypeAnnotation' || !options.experimentalTernaries) &&
		...
		!(parent.type === 'FunctionTypeParam' && !parent.name && path.grandparent.this !== parent) &&
		!(
			(parent.type === 'TypeAlias' || parent.type === 'VariableDeclarator' || parent.type === 'TSTypeAliasDeclaration') &&
...
function isFlowArrowFunctionTypeAnnotation(path) {
	...
	return (
		...
		(isFlowObjectTypePropertyAFunction(parent) ||
			!(
				((parent.type === 'ObjectTypeProperty' || parent.type === 'ObjectTypeInternalSlot') && !parent.variance && !parent.optional && hasSameLocStart(parent, node)) ||
...
function printIndexedAccessType(path, options, print) {
	return [print('objectType'), printOptionalToken(path), '[', print('indexType'), ']'];
}
...
function printJSDocType(path, print, token) {
	const {node} = path;
	return [node.postfix ? '' : token, printTypeAnnotationProperty(path, print), node.postfix ? token : ''];
}
...
function printRestType(path, options, print) {
	...
	return ['...', ...(node.type === 'TupleTypeSpreadElement' && node.label ? [print('label'), ': '] : []), print('typeAnnotation')];
}
...
function printTypeAnnotationProperty(path, print, propertyName = 'typeAnnotation') {
	const {
...
function printTypeAnnotation(path, options, print) {
	...
	if (process.env.NODE_ENV !== 'production') {
		...
		if (!typeAnnotationNodesCheckedLeadingComments.has(node)) {
			throw Object.assign(new Error(`'${node.type}' should be printed by '${printTypeAnnotationProperty.name}' function.`), {
