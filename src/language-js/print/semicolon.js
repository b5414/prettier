...
import {getLeftSidePathName, hasNakedLeftSide, isJsxElement} from '../utils/index.js';
import {shouldPrintParamsWithoutParens} from './function.js';

function shouldPrintLeadingSemicolon(path, options) {
	if (options.semi || isSingleJsxExpressionStatementInMarkdown(path, options) || isSingleVueEventBindingExpressionStatement(path, options)) {
		return false;
	}
	...
	if (
		...
		((key === 'body' && (parent.type === 'Program' || parent.type === 'BlockStatement' || parent.type === 'StaticBlock' || parent.type === 'TSModuleBlock')) ||
...
function expressionNeedsASIProtection(path, options) {
	...
	return path.call(() => expressionNeedsASIProtection(path, options), ...getLeftSidePathName(node));
}
...
function isSingleVueEventBindingExpressionStatement({node, parent}, options) {
	return (
		(options.parser === '__vue_event_binding' || options.parser === '__vue_ts_event_binding') &&
		node.type === 'ExpressionStatement' &&
		parent.type === 'Program' &&
		parent.body.length === 1
	);
...
export {isSingleJsxExpressionStatementInMarkdown, isSingleVueEventBindingExpressionStatement, isVueEventBindingExpression, shouldPrintLeadingSemicolon};
