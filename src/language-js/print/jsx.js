...
const {
	builders: {line, hardline, softline, group, indent, conditionalGroup, fill, ifBreak, lineSuffixBoundary, join},
	utils: {willBreak},
} = require('../../document');

const {getLast, getPreferredQuote} = require('../../common/util');
const {isJsxNode, rawText, isLiteral, isCallExpression, isStringLiteral, isBinaryish, hasComment, CommentCheckFlags, hasNodeIgnoreComment} = require('../utils');
...
function printJsxElementInternal(path, options, print) {
	...
	for(let i = children.length - 2; i >= 0; i--){
		...
		const isPairOfHardOrSoftLines =
			(children[i] === softline && children[i + 1] === '' && children[i + 2] === hardline) || (children[i] === hardline && children[i + 1] === '' && children[i + 2] === softline);
	...
	const content = containsText ? fill(multilineChildren) : group(multilineChildren, {shouldBreak: true});
	...
	const shouldInline =
		...
		(!hasComment(node.expression) &&
			(node.expression.type === 'ArrayExpression' ||
				...
				(isJsxNode(parent) && (node.expression.type === 'ConditionalExpression' || isBinaryish(node.expression)))));
...
function printJsxOpeningClosingFragment(path, options /*, print*/) {
	...
	return [
		isOpeningFragment ? '<' : '</',
		indent([hasOwnLineComment ? hardline : nodeHasComment && !isOpeningFragment ? ' ' : '', printDanglingComments(path, options, true)]),
...
function printJsxEmptyExpression(path, options /*, print*/) {
	...
	return [printDanglingComments(path, options, /* sameIndent */ !requiresHardline), requiresHardline ? hardline : ''];
}
