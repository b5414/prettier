import {conditionalGroup, cursor, fill, group, hardline, ifBreak, indent, join, line, lineSuffixBoundary, softline} from '../../document/builders.js';
import {replaceEndOfLine, willBreak} from '../../document/utils.js';
import {printComments, printDanglingComments} from '../../main/comments/print.js';
...
const isEmptyStringOrAnyLine = (doc) => doc === '' || doc === line || doc === hardline || doc === softline;
...
function printJsxElementInternal(path, options, print) {
  ...
  */
	if (options.cursorNode?.type === 'JSXText' && node.children.includes(options.cursorNode)) {
	...
	path.each(({node, next}) => {
		if (node.type === 'JSXText') {
			...
			if (isMeaningfulJsxText(node)) {
				const words = jsxWhitespaceUtils.split(text, /* captureWhitespace */ true);
				...
				if (endWhitespace !== undefined) {
					if (/\n/u.test(endWhitespace)) {
						parts.push(separatorWithWhitespace(isFacebookTranslationTag, parts.at(-1), node, next));
					} else {
						parts.push(jsxWhitespace);
					}
				} else {
					parts.push(separatorNoWhitespace(isFacebookTranslationTag, parts.at(-1), node, next));
				}
			} else if (/\n/u.test(text)) {
...
function maybeWrapJsxElementInParens(path, elem, options) {
	...
	return group([needsParens ? '' : ifBreak('('), indent([softline, elem]), softline, needsParens ? '' : ifBreak(')')], {shouldBreak});
}
...
function printJsxAttribute(path, options, print) {
	...
	if (node.value) {
		...
		if (isStringLiteral(node.value)) {
			...
			let final = raw.slice(1, -1).replaceAll('&apos;', "'").replaceAll('&quot;', '"');
			const quote = getPreferredQuote(final, options.jsxSingleQuote);
			final = quote === '"' ? final.replaceAll('"', '&quot;') : final.replaceAll("'", '&apos;');
			res = path.call(() => printComments(path, replaceEndOfLine(quote + final + quote), options), 'value');
		} else {
			res = print('value');
		}
		parts.push('=', res);
	...
	const shouldInline = (node, parent) =>
		...
		(!hasComment(node) &&
			(isArrayOrTupleExpression(node) ||
				...
				(node.type === 'AwaitExpression' && (shouldInline(node.argument, node) || node.argument.type === 'JSXElement')) ||
				isCallExpression(node) ||
				(node.type === 'ChainExpression' && isCallExpression(node.expression)) ||
				node.type === 'FunctionExpression' ||
				node.type === 'TemplateLiteral' ||
				node.type === 'TaggedTemplateExpression' ||
				node.type === 'DoExpression' ||
				(isJsxElement(parent) && (node.type === 'ConditionalExpression' || isBinaryish(node)))));
...
function printJsxOpeningElement(path, options, print) {
	...
	const shouldBreak = node.attributes?.some((attr) => isStringLiteral(attr.value) && attr.value.value.includes('\n'));
...
function shouldPrintBracketSameLine(node, options, nameHasComments) {
	const lastAttrHasTrailingComments = node.attributes.length > 0 && hasComment(node.attributes.at(-1), CommentCheckFlags.Trailing);
	return (
...
function printJsxEmptyExpression(path, options /*, print*/) {
	...
	return [printDanglingComments(path, options, {indent: requiresHardline}), requiresHardline ? hardline : ''];
}
	...
	return [
		...
		path.call(
			({node}) => {
				...
				return [indent([softline, printComments(path, printed, options)]), softline];
			},
