import {breakParent, conditionalGroup, group, hardline, indent, join, label} from '../../document/builders.js';
...
import {printBindExpressionCallee, printFunctionTypeParameters, printOptionalToken} from './misc.js';
...
function printMemberChain(path, options, print) {
	...
	if (path.node.type === 'ChainExpression') {
		return path.call(() => printMemberChain(path, options, print), 'expression');
	}
	...
	function shouldInsertEmptyLineAfter(node) {
		...
		const nextCharIndex = getNextNonSpaceNonCommentCharacterIndex(originalText, locEnd(node));
	...
	function rec(path) {
		...
		if (isCallExpression(node) && (isMemberish(node.callee) || isCallExpression(node.callee))) {
			const hasTrailingEmptyLine = shouldInsertEmptyLineAfter(node);
			printedNodes.unshift({
				node,
				hasTrailingEmptyLine,
				printed: [
					printComments(path, [printOptionalToken(path), printFunctionTypeParameters(path, options, print), printCallArguments(path, options, print)], options),
			...
			printedNodes.unshift({
				...
				printed: printComments(path, isMemberExpression(node) ? printMemberLookup(path, options, print) : printBindExpressionCallee(path, options, print), options),
			});
	...
	for (; i < printedNodes.length; ++i) {
		if (
			...
			(isMemberExpression(printedNodes[i].node) && printedNodes[i].node.computed && isNumericLiteral(printedNodes[i].node.property))
		) {
	...
	function shouldNotWrap(groups) {
		...
		if (groups[0].length === 1) {
			...
			return (
				...
				(firstNode.type === 'Identifier' && (isFactory(firstNode.name) || (isExpressionStatement && isShort(firstNode.name)) || hasComputed))
			);
		}
	...
	const nodeHasComment =
		flatGroups.slice(1, -1).some((node) => hasComment(node.node, CommentCheckFlags.Leading)) ||
		flatGroups.slice(0, -1).some((node) => hasComment(node.node, CommentCheckFlags.Trailing)) ||
		(groups[cutoff] && hasComment(groups[cutoff][0].node, CommentCheckFlags.Leading));

	// If we only have a single `.`, we shouldn't do anything fancy and just
	// render everything concatenated together.
	if (groups.length <= cutoff && !nodeHasComment && !groups.some((g) => g.at(-1).hasTrailingEmptyLine)) {
	...
	const callExpressions = printedNodes.map(({node}) => node).filter(isCallExpression);
	...
	function lastGroupWillBreakAndOtherCallsHaveFunctionArguments() {
		...
		return isCallExpression(lastGroupNode) && willBreak(lastGroupDoc) && callExpressions.slice(0, -1).some((node) => node.arguments.some(isFunctionOrArrowExpression));
	}
	...
	if (
		nodeHasComment ||
		(callExpressions.length > 2 && callExpressions.some((expr) => !expr.arguments.every((arg) => isSimpleCallArgument(arg)))) ||
