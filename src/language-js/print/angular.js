	...
	switch(node.type){
		...
		case 'NGChainedExpression':
			return group(
				join(
					[';', line],
					path.map((childPath)=>(hasNgSideEffect(childPath) ? print() : ['(', print(), ')']), 'expressions'),
				),
			);
		...
		case 'NGMicrosyntaxKeyedExpression': {
			...
			const shouldNotPrintColon =
				...
				(((index === 1 && (node.key.name === 'then' || node.key.name === 'else')) ||
					(index === 2 && node.key.name === 'else' && parentNode.body[index - 1].type === 'NGMicrosyntaxKeyedExpression' && parentNode.body[index - 1].key.name === 'then')) &&
