...
function shouldAddParensIfNotBreak(node) {
	if (!shouldAddParensIfNotBreakCache.has(node)) {
		shouldAddParensIfNotBreakCache.set(node, node.type === 'ConditionalExpression' && !startsWithNoLookaheadToken(node, (node) => node.type === 'ObjectExpression'));
	}
...
function printArrowFunction(path, options, print, args = {}) {
	...
	(function rec() {
		const {node} = path;
		const signatureDoc = printArrowFunctionSignature(path, options, print, args);
...
function printArrowFunctionSignature(path, options, print, args) {
	...
	if (shouldPrintParamsWithoutParens(path, options)) {
		...
		parts.push(group([printFunctionParameters(path, print, options, expandArg, /* printTypeParams */ true), returnTypeDoc]));
	}

	const dangling = printDanglingComments(path, options, {
		filter(comment) {
			const nextCharacter = getNextNonSpaceNonCommentCharacterIndex(options.originalText, locEnd(comment));
...
function mayBreakAfterShortPrefix(functionBody, bodyDoc, options) {
	return (
		...
		(bodyDoc.label?.hug !== false && (bodyDoc.label?.embed || isTemplateOnItsOwnLine(functionBody, options.originalText)))
	);
}
 ...
 */
function printArrowFunctionBody(path, options, args, {bodyDoc, bodyComments, functionBody, shouldPutBodyOnSameLine}) {
