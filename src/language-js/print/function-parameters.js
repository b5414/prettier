...
function printFunctionParameters(path, print, options, expandArg, printTypeParams) {
	...
	if (parameters.length === 0) {
		return [
			...
			printDanglingComments(path, options, {
				filter: (comment) => getNextNonSpaceNonCommentCharacter(options.originalText, locEnd(comment)) === ')',
			}),
			')',
		];
	}
	...
	//   c
	// }) {}
	const hasNotParameterDecorator = parameters.every((node) => !isNonEmptyArray(node.decorators));
	...
	const isFlowShorthandWithOneArg =
		(isFlowObjectTypePropertyAFunction(parent) ||
			...
			(parent.type === 'FunctionTypeAnnotation' && parent.returnType === functionNode)) &&
		parameters.length === 1 &&
	...
	if (isFlowShorthandWithOneArg) {
		if (options.arrowParens === 'always' || functionNode.type === 'HookTypeAnnotation') {
			return ['(', ...printed, ')'];
		}
		return printed;
	}
	...
	return (
		...
		(parameter.type === 'ObjectPattern' ||
			...
			(parameter.type === 'Identifier' &&
				...
				(parameter.typeAnnotation.type === 'TypeAnnotation' || parameter.typeAnnotation.type === 'TSTypeAnnotation') &&
				isObjectType(parameter.typeAnnotation.typeAnnotation)) ||
			(parameter.type === 'FunctionTypeParam' && isObjectType(parameter.typeAnnotation) && parameter !== node.rest) ||
			(parameter.type === 'AssignmentPattern' &&
				(parameter.left.type === 'ObjectPattern' || parameter.left.type === 'ArrayPattern') &&
				(parameter.right.type === 'Identifier' ||
					(isObjectOrRecordExpression(parameter.right) && parameter.right.properties.length === 0) ||
					(isArrayOrTupleExpression(parameter.right) && parameter.right.elements.length === 0))))
	);
}
		...
		(node, name) => {
			if (node.type === 'CallExpression' && name === 'arguments' && node.arguments.length === 1 && node.callee.type === 'CallExpression') {
				...
				return (
					...
					(decorator.type === 'MemberExpression' && !decorator.computed && decorator.object.type === 'Identifier' && decorator.property.type === 'Identifier')
				);
			}
			return false;
		},
...
export {printFunctionParameters, shouldBreakFunctionParameters, shouldGroupFunctionParameters, shouldHugTheOnlyFunctionParameter};
