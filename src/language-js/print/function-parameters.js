...
function printFunctionParameters(path, print, options, expandArg, printTypeParams) {
	...
	if(parameters.length === 0){
		return [typeParams, '(', printDanglingComments(path, options, /* sameIndent */ true, (comment)=>getNextNonSpaceNonCommentCharacter(options.originalText, comment, locEnd) === ')'), ')'];
	}

	const parent = path.getParentNode();
	const isParametersInTestCall = isTestCall(parent);
	const shouldHugParameters = shouldHugFunctionParameters(functionNode);
	const shouldExpandParameters = expandArg && !parameters.some((node)=>hasComment(node));
	const printed = [];
	iterateFunctionParametersPath(path, (parameterPath, index)=>{
		...
		printed.push(',');
		if(isParametersInTestCall || shouldHugParameters || shouldExpandParameters){
			printed.push(' ');
...
function shouldHugFunctionParameters(node) {
	...
	return(
		...
		(parameter.type === 'ObjectPattern' ||
			...
			(parameter.type === 'FunctionTypeParam' && isObjectType(parameter.typeAnnotation)) ||
			(parameter.type === 'AssignmentPattern' &&
				(parameter.left.type === 'ObjectPattern' || parameter.left.type === 'ArrayPattern') &&
				(parameter.right.type === 'Identifier' ||
					(parameter.right.type === 'ObjectExpression' && parameter.right.properties.length === 0) ||
