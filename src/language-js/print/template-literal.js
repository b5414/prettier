import {addAlignmentToDoc, align, group, hardline, indent, join, label, lineSuffixBoundary, softline} from '../../document/builders.js';
...
import {hasComment, isBinaryCastExpression, isBinaryish, isMemberExpression} from '../utils/index.js';
...
function printTemplateLiteral(path, print, options) {
	...
	path.each(({index, node: quasi}) => {
		...
		let interpolationHasNewline = hasNewlineInRange(options.originalText, locEnd(quasi), locStart(node.quasis[index + 1]));
...
function printJestEachTemplateLiteral(path, options, print) {
	...
	if (headerNames.length > 1 || headerNames.some((headerName) => headerName.length > 0)) {
		...
		const table = [{cells: headerNames}, ...tableBody.filter((row) => row.cells.length > 0)];
...
function escapeTemplateCharacters(doc, raw) {
	return mapDoc(doc, (currentDoc) => {
		if (typeof currentDoc === 'string') {
			return raw ? currentDoc.replaceAll(/(\\*)`/gu, '$1$1\\`') : uncookTemplateElementValue(currentDoc);
		}
...
function isJestEachTemplateLiteral({node, parent}) {
	...
	return (
		...
		((parent.tag.object.type === 'Identifier' && jestEachTriggerRegex.test(parent.tag.object.name)) ||
			(parent.tag.object.type === 'MemberExpression' &&
				parent.tag.object.property.type === 'Identifier' &&
				(parent.tag.object.property.name === 'only' || parent.tag.object.property.name === 'skip') &&
				parent.tag.object.object.type === 'Identifier' &&
				jestEachTriggerRegex.test(parent.tag.object.object.name)))
	);
}

export {escapeTemplateCharacters, printTaggedTemplateLiteral, printTemplateExpressions, printTemplateLiteral, uncookTemplateElementValue};
