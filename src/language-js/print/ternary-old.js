import {align, breakParent, dedent, group, ifBreak, indent, line, softline} from '../../document/builders.js';
import hasNewlineInRange from '../../utils/has-newline-in-range.js';
import {locEnd, locStart} from '../loc.js';
import {hasComment, isBinaryCastExpression, isCallExpression, isJsxElement, isMemberExpression} from '../utils/index.js';
...
function printTernaryOld(path, options, print) {
	...
	if (
		...
		parts.push(
			' ? ',
			isNil(consequentNode) ? print(consequentNodePropertyName) : wrap(print(consequentNodePropertyName)),
			' : ',
			alternateNode.type === node.type || isNil(alternateNode) ? print(alternateNodePropertyName) : wrap(print(alternateNodePropertyName)),
		);
	} else {
		/*
    ...
    ```
    */
		const printBranch = (nodePropertyName) => (options.useTabs ? indent(print(nodePropertyName)) : align(2, print(nodePropertyName)));
		// normal mode
		const part = [
			line,
			'? ',
		...
		parts.push(
			parent.type !== node.type || parent[alternateNodePropertyName] === node || isParentTest
				? part
				: options.useTabs
					? dedent(indent(part))
	...
	const shouldBreak = [consequentNodePropertyName, alternateNodePropertyName, ...testNodePropertyNames].some((property) =>
		hasComment(node[property], (comment) => isBlockComment(comment) && hasNewlineInRange(options.originalText, locStart(comment), locEnd(comment))),
	);
