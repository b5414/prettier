import {fill, group, hardline, ifBreak, indent, line, softline} from '../../document/builders.js';
...
import {CommentCheckFlags, hasComment, isArrayOrTupleExpression, isNumericLiteral, isObjectOrRecordExpression, isSignedNumericLiteral, shouldPrintComma} from '../utils/index.js';
...
function printEmptyArrayElements(path, options, openBracket, closeBracket) {
	...
	return group([openBracket, inexact, printDanglingComments(path, options, {indent: true}), softline, closeBracket]);
}
...
function printArray(path, options, print) {
	...
	if (elements.length === 0) {
		parts.push(printEmptyArrayElements(path, options, openBracket, closeBracket));
	} else {
				...
				[
					...
					indent([
						...
						shouldUseConciseFormatting
							...
							: [printArrayElements(path, options, elementsProperty, node.inexact, print), trailingComma],
	...
	parts.push(printOptionalToken(path), printTypeAnnotationProperty(path, print));
...
function isConciselyPrintedArray(node, options) {
	return (
		...
		node.elements.every(
			(element) =>
				element &&
				(isNumericLiteral(element) || (isSignedNumericLiteral(element) && !hasComment(element.argument))) &&
				!hasComment(
					element,
...
function isLineAfterElementEmpty({node}, {originalText: text}) {
	const skipComment = (idx) => skipInlineComment(text, skipTrailingComment(text, idx));
...
function printArrayElements(path, options, elementsProperty, inexact, print) {
	...
	path.each(({node, isLast}) => {
		...
		if (!isLast || inexact) {
			parts.push([',', line, node && isLineAfterElementEmpty(path, options) ? softline : '']);
		}
	}, elementsProperty);
...
function printArrayElementsConcisely(path, options, print, trailingComma) {
	...
	path.each(({isLast, next}) => {
		...
		if (!isLast) {
			parts.push(isLineAfterElementEmpty(path, options) ? [hardline, hardline] : hasComment(next, CommentCheckFlags.Leading | CommentCheckFlags.Line) ? hardline : line);
		}
	}, 'elements');
