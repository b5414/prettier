...
const {shouldPrintComma, hasComment, CommentCheckFlags, isNextLineEmpty, isNumericLiteral, isSignedNumericLiteral} = require('../utils');
...
function printArray(path, options, print) {
	...
	if(node.elements.length === 0){
		...
		const trailingComma = !canHaveTrailingComma ? '' : needsForcedTrailingComma ? ',' : !shouldPrintComma(options) ? '' : shouldUseConciseFormatting ? ifBreak(',', '', {groupId}) : ifBreak(',');
		...
		parts.push(
			group(
				[
					...
					indent([
						softline,
						shouldUseConciseFormatting ? printArrayItemsConcisely(path, options, print, trailingComma) : [printArrayItems(path, options, 'elements', print), trailingComma],
