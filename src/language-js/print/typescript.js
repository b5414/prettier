...
const {
	builders: {join, line, hardline, softline, group, indent, conditionalGroup, ifBreak},
} = require('../../document');
const {isLiteral, getTypeScriptMappedTypeModifier, shouldPrintComma, isCallExpression, isMemberExpression} = require('../utils');
...
const {printTypeParameter, printTypeParameters} = require('./type-parameters');
...
const {printTypeAlias, printIntersectionType, printUnionType, printFunctionType, printTupleType, printIndexedAccessType} = require('./type-annotation');
...
function printTypescript(path, options, print) {
	...
	switch(node.type){
		...
		case 'TSConstructorType': {
			...
			parts.push(group(printFunctionParameters(path, print, options, /* expandArg */ false, /* printTypeParams */ true)));
			...
			return group(
				[
					...
					indent([
						...
						node.readonly ? [getTypeScriptMappedTypeModifier(node.readonly, 'readonly'), ' '] : '',
		...
		case 'TSEnumDeclaration':
			...
			if(node.members.length === 0){
				...
				parts.push(
					group([
						'{',
						indent([hardline, printArrayItems(path, options, 'members', print), shouldPrintComma(options, 'es5') ? ',' : '']),
						printDanglingComments(path, options, /* sameIndent */ true),
						hardline,
						'}',
					]),
