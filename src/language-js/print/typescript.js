import {conditionalGroup, group, ifBreak, indent, join, softline} from '../../document/builders.js';
import UnexpectedNodeError from '../../utils/unexpected-node-error.js';
import {locStart} from '../loc.js';
import getTextWithoutComments from '../utils/get-text-without-comments.js';
import {isArrayOrTupleExpression, isObjectOrRecordExpression, isStringLiteral, shouldPrintComma} from '../utils/index.js';
...
import {printDeclareToken, printOptionalToken, printTypeScriptAccessibilityToken} from './misc.js';
	...
	switch (node.type) {
		...
		case 'TSImportType':
			return [
				...
				printTypeParameters(path, options, print, node.typeArguments ? 'typeArguments' : 'typeParameters'),
			];
		case 'TSLiteralType':
		...
		case 'TSMethodSignature': {
			...
			parts.push(printTypeScriptAccessibilityToken(node), kind, node.computed ? '[' : '', print('key'), node.computed ? ']' : '', printOptionalToken(path));
			...
			const returnTypeDoc = returnTypeNode ? printTypeAnnotationProperty(path, print, returnTypePropertyName) : '';
