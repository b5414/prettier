import {group, indent, indentIfBreak, line, lineSuffixBoundary} from '../../document/builders.js';
...
function printAssignment(path, options, print, leftDoc, operator, rightPropertyName) {
	...
	const rightDoc = rightPropertyName ? print(rightPropertyName, {assignmentLayout: layout}) : '';
	...
	switch (layout) {
		...
		case 'fluid': {
			...
			return group([group(leftDoc), operator, group(indent(line), {id: groupId}), lineSuffixBoundary, indentIfBreak(rightDoc, {groupId})]);
		}
...
function chooseLayout(path, options, print, leftDoc, rightPropertyName) {
	...
	const shouldUseChainFormatting = path.match(
		...
		(node) => !isTail || (node.type !== 'ExpressionStatement' && node.type !== 'VariableDeclaration'),
	);
	...
	if (
		...
		(rightNode.type === 'CallExpression' && rightNode.callee.name === 'require') ||
...
function shouldBreakAfterOperator(path, options, print, hasShortKey) {
	...
	switch (rightNode.type) {
		...
		case 'ConditionalTypeAnnotation':
			if (!options.experimentalTernaries && !shouldBreakBeforeConditionalType(rightNode)) {
				break;
			}
			return true;
...
function isComplexDestructuring(node) {
	if (isAssignmentOrVariableDeclarator(node)) {
		...
		return (
			...
			leftNode.properties.some((property) => isObjectProperty(property) && (!property.shorthand || property.value?.type === 'AssignmentPattern'))
		);
	}
	return false;
}
...
const isTypeAlias = createTypeCheckFunction(['TSTypeAliasDeclaration', 'TypeAlias']);
...
function hasComplexTypeAnnotation(node) {
	...
	return (
		...
		typeParams.length > 1 &&
		typeParams.some((param) => isNonEmptyArray(getTypeParametersFromTypeReference(param)) || param.type === 'TSConditionalType')
	);
}
...
const isTypeReference = createTypeCheckFunction(['TSTypeReference', 'GenericTypeAnnotation']);
...
export {isArrowFunctionVariableDeclarator, printAssignment, printAssignmentExpression, printVariableDeclarator};
