...
const {hasLeadingOwnLineComment, isBinaryish, isStringLiteral, isNumericLiteral, isCallExpression, isMemberExpression, getCallArguments, isSimpleCallArgument} = require('../utils');
...
function printAssignment(path, options, print, leftDoc, operator, rightPropertyName) {
	...
	switch(layout){
		...
		case 'fluid': {
			...
			return group([group(leftDoc), operator, group(indent(line), {id: groupId}), indentIfBreak(rightDoc, {groupId})]);
		}
...
function shouldBreakAfterOperator(rightNode, hasShortKey) {
	...
	if(
		...
		(isSimpleCall(node) && isSimpleCall(node.callee) && (isMemberExpressionChainHead(node.callee.callee) || isMemberExpressionChainWithSimpleCalls(node.callee.callee)))
	){
		return true;
	}
...
function isMemberExpressionChainWithSimpleCalls(node) {
	...
	return isMemberExpressionChainHead(object) || isMemberExpressionChainWithSimpleCalls(object);
}
...
function isSimpleCall(node) {
	...
	return args.length === 0 || (args.length === 1 && isSimpleCallArgument(args[0], 1));
}
