...
function printWithoutParentheses(path, options, print, args) {
	...
	for (const printer of [printAngular, printJsx, printFlow, printTypescript, printEstree]) {
...
function print(path, options, print, args) {
	...
	return inheritLabel(doc, (doc) => [
		needsSemi ? ';' : '',
		needsParens ? '(' : '',
		needsParens && isClassExpression && hasDecorators ? [indent([line, decoratorsDoc, doc]), line] : [decoratorsDoc, doc],
		needsParens ? ')' : '',
	]);
}
