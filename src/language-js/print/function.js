...
const {printDanglingComments, printCommentsSeparately} = require('../../main/comments');
...
function printMethod(path, options, print) {
	...
	if(node === value){
		...
		parts.push(path.call((path)=>printMethodInternal(path, options, print), 'value'));
	}else{
...
function printArrowFunctionSignature(path, options, print, args) {
	...
	if(shouldPrintParamsWithoutParens(path, options)){
		...
		parts.push(
			group([
				printFunctionParameters(path, print, options, /* expandLast */ args && (args.expandLastArg || args.expandFirstArg), /* printTypeParams */ true),
				printReturnType(path, print, options),
			]),
		);
	}

	const dangling = printDanglingComments(path, options, /* sameIndent */ true, (comment)=>{
...
function printArrowFunctionExpression(path, options, print, args) {
	...
	// a => a ? a : a
	// a <= a ? a : a
	const shouldAddParens = node.body.type === 'ConditionalExpression' && !startsWithNoLookaheadToken(node.body, /* forbidFunctionAndClass */ false);
