...
const {getLast, hasNewlineInRange, hasNewline, isNonEmptyArray} = require('../../common/util');
const {shouldPrintComma, hasComment, getComments, CommentCheckFlags, isNextLineEmpty} = require('../utils');
...
function printObject(path, options, print) {
	...
	const isFlowInterfaceLikeBody =
		isTypeAnnotation && parent && (parent.type === 'InterfaceDeclaration' || parent.type === 'DeclareInterface' || parent.type === 'DeclareClass') && path.getName() === 'body';
	const shouldBreak =
		...
		(node.type !== 'ObjectPattern' && firstProperty && hasNewlineInRange(options.originalText, locStart(node), locStart(firstProperty)));
	...
	if(node.inexact){
		...
		if(hasComment(node, CommentCheckFlags.Dangling)){
			...
			const printedDanglingComments = printDanglingComments(path, options, /* sameIndent */ true);
	...
	if(
		path.match(
			...
			(node, name, number)=>shouldHugFunctionParameters(node) && (name === 'params' || name === 'parameters' || name === 'this' || name === 'rest') && number === 0,
		) ||
		path.match(
			shouldHugType,
