import {group, hardline, ifBreak, indent, line, softline} from '../../document/builders.js';
import {printDanglingComments} from '../../main/comments/print.js';
import hasNewline from '../../utils/has-newline.js';
import hasNewlineInRange from '../../utils/has-newline-in-range.js';
import isNonEmptyArray from '../../utils/is-non-empty-array.js';
import {locEnd, locStart} from '../loc.js';
import {CommentCheckFlags, getComments, hasComment, isNextLineEmpty, isObjectType, shouldPrintComma} from '../utils/index.js';
...
function printObject(path, options, print) {
	...
	const isFlowInterfaceLikeBody =
		isTypeAnnotation && key === 'body' && (parent.type === 'InterfaceDeclaration' || parent.type === 'DeclareInterface' || parent.type === 'DeclareClass');
	const shouldBreak =
		...
		(node.type === 'ObjectPattern' &&
			...
			node.properties.some((property) => property.value && (property.value.type === 'ObjectPattern' || property.value.type === 'ArrayPattern'))) ||
		(node.type !== 'ObjectPattern' && propsAndLoc.length > 0 && hasNewlineInRange(options.originalText, locStart(node), propsAndLoc[0].loc));
	...
	if (node.inexact || node.hasUnknownMembers) {
		...
		if (hasComment(node, CommentCheckFlags.Dangling)) {
			...
			printed = [printedDanglingComments, hasLineComments || hasNewline(options.originalText, locEnd(getComments(node).at(-1))) ? hardline : line, '...'];
		} else {
			printed = ['...'];
		}
	...
	if (props.length === 0) {
		...
		content = [
			isFlowInterfaceLikeBody && isNonEmptyArray(node.properties) ? printHardlineAfterHeritage(parent) : '',
			leftBrace,
			indent([options.bracketSpacing ? line : softline, ...props]),
			ifBreak(canHaveTrailingSeparator && (separator !== ',' || shouldPrintComma(options)) ? separator : ''),
	...
	// type
	if (
		path.match((node) => node.type === 'ObjectPattern' && !isNonEmptyArray(node.decorators), shouldHugTheOnlyParameter) ||
		(isObjectType(node) &&
			(path.match(
				undefined,
				...
				path.match(undefined, (node, name) => node.type === 'FunctionTypeParam' && name === 'typeAnnotation', shouldHugTheOnlyParameter))) ||
