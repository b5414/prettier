import {breakParent, dedent, group, hardline, ifBreak, indent, line, softline} from '../../document/builders.js';
...
function hasMultilineBlockComments(testNodes, consequentNode, alternateNode, options) {
	const comments = [...testNodes.map((node) => getComments(node)), getComments(consequentNode), getComments(alternateNode)].flat();
	return comments.some((comment) => isBlockComment(comment) && hasNewlineInRange(options.originalText, locStart(comment), locEnd(comment)));
}
...
function shouldExtraIndentForConditionalExpression(path) {
	...
	for (let ancestorCount = 0; !parent; ancestorCount++) {
		...
		// Reached chain root
		if ((node.type === 'NewExpression' && node.callee === child) || (isBinaryCastExpression(node) && node.expression === child)) {
...
const wrapInParens = (doc) => [ifBreak('('), indent([softline, doc]), softline, ifBreak(')')];
...
function printTernary(path, options, print, args) {
	...
	const isTSConditional = node.type === 'TSConditionalType' || node.type === 'ConditionalTypeAnnotation'; // For Flow.
	...
	const shouldBreak = hasMultilineBlockComments(testNodes, consequentNode, alternateNode, options) || isConsequentTernary || isAlternateTernary;
	...
	const tryToParenthesizeAlternate =
		...
		(isInJsx
			...
			: // Right now, we do this when:
				...
				isLoneShortArgument(consequentNode, options) && isSimpleExpressionByNodeCount(node.test, 3));

	const shouldGroupTestAndConsequent =
		isInChain ||
		isInAlternate ||
		(isTSConditional && !isParentTernary) ||
		(isParentTernary && isConditionalExpression && isSimpleExpressionByNodeCount(node.test, 1)) ||
	...
	if (!isConsequentTernary && hasComment(consequentNode, CommentCheckFlags.Dangling)) {
		path.call((childPath) => {
			consequentComments.push(printDanglingComments(childPath, options), hardline);
		}, 'consequent');
	}
