...
const {isJsxNode, isBlockComment, getComments, isCallExpression, isMemberExpression} = require('../utils');
const {locStart, locEnd} = require('../loc');
const {
	builders: {line, softline, group, indent, align, ifBreak, dedent, breakParent},
} = require('../../document');
...
function shouldExtraIndentForConditionalExpression(path) {
	...
	for(let ancestorCount = 0; !parent; ancestorCount++){
		...
		if((isCallExpression(node) && node.callee === child) || (isMemberExpression(node) && node.object === child) || (node.type === 'TSNonNullExpression' && node.expression === child)){
			child = node;
			continue;
		}
...
function printTernary(path, options, print) {
	...
	if(isConditionalExpression && (isJsxNode(node[testNodePropertyNames[0]]) || isJsxNode(consequentNode) || isJsxNode(alternateNode) || conditionalExpressionChainContainsJsx(lastConditionalParent))){
		...
		const part = [
			...
			line,
			': ',
			alternateNode.type === node.type ? print(alternateNodePropertyName) : align(2, print(alternateNodePropertyName)),
		];
		parts.push(
			parent.type !== node.type || parent[alternateNodePropertyName] === node || isParentTest ? part : options.useTabs ? dedent(indent(part)) : align(Math.max(0, options.tabWidth - 2), part),
		);
	}
	...
	const comments = flat([...testNodePropertyNames.map((propertyName)=>getComments(node[propertyName])), getComments(consequentNode), getComments(alternateNode)]);
