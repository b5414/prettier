...
import {CommentCheckFlags, createTypeCheckFunction, hasComment, isNextLineEmpty} from '../utils/index.js';
import {printAssignment} from './assignment.js';
import {printClassMemberDecorators} from './decorators.js';
import {printMethod} from './function.js';
import {printAbstractToken, printDeclareToken, printDefiniteToken, printOptionalToken, printTypeScriptAccessibilityToken} from './misc.js';
...
function printClass(path, options, print) {
	...
	extendsParts.push(printHeritageClauses(path, options, print, 'mixins'), printHeritageClauses(path, options, print, 'implements'));
...
function hasMultipleHeritage(node) {
	return ['extends', 'mixins', 'implements'].reduce((count, key) => count + (Array.isArray(node[key]) ? node[key].length : 0), node.superClass ? 1 : 0) > 1;
}
...
function printClassProperty(path, options, print) {
	...
	parts.push(printPropertyKey(path, options, print), printOptionalToken(path), printDefiniteToken(path), printTypeAnnotationProperty(path, print));
...
function printClassBody(path, options, print) {
	...
	path.each(({node, next, isLast}) => {
		...
		if (!options.semi && isClassProperty(node) && shouldPrintSemicolonAfterClassProperty(node, next)) {
			parts.push(';');
		}
	...
	return [isNonEmptyArray(node.body) ? printHardlineAfterHeritage(path.parent) : '', '{', parts.length > 0 ? [indent([hardline, parts]), hardline] : '', '}'];
}
...
export {printClass, printClassBody, printClassMethod, printClassProperty, printHardlineAfterHeritage};
