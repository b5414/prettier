...
const {
	builders: {join, line, softline, group, indent, align, ifBreak, indentIfBreak},
	utils: {cleanDoc, getDocParts, isConcat},
} = require('../../document');
const {hasLeadingOwnLineComment, isBinaryish, isJsxNode, shouldFlatten, hasComment, CommentCheckFlags, isCallExpression, isMemberExpression} = require('../utils');
	...
	const shouldNotIndent =
		...
		(node.type !== 'NGPipeExpression' &&
			((parent.type === 'NGRoot' && options.parser === '__ng_binding') ||
				(parent.type === 'NGMicrosyntaxExpression' && parentParent.type === 'NGMicrosyntax' && parentParent.body.length === 1))) ||
		(node === parent.body && parent.type === 'ArrowFunctionExpression') ||
		(node !== parent.body && parent.type === 'ForStatement') ||
		(parent.type === 'ConditionalExpression' && parentParent.type !== 'ReturnStatement' && parentParent.type !== 'ThrowStatement' && !isCallExpression(parentParent)) ||
...
function printBinaryishExpressions(path, print, options, isNested, isInsideParenthesis) {
	...
	if(isBinaryish(node)){
		...
		if(shouldFlatten(node.operator, node.left.operator)){
			...
			parts = [...parts, ...path.call((left)=>printBinaryishExpressions(left, print, options, /* isNested */ true, isInsideParenthesis), 'left')];
		}else{
		...
		const lineBeforeOperator =
			(node.operator === '|>' || node.type === 'NGPipeExpression' || (node.operator === '|' && options.parser === '__vue_expression')) &&
		...
		const right = shouldInline ? [operator, ' ', print('right'), rightSuffix] : [lineBeforeOperator ? line : '', operator, lineBeforeOperator ? ' ' : line, print('right'), rightSuffix];
