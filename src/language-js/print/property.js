...
const {isNumericLiteral, isSimpleNumber, isStringLiteral, isStringPropSafeToUnquote, rawText} = require('../utils');
...
function printPropertyKey(path, options, print) {
	...
	if(options.quoteProps === 'consistent' && !needsQuoteProps.has(parent)){
		const objectHasStringProp = (parent.properties || parent.body || parent.members).some(
			(prop)=>!prop.computed && prop.key && isStringLiteral(prop.key) && !isStringPropSafeToUnquote(prop, options),
		);
	...
	if(
		...
		(options.parser === 'json' || (options.quoteProps === 'consistent' && needsQuoteProps.get(parent)))
	){
		// a -> "a"
		// 1 -> "1"
		// 1.5 -> "1.5"
