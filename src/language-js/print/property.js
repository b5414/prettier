...
function isStringKeySafeToUnquote(node, options) {
	if (options.parser === 'json' || options.parser === 'jsonc' || !isStringLiteral(node.key) || printString(rawText(node.key), options).slice(1, -1) !== node.key.value) {
		return false;
	}
	...
	if (
		...
		!((options.parser === 'babel-ts' && node.type === 'ClassProperty') || (options.parser === 'typescript' && node.type === 'PropertyDefinition'))
	) {
		return true;
	}
	...
	if (
		...
		(options.parser === 'babel' || options.parser === 'acorn' || options.parser === 'espree' || options.parser === 'meriyah' || options.parser === '__babel_estree')
	) {
		return true;
	}
...
function shouldQuotePropertyKey(path, options) {
	...
	return (
		...
		(options.parser === 'json' || options.parser === 'jsonc' || (options.quoteProps === 'consistent' && needsQuoteProps.get(path.parent)))
	);
}
...
function printPropertyKey(path, options, print) {
	...
	if (options.quoteProps === 'consistent' && !needsQuoteProps.has(parent)) {
		const objectHasStringProp = path.siblings.some((prop) => !prop.computed && isStringLiteral(prop.key) && !isStringKeySafeToUnquote(prop, options));
	...
	if (isStringKeySafeToUnquote(node, options) && (options.quoteProps === 'as-needed' || (options.quoteProps === 'consistent' && !needsQuoteProps.get(parent)))) {
		// 'a' -> a
		// '1' -> 1
		// '1.5' -> 1.5
		return path.call((keyPath) => printComments(keyPath, /^\d/u.test(key.value) ? printNumber(key.value) : key.value, options), 'key');
	}
