...
const {getLeftSidePathName, hasNakedLeftSide, isJsxNode, isTheOnlyJsxElementInMarkdown, hasComment, CommentCheckFlags, isNextLineEmpty} = require('../utils');
...
function printStatementSequence(path, options, print, property) {
	...
	path.each((path, index, statements)=>{
		...
		if(!options.semi && !isClassBody && !isTheOnlyJsxElementInMarkdown(options, path) && statementNeedsASIProtection(path, options)){
...
const isClassProperty = ({type})=>type === 'ClassProperty' || type === 'PropertyDefinition' || type === 'ClassPrivateProperty';
/**
 * @returns {boolean}
 */
