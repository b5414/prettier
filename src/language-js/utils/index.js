...
const isArrayOrTupleExpression = createTypeCheckFunction(['ArrayExpression', 'TupleExpression']);

const isObjectOrRecordExpression = createTypeCheckFunction(['ObjectExpression', 'RecordExpression']);
...
const isSingleWordType = createTypeCheckFunction(['Identifier', 'ThisExpression', 'Super', 'PrivateName', 'PrivateIdentifier', 'Import']);

const isObjectType = createTypeCheckFunction(['ObjectTypeAnnotation', 'TSTypeLiteral', 'TSMappedType']);

const isFunctionOrArrowExpression = createTypeCheckFunction(['FunctionExpression', 'ArrowFunctionExpression']);
...
const isBinaryish = createTypeCheckFunction(['BinaryExpression', 'LogicalExpression', 'NGPipeExpression']);
...
function isTestCall(node, parent) {
	...
	if (args.length === 1) {
		...
		return (
			(args.length === 2 ? isFunctionOrArrowExpression(args[1]) : isFunctionOrArrowExpressionWithBody(args[1]) && getFunctionParameters(args[1]).length <= 1) ||
...
const isCallExpression = skipChainExpression(createTypeCheckFunction(['CallExpression', 'OptionalCallExpression']));

const isMemberExpression = skipChainExpression(createTypeCheckFunction(['MemberExpression', 'OptionalMemberExpression']));
...
function isTemplateOnItsOwnLine(node, text) {
	return (
		((node.type === 'TemplateLiteral' && templateLiteralHasNewLines(node)) || (node.type === 'TaggedTemplateExpression' && templateLiteralHasNewLines(node.quasi))) &&
 ...
 * @returns {boolean}
 */
const isNextLineEmpty = (node, {originalText}) => isNextLineEmptyAfterIndex(originalText, locEnd(node));
...
const isUnionType = createTypeCheckFunction(['UnionTypeAnnotation', 'TSUnionType']);

const isIntersectionType = createTypeCheckFunction(['IntersectionTypeAnnotation', 'TSIntersectionType']);
