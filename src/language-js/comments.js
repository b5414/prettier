...
function handleConditionalExpressionComments({comment, precedingNode, enclosingNode, followingNode, text}) {
...
function handleCommentInEmptyParens({comment, enclosingNode, text}) {
	...
	if(
		enclosingNode &&
		((isRealFunctionLikeNode(enclosingNode) && getFunctionParameters(enclosingNode).length === 0) || (isCallLikeExpression(enclosingNode) && getCallArguments(enclosingNode).length === 0))
	){
		addDanglingComment(enclosingNode, comment);
		return true;
	}
...
function handleOnlyComments({comment, enclosingNode, followingNode, ast, isLastComment}) {
	...
	if(enclosingNode && enclosingNode.type === 'Program' && enclosingNode.body.length === 0 && enclosingNode.directives && enclosingNode.directives.length === 0){
		if(isLastComment){
			addDanglingComment(enclosingNode, comment);
		}else{
			addLeadingComment(enclosingNode, comment);
...
function handleVariableDeclaratorComments({comment, enclosingNode, followingNode}) {
	if(
		enclosingNode &&
		(enclosingNode.type === 'VariableDeclarator' || enclosingNode.type === 'AssignmentExpression') &&
		followingNode &&
...
function getCommentChildNodes(node, options) {
	...
	//     }
	if(
		(options.parser === 'typescript' || options.parser === 'flow' || options.parser === 'espree' || options.parser === 'meriyah') &&
	...
	return(
		((node &&
			(isJsxNode(node) ||
				...
				(isCallExpression(parent) && (hasFlowAnnotationComment(node.leadingComments) || hasFlowAnnotationComment(node.trailingComments))))) ||
			(parent &&
				(parent.type === 'JSXSpreadAttribute' ||
					parent.type === 'JSXSpreadChild' ||
					parent.type === 'UnionTypeAnnotation' ||
					parent.type === 'TSUnionType' ||
					((parent.type === 'ClassDeclaration' || parent.type === 'ClassExpression') && parent.superClass === node)))) &&
		(!hasIgnoreComment(path) || parent.type === 'UnionTypeAnnotation' || parent.type === 'TSUnionType')
	);
}
