...
const {hasNewline, skipNewline, skipSpaces, isPreviousLineEmpty, addLeadingComment, addDanglingComment, addTrailingComment, isNonEmptyArray} = require('../common/util');
...
function getSortedChildNodes(node, options, resultArray) {
	...
	const childNodes =
		(printer.getCommentChildNodes && printer.getCommentChildNodes(node, options)) ||
		(typeof node === 'object' &&
			Object.entries(node).filter(([key])=>key !== 'enclosingNode' && key !== 'precedingNode' && key !== 'followingNode' && key !== 'tokens' && key !== 'comments').map(([, value])=>value));
...
function attach(comments, ast, text, options) {
	...
	const {avoidAstMutation, ownLine: handleOwnLineComment = returnFalse, endOfLine: handleEndOfLineComment = returnFalse, remaining: handleRemainingComment = returnFalse} = handleComments;
	...
	for(const [index, context] of decoratedComments.entries()){
		const {comment, precedingNode, enclosingNode, followingNode, text, options, ast, isLastComment} = context;
...
function isOwnLineComment(text, options, decoratedComments, commentIndex) {
	...
	if(precedingNode){
		...
		for(let index = commentIndex - 1; index >= 0; index--){
			const {comment, precedingNode: currentCommentPrecedingNode} = decoratedComments[index];
...
function isEndOfLineComment(text, options, decoratedComments, commentIndex) {
	...
	if(followingNode){
		...
		for(let index = commentIndex + 1; index < decoratedComments.length; index++){
			const {comment, followingNode: currentCommentFollowingNode} = decoratedComments[index];
