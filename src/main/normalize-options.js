 ...
 */
function normalizeOptions(options, optionInfos, {logger = false, isCLI = false, passThrough = false, FlagSchema, descriptor} = {}) {
	...
	const unknown = !passThrough
		...
		: Array.isArray(passThrough)
			? (key, value) => (!passThrough.includes(key) ? undefined : {[key]: value})
			: (key, value) => ({[key]: value});
...
function optionInfoToSchema(optionInfo, {isCLI, optionInfos, FlagSchema}) {
	...
	switch (optionInfo.type) {
		...
		case 'flag':
			...
			parameters.flags = optionInfos.flatMap((optionInfo) =>
				[optionInfo.alias, optionInfo.description && optionInfo.name, optionInfo.oppositeDescription && `no-${optionInfo.name}`].filter(Boolean),
			);
			break;
		case 'path':
			SchemaConstructor = vnopts.StringSchema;
	...
	if (isCLI && !optionInfo.array) {
		...
		parameters.preprocess = (value, schema, utils) => schema.preprocess(originalPreprocess(Array.isArray(value) ? value.at(-1) : value), utils);
	}
