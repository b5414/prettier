...
async function printEmbeddedLanguages(/** @type {import("../common/ast-path.js").default} */ path, genericPrint, options, printAstToDoc, embeds) {
	const {
	...
	const getVisitorKeys = createGetVisitorKeysFunction(embed.getVisitorKeys ?? printerGetVisitorKeys);
	...
	function recurse() {
		...
		if (process.env.NODE_ENV !== 'production' && typeof result.then === 'function') {
...
async function textToDoc(text, partialNextOptions, parentOptions, printAstToDoc) {
