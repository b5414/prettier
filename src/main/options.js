...
function normalize(options, opts = {}) {
	...
	const defaults = {
		...hiddenDefaults,
		...fromPairs(supportOptions.filter((optionInfo)=>optionInfo.default !== undefined).map((option)=>[option.name, option.default])),
	};
	...
	const parser = resolveParser(normalizer.normalizeApiOptions(rawOptions, [supportOptions.find((x)=>x.name === 'parser')], {passThrough: true, logger: false}));
	...
	const pluginDefaults = fromPairs(
		supportOptions.filter((optionInfo)=>optionInfo.pluginDefaults && optionInfo.pluginDefaults[plugin.name] !== undefined).map((optionInfo)=>[
			optionInfo.name,
...
function inferParser(filepath, plugins) {
	...
	const languages = getSupportInfo({plugins}).languages.filter((language)=>language.since !== null);
	...
	let language = languages.find(
		(language)=>
			(language.extensions && language.extensions.some((extension)=>filename.endsWith(extension))) || (language.filenames && language.filenames.some((name)=>name.toLowerCase() === filename)),
	);
