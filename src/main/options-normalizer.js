...
function normalizeOptions(options, optionInfos, {logger, isCLI = false, passThrough = false} = {}) {
	...
	switch(optionInfo.type){
		...
		case 'flag':
			...
			parameters.flags = flat(
				optionInfos.map((optionInfo)=>[optionInfo.alias, optionInfo.description && optionInfo.name, optionInfo.oppositeDescription && `no-${optionInfo.name}`].filter(Boolean)),
			);
			break;
		case 'path':
			SchemaConstructor = vnopts.StringSchema;
