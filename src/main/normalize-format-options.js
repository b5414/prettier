...
import {getParserPluginByParserName, getPrinterPluginByAstFormat, initParser, initPrinter} from './parser-and-printer.js';
...
async function normalizeFormatOptions(options, opts = {}) {
	...
	const defaults = {
		...
		...Object.fromEntries(supportOptions.filter((optionInfo) => optionInfo.default !== undefined).map((option) => [option.name, option.default])),
	};
