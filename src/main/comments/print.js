import {breakParent, hardline, indent, join, line, lineSuffix} from '../../document/builders.js';
...
function printTrailingComment(path, options, previousComment) {
	...
	if ((previousComment?.hasLineSuffix && !previousComment?.isBlock) || hasNewline(originalText, locStart(comment), {backwards: true})) {
		...
		const isLineBeforeEmpty = isPreviousLineEmpty(originalText, locStart(comment));
 ...
 * @returns {Doc}
 */
function printDanglingComments(path, options, danglingCommentsPrintOptions = {}) {
	const {node} = path;
	...
	const {indent: shouldIndent = false, marker, filter = returnTrue} = danglingCommentsPrintOptions;
...
function printCommentsSeparately(path, options) {
	...
	path.each(() => {
		...
		if (leading) {
			...
			printedTrailingComment = printTrailingComment(path, options, printedTrailingComment);
...
function ensureAllCommentsPrinted(options) {
	const {[Symbol.for('comments')]: comments, [Symbol.for('printedComments')]: printedComments} = options;
...
export {ensureAllCommentsPrinted, printComments, printCommentsSeparately, printDanglingComments};
