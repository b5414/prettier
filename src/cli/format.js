...
import {createIsIgnoredFunction, createTwoFilesPatch, errors, mockable} from './prettier-internal.js';
...
async function format(context, input, opt) {
	...
	if (context.argv.debugCheck) {
		...
		if (pp !== pppp) {
			...
			const ast = stringify((await prettier.__debug.parse(input, opt, {massage: true})).ast);
			const past = stringify((await prettier.__debug.parse(pp, opt, {massage: true})).ast);
...
async function formatStdin(context) {
	...
	try {
		...
		const options = await getOptionsForFile(context, filepath ? path.resolve(filepath) : undefined);
...
async function formatFiles(context) {
	...
	for await (const {error, filename, ignoreUnknown} of expandPatterns(context)) {
		...
		try {
			...
			context.logger.error(`Unable to read file "${fileNameToDisplay}":\n${error.message}`);
		...
		try {
			...
			handleError(context, fileNameToDisplay, error, printedFilename, ignoreUnknown);
			continue;
		}
		...
		if (context.argv.write) {
			...
			if (isDifferent) {
				...
				try {
					...
					context.logger.error(`Unable to write file "${fileNameToDisplay}":\n${error.message}`);

					// Don't exit the process if one file failed
					process.exitCode = 2;
				}
			} else if (!context.argv.check && !context.argv.listDifferent) {
				const message = `${chalk.grey(fileNameToDisplay)} ${Date.now() - start}ms (unchanged)`;
				if (isCacheExists) {
	...
	if (context.argv.check) {
		if (numberOfUnformattedFilesFound === 0) {
			...
			context.logger.warn(context.argv.write ? `Code style issues fixed in ${files}.` : `Code style issues found in ${files}. Run Prettier with --write to fix.`);
		}
	}
