...
function format(context, input, opt) {
	...
	if(context.argv['debug-print-comments']){
		return {
			formatted: prettier.format(JSON.stringify(prettier.formatWithCursor(input, opt).comments || []), {parser: 'json'}),
		};
	}
	...
	if(context.argv['debug-check']){
		...
		if(pp !== pppp){
			...
			const ast = stringify(prettier.__debug.parse(input, opt, /* massage */ true).ast);
			const past = stringify(prettier.__debug.parse(pp, opt, /* massage */ true).ast);
