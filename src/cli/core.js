...
const {normalizeDetailedOptionMap, createDetailedOptionMap} = require('./option-map');
...
function logFileInfoOrDie(context) {
	...
	context.logger.log(prettier.format(stringify(prettier.getFileInfo.sync(context.argv['file-info'], options)), {parser: 'json'}));
}
