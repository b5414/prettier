...
function getOptions(argv, detailedOptions) {
	return fromPairs(detailedOptions.filter(({forwardToApi})=>forwardToApi).map(({forwardToApi, name})=>[forwardToApi, argv[name]]));
}
...
function createApiDetailedOptionMap(detailedOptions) {
	return fromPairs(detailedOptions.filter((option)=>option.forwardToApi && option.forwardToApi !== option.name).map((option)=>[option.forwardToApi, option]));
}
