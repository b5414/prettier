...
function genericPrint(path, options, print) {
	...
	switch (node.kind) {
		case 'Document':
			return [...join(hardline, printSequence(path, options, print, 'definitions')), hardline];
		...
		case 'Field':
			return group([
				...
				node.arguments.length > 0
					? group(['(', indent([softline, join([ifBreak('', ', '), softline], printSequence(path, options, print, 'arguments'))]), softline, ')'])
					: '',
				printDirectives(path, print, node),
				node.selectionSet ? ' ' : '',
				print('selectionSet'),
		...
		case 'StringValue':
			if (node.block) {
				const lines = node.value.replaceAll('"""', String.raw`\"""`).split('\n');
				if (lines.length === 1) {
			...
			return ['"', node.value.replaceAll(/["\\]/gu, String.raw`\$&`).replaceAll('\n', String.raw`\n`), '"'];
		...
		case 'ObjectValue': {
			const bracketSpace = options.bracketSpacing && node.fields.length > 0 ? ' ' : '';
		...
		case 'InterfaceTypeDefinition': {
			...
			if (node.fields.length > 0) {
				parts.push([' {', indent([hardline, join(hardline, printSequence(path, options, print, 'fields'))]), hardline, '}']);
			}
		...
		case 'EnumTypeDefinition':
			return [
				...
				print('name'),
				printDirectives(path, print, node),
				node.values.length > 0 ? [' {', indent([hardline, join(hardline, printSequence(path, options, print, 'values'))]), hardline, '}'] : '',
			];

		case 'EnumValueDefinition':
			return [printDescription(path, options, print), print('name'), printDirectives(path, print, node)];
		...
		case 'SchemaExtension':
			return [
				'extend schema',
				printDirectives(path, print, node),
				...(node.operationTypes.length > 0 ? [' {', indent([hardline, join(hardline, printSequence(path, options, print, 'operationTypes'))]), hardline, '}'] : []),
			];
		...
		case 'SchemaDefinition':
			return [
				...
				printDirectives(path, print, node),
				' {',
				node.operationTypes.length > 0 ? indent([hardline, join(hardline, printSequence(path, options, print, 'operationTypes'))]) : '',
				hardline,
				'}',
			];
		...
		case 'UnionTypeDefinition':
			return group([
				...
				group([
					...
					print('name'),
					printDirectives(path, print, node),
					node.types.length > 0 ? [' =', ifBreak('', ' '), indent([ifBreak([line, '| ']), join([line, '| '], path.map(print, 'types'))])] : '',
				]),
			]);

		case 'ScalarTypeExtension':
		case 'ScalarTypeDefinition':
			return [printDescription(path, options, print), node.kind === 'ScalarTypeExtension' ? 'extend ' : '', 'scalar ', print('name'), printDirectives(path, print, node)];
