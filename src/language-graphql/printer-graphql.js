	...
	switch(node.kind){
		...
		case 'OperationDefinition': {
			...
			return [
				...
				isNonEmptyArray(node.variableDefinitions) ? group(['(', indent([softline, join([ifBreak('', ', '), softline], path.map(print, 'variableDefinitions'))]), softline, ')']) : '',
		...
		case 'SelectionSet': {
			return [
				...
				indent([
					...
					join(
						hardline,
						path.call((selectionsPath)=>printSequence(selectionsPath, options, print), 'selections'),
					),
				]),
				hardline,
				'}',
				...
				node.arguments.length > 0
					? group([
							...
							indent([
								...
								join(
									[ifBreak('', ', '), softline],
									path.call((argsPath)=>printSequence(argsPath, options, print), 'arguments'),
								),
							]),
							softline,
							')',
		...
		case 'ObjectValue': {
			return group([
				...
				indent([softline, join([ifBreak('', ', '), softline], path.map(print, 'fields'))]),
				softline,
		...
		case 'ObjectTypeDefinition': {
			return [
				...
				'type ',
				print('name'),
				node.interfaces.length > 0 ? [' implements ', ...printInterfaces(path, options, print)] : '',
				printDirectives(path, print, node),
