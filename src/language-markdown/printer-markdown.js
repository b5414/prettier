...
import {align, fill, group, hardline, indent, line, literalline, markAsRoot, softline} from '../document/builders.js';
...
import {getFencedCodeBlockValue, hasGitDiffFriendlyOrderedList, INLINE_NODE_TYPES, INLINE_NODE_WRAPPER_TYPES, isAutolink, splitText} from './utils.js';
...
function genericPrint(path, options, print) {
	...
	if (shouldRemainTheSameContent(path)) {
		/** @type {Doc} */
		const parts = [''];
		const textsNodes = splitText(options.originalText.slice(node.position.start.offset, node.position.end.offset));
	...
	switch (node.type) {
		...
		case 'word': {
			let escapedValue = node.value
				...
				.replaceAll(
					...
					(_, text1, underscore1, underscore2, text2) => (underscore1 ? `${text1}${underscore1}` : `${underscore2}${text2}`).replaceAll('_', String.raw`\_`),
			...
			if (
				...
				(path.match(undefined, isFirstSentence, isLastChildAutolink) ||
					path.match(undefined, isFirstSentence, (node, name, index) => node.type === 'emphasis' && index === 0, isLastChildAutolink))
			) {
				// backslash is parsed as part of autolinks, so we need to remove it
				escapedValue = escapedValue.replace(/^(\\?[*_])+/u, (prefix) => prefix.replaceAll('\\', ''));
			}
		...
		case 'whitespace': {
			...
			const proseWrap =
				...
				next && /^>|^(?:[*+-]|#{1,6}|\d+[).])$/u.test(next.value) ? 'never' : options.proseWrap;
		...
		case 'emphasis': {
			...
			if (isAutolink(node.children[0])) {
				...
				const hasPrevOrNextWord = // `1*2*3` is considered emphasis but `1_2_3` is not
					(previous?.type === 'sentence' && previous.children.at(-1)?.type === 'word' && !previous.children.at(-1).hasTrailingPunctuation) ||
					(next?.type === 'sentence' && next.children[0]?.type === 'word' && !next.children[0].hasLeadingPunctuation);
				style = hasPrevOrNextWord || path.hasAncestor((node) => node.type === 'emphasis') ? '*' : '_';
			}
		...
		case 'delete':
			return ['~~', printChildren(path, options, print), '~~'];
		case 'inlineCode': {
			const code = options.proseWrap === 'preserve' ? node.value : node.value.replaceAll('\n', ' ');
			...
			switch (options.originalText[node.position.start.offset]) {
				case '<': {
					...
					const url =
						...
						node.url.startsWith(mailto) && options.originalText.slice(node.position.start.offset + 1, node.position.start.offset + 1 + mailto.length) !== mailto
		...
		case 'code': {
			...
			return [
				...
				node.meta ? ' ' + node.meta : '',
				hardline,
				replaceEndOfLine(getFencedCodeBlockValue(node, options.originalText), hardline),
				hardline,
				style,
			];
		}
				...
				processor(childPath) {
					...
					function getPrefix() {
						const rawPrefix = node.ordered
							? (childPath.isFirst ? node.start : isGitDiffFriendlyOrderedList ? 1 : node.start + childPath.index) + (nthSiblingIndex % 2 === 0 ? '. ' : ') ')
							: nthSiblingIndex % 2 === 0
								? '- '
								: '* ';

						return node.isAligned || /* workaround for https://github.com/remarkjs/remark/issues/315 */ node.hasIndentedCodeblock
		...
		case 'definition': {
			...
			return group([
				...
				':',
				indent([lineOrSpace, printUrl(node.url), node.title === null ? '' : [lineOrSpace, printTitle(node.title, options, false)]]),
			]);
		}
		...
		case 'footnoteDefinition': {
			const shouldInlineFootnote =
				...
				(options.proseWrap === 'never' || (options.proseWrap === 'preserve' && node.children[0].position.start.line === node.children[0].position.end.line));
			return [
				...
				shouldInlineFootnote
					...
					: group([
							align(
								' '.repeat(4),
								printChildren(path, options, print, {
									processor: ({isFirst}) => (isFirst ? group([softline, print()]) : print()),
								}),
							),
						]),
			];
		...
		case 'tableCell':
			return printChildren(path, options, print);
		case 'break':
			return /\s/u.test(options.originalText[node.position.start.offset]) ? ['  ', markAsRoot(literalline)] : ['\\', hardline];
		case 'liquidNode':
			...
			if (ignoreRanges.length > 0) {
				...
				if (index === ignoreRange.start.index) {
					return [
						...
						options.originalText.slice(ignoreRange.start.offset, ignoreRange.end.offset),
	...
	path.each(() => {
		...
		if (result !== false) {
			if (parts.length > 0 && shouldPrePrintHardline(path)) {
				...
				if (shouldPrePrintDoubleHardline(path, options) || shouldPrePrintTripleHardline(path)) {
					parts.push(hardline);
				}
...
function isPrettierIgnore(node) {
	...
	if (node.type === 'html') {
		match = node.value.match(/^<!--\s*prettier-ignore(?:-(start|end))?\s*-->$/u);
	} else {
		let comment;
...
function shouldPrePrintDoubleHardline({node, previous, parent}, options) {
	...
	const isInTightListItem = parent.type === 'listItem' && !isLooseListItem(parent, options);
	...
	const isHtmlDirectAfterListItem =
		node.type === 'html' && parent.type === 'listItem' && previous.type === 'paragraph' && previous.position.end.line + 1 === node.position.start.line;
...
function shouldRemainTheSameContent(path) {
	const node = path.findAncestor((node) => node.type === 'linkReference' || node.type === 'imageReference');
...
function printUrl(url, dangerousCharOrChars = []) {
	...
	return new RegExp(dangerousChars.map((x) => escapeStringRegexp(x)).join('|'), 'u').test(url) ? `<${encodeUrl(url, '<>')}>` : url;
}
