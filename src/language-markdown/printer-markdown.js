...
const {getLast, getMinNotPresentContinuousCount, getMaxContinuousCount, getStringWidth, isNonEmptyArray} = require('../common/util');
const {
	builders: {breakParent, join, line, literalline, markAsRoot, hardline, softline, ifBreak, fill, align, indent, group, hardlineWithoutBreakParent},
	utils: {normalizeDoc},
...
const {getFencedCodeBlockValue, hasGitDiffFriendlyOrderedList, splitText, punctuationPattern, INLINE_NODE_TYPES, INLINE_NODE_WRAPPER_TYPES, isAutolink} = require('./utils');
...
function genericPrint(path, options, print) {
	...
	if(shouldRemainTheSameContent(path)){
		return splitText(options.originalText.slice(node.position.start.offset, node.position.end.offset), options).map((node)=>
	...
	switch(node.type){
		...
		case 'word': {
			let escapedValue = node.value
				...
				.replace(new RegExp([`(^|${punctuationPattern})(_+)`, `(_+)(${punctuationPattern}|$)`].join('|'), 'g'), (_, text1, underscore1, underscore2, text2)=>
					(underscore1 ? `${text1}${underscore1}` : `${underscore2}${text2}`).replace(/_/g, '\\_'),
		...
		case 'emphasis': {
			...
			if(isAutolink(node.children[0])){
				...
				const hasPrevOrNextWord = // `1*2*3` is considered emphasis but `1_2_3` is not
					(prevNode && prevNode.type === 'sentence' && prevNode.children.length > 0 && getLast(prevNode.children).type === 'word' && !getLast(prevNode.children).hasTrailingPunctuation) ||
					(nextNode && nextNode.type === 'sentence' && nextNode.children.length > 0 && nextNode.children[0].type === 'word' && !nextNode.children[0].hasLeadingPunctuation);
		...
		case 'code': {
			if(node.isIndented){
				...
				return align(alignment, [alignment, ...replaceEndOfLineWith(node.value, hardline)]);
			}

			// fenced code block
			const styleUnit = options.__inJsTemplate ? '~' : '`';
			const style = styleUnit.repeat(Math.max(3, getMaxContinuousCount(node.value, styleUnit) + 1));
			return [style, node.lang || '', node.meta ? ' ' + node.meta : '', hardline, ...replaceEndOfLineWith(getFencedCodeBlockValue(node, options.originalText), hardline), hardline, style];
		}
		case 'html': {
			const parentNode = path.getParentNode();
			...
			return replaceEndOfLineWith(value, isHtmlComment ? hardline : markAsRoot(literalline));
		}
		case 'list': {
				...
				processor: (childPath, index)=>{
					...
					function getPrefix() {
						const rawPrefix = node.ordered
							? (index === 0 ? node.start : isGitDiffFriendlyOrderedList ? 1 : node.start + index) + (nthSiblingIndex % 2 === 0 ? '. ' : ') ')
							: nthSiblingIndex % 2 === 0
							? '- '
							: '* ';

						return node.isAligned || /* workaround for https://github.com/remarkjs/remark/issues/315 */ node.hasIndentedCodeblock ? alignListPrefix(rawPrefix, options) : rawPrefix;
					}
				},
			});
		}
		...
		case 'jsx':
			return node.value;
		case 'math':
			return ['$$', hardline, node.value ? [...replaceEndOfLineWith(node.value, hardline), hardline] : '', '$$'];
		case 'inlineMath': {
...
function shouldPrePrintDoubleHardline(node, data) {
	...
	const isHtmlDirectAfterListItem =
		node.type === 'html' && data.parentNode.type === 'listItem' && data.prevNode && data.prevNode.type === 'paragraph' && data.prevNode.position.end.line + 1 === node.position.start.line;
