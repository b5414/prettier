import {breakParent, group, hardlineWithoutBreakParent, ifBreak, join} from '../../document/builders.js';
...
function printTable(path, options, print) {
	...
	function printTableContents(isCompact) {
		return join(
			...
			[printRow(contents[0], isCompact), printAlign(isCompact), ...contents.slice(1).map((rowContents) => printRow(rowContents, isCompact))].map(
