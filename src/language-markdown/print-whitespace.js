...
import {KIND_CJ_LETTER, KIND_CJK_PUNCTUATION, KIND_K_LETTER, KIND_NON_CJK} from './utils.js';
...
const SINGLE_LINE_NODE_TYPES = new Set(['heading', 'tableCell', 'link', 'wikiLink']);
 ...
 *         `"檜原村（ひ\nのはらむら）"`
 */
const noBreakAfter = new Set('$(£¥·\'"〈《「『【〔〖〝﹙﹛＄（［｛￡￥[{‵︴︵︷︹︻︽︿﹁﹃﹏〘｟«');
 ...
 */
const lineBreakBetweenTheseAndCJConvertsToSpace = new Set('!"#$%&\'()*+,-./:;<=>?@[\\]^_`{|}~');
...
function lineBreakCanBeConvertedToSpace(path, isLink) {
	...
	// `isBreakable(...)`.
	if (lineBreakBetweenTheseAndCJConvertsToSpace.has(next.value[0]) || lineBreakBetweenTheseAndCJConvertsToSpace.has(previous.value.at(-1))) {
		return true;
	}
...
function isBreakable(path, value, proseWrap, isLink, canBeSpace) {
	...
	if (value === '' && ((previous?.kind === KIND_K_LETTER && isLetter(next?.kind)) || (next?.kind === KIND_K_LETTER && isLetter(previous?.kind)))) {
		return false;
	}

	// https://en.wikipedia.org/wiki/Line_breaking_rules_in_East_Asian_languages
	const violatesCJKLineBreakingRules = !canBeSpace && ((next && noBreakBefore.has(next.value[0])) || (previous && noBreakAfter.has(previous.value.at(-1))));
