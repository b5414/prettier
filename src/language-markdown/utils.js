...
const {cjkPattern, kPattern, punctuationPattern} = require('./constants.evaluate');
...
const INLINE_NODE_WRAPPER_TYPES = [...INLINE_NODE_TYPES, 'tableCell', 'paragraph', 'heading'];
...
function splitText(text, options) {
	...
	function appendNode(node) {
		...
		if(lastNode && lastNode.type === 'word'){
			if(
				(lastNode.kind === KIND_NON_CJK && node.kind === KIND_CJ_LETTER && !lastNode.hasTrailingPunctuation) ||
				(lastNode.kind === KIND_CJ_LETTER && node.kind === KIND_NON_CJK && !node.hasLeadingPunctuation)
			){
