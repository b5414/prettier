...
const INLINE_NODE_WRAPPER_TYPES = new Set([...INLINE_NODE_TYPES, 'tableCell', 'paragraph', 'heading']);
...
function getOrderedListItemInfo(orderListItem, options) {
	const text = options.originalText.slice(orderListItem.position.start.offset, orderListItem.position.end.offset);

	const {numberText, leadingSpaces} = text.match(/^\s*(?<numberText>\d+)(\.|\))(?<leadingSpaces>\s*)/u).groups;
...
function hasGitDiffFriendlyOrderedList(node, options) {
	...
	return node.children.length > 2 && getOrderedListItemInfo(node.children[2], options).number === 1;
}
