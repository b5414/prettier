...
console.log(replaceVersions(printEntries(entries).join('\n\n'), previousVersion, newVersion, /** isPatch */ true));

function parseArgv() {
	const argv = minimist(process.argv.slice(2));
	const previousVersion = argv['prev-version'];
	const newVersion = argv['new-version'];
	if (!previousVersion || !newVersion || semver.compare(previousVersion, newVersion) !== -1) {
		throw new Error(`Invalid argv, prev-version: ${previousVersion}, new-version: ${newVersion}`);
	}
