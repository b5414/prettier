...
const parentPropertyCheckSelector = [
	...
	':matches(',
	[
		['[left.type="MemberExpression"]', '[left.object.type="Identifier"]', '[left.object.name="parent"]', '[right.type="Identifier"]', '[right.name="node"]'],
		['[right.type="MemberExpression"]', '[right.object.type="Identifier"]', '[right.object.name="parent"]', '[left.type="Identifier"]', '[left.name="node"]'],
...
module.exports = {
	meta: {
		...
		messages: {
			[MESSAGE_ID_PREFER_NAME_CHECK]: 'Prefer `name {{operator}} {{propertyText}}` over `parent.{{property}} {{operator}} node`.',
			[MESSAGE_ID_NAME_CHECK_FIRST]: '`name` comparison should be on left side.',
		},
		fixable: 'code',
	},
	create(context) {
		...
		return {
			[parentPropertyCheckSelector](node) {
				...
				const {property} = [left, right].find(({type})=>type === 'MemberExpression');
				const propertyText = property.type === 'Identifier' ? `"${property.name}"` : sourceCode.getText(property);
				...
				context.report({
					...
					fix: (fixer)=>fixer.replaceText(node, `name ${operator} ${propertyText}`),
				});
			},
