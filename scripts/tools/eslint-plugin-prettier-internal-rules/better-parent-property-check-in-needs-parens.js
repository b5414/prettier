...
module.exports = {
	meta: {
		...
		messages: {
			[MESSAGE_ID_PREFER_KEY_CHECK]: 'Prefer `key {{operator}} {{propertyText}}` over `parent.{{property}} {{operator}} node`.',
		...
		return {
			[parentPropertyCheckSelector](node) {
				...
				context.report({
					...
					fix: (fixer) => fixer.replaceText(node, `key ${operator} ${propertyText}`),
				});
			},
