			...
			[selector](node) {
				...
				context.report({
					...
					*fix(fixer) {
						...
						const openingParenthesisToken = sourceCode.getTokenAfter(node.callee);
						const commaToken = sourceCode.getTokenBefore(doc, ({type, value}) => type === 'Punctuator' && value === ',');
						yield fixer.replaceTextRange([openingParenthesisToken.range[1], commaToken.range[1]], '');
					},
				});
			},
		};
