			...
			[selector](outerCall) {
				...
				context.report({
					...
					*fix(fixer) {
						...
						let innerNamesText = sourceCode.text.slice(innerNamesStart, innerNamesEnd);

						yield fixer.replaceTextRange([innerNamesStart, innerNamesEnd + 1], '');

						const [penultimateToken, lastToken] = sourceCode.getLastTokens(outerCall, 2);

						// `outer` call has `trailing comma`
						if(penultimateToken && penultimateToken.type === 'Punctuator' && penultimateToken.value === ','){
