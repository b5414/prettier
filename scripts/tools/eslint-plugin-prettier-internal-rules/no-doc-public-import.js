...
const selector = [':matches(ImportDeclaration, ExportNamedDeclaration, ImportExpression)', ' > ', 'Literal.source'].join('');

const messageId = 'no-doc-public-import';
const docIndexFile = path.join(__dirname, '../../../src/document/public.js');
const ignored = new Set([path.join(__dirname, '../../../src/index.js'), path.join(__dirname, '../../../src/standalone.js')]);
	...
	if (
		!(
			...
			identifier.parent.id.properties.every(
				(property) => property.type === 'Property' && !property.computed && property.key.type === 'Identifier' && docProperties.has(property.key.name),
			) &&
	...
	return function* (fixer) {
		...
		const text = identifier.parent.id.properties
			.map((property) => {
				...
				return `import ${sourceCode.getText(property.value)} from "${source.value.replace('/document/public.js', `/document/${propertyName}.js`)}";`;
			})
			.join('\n');
	...
	create(context) {
		...
		return {
			[selector](node) {
				...
				if (!value.startsWith('.') || !value.endsWith('/document/public.js') || path.join(dir, value) !== docIndexFile) {
					return;
				}
