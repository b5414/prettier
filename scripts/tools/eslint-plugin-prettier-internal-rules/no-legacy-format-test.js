...
const legacyRunFormatTestCall = ['CallExpression', '[callee.type="Identifier"]', '[callee.name="run_spec"]'].join('');

const runFormatTestCall = ['CallExpression', '[callee.type="Identifier"]', '[callee.name="runFormatTest"]'].join('');

const dirnameArgumentSelector = [runFormatTestCall, ' > ', 'Identifier.arguments:first-child', '[name="__dirname"]'].join('');
...
module.exports = {
	meta: {
		...
		messages: {
			[MESSAGE_ID_LEGACY_FUNCTION_NAME]: 'Use `runFormatTest(…)` instead of `run_spec(…)`.',
			[MESSAGE_ID_ARGUMENT]: 'Use `import.meta` instead of `__dirname`.',
			[MESSAGE_ID_PROPERTY]: 'Use `importMeta: import.meta` instead of `dirname: __dirname`.',
		...
		return {
			[legacyRunFormatTestCall](callExpression) {
				context.report({
					...
					fix: (fixer) => fixer.replaceText(callExpression.callee, 'runFormatTest'),
				});
			},
			...
			[dirnamePropertySelector](node) {
				context.report({
					...
					fix: (fixer) => [fixer.replaceText(node.key, 'importMeta'), fixer.replaceText(node.value, 'import.meta')],
				});
			},
			Program(node) {
