...
const isEqualCheck = (node) => node.type === 'BinaryExpression' && node.operator === '===';

const isTypeIdentifier = (node) => node.type === 'Identifier' && node.name === 'type';

const isTypeIdentifierCheck = (node) => isEqualCheck(node) && isTypeIdentifier(node.left);
const isTypeAccessCheck = (node, parameterName) => isEqualCheck(node) && isTypeAccess(node.left, parameterName);
...
const isMultipleTypeAccessCheck = (node, parameterName) => isSetHasOrArrayIncludesCall(node) && isTypeAccess(node.arguments[0], parameterName);

const isMultipleTypeIdentifierCheck = (node) => isSetHasOrArrayIncludesCall(node) && isTypeIdentifier(node.arguments[0]);
...
function getTypes(node, parameter) {
	...
	// `function({type}) {}`
	if (parameter.type === 'ObjectPattern' && parameter.properties.length === 1 && parameter.properties[0].type === 'Property') {
		const [{shorthand, computed, key, value}] = parameter.properties;

		if (shorthand && !computed && key.type === 'Identifier' && key.name === 'type' && value.type === 'Identifier' && value.name === 'type') {
...
function isTopLevelFunction(node) {
	return (
		...
		(node.parent.type === 'VariableDeclarator' && node.parent.parent.type === 'VariableDeclaration' && node.parent.parent.parent.type === 'Program') ||
		node.parent.type === 'ExportDefaultDeclaration'
	);
}

const selector = [':function', '[params.length=1]', '[async!=true]', '[generator!=true]'].join('');
...
module.exports = {
	meta: {
		...
		messages: {
			[MESSAGE_ID]: 'Prefer use `createTypeCheckFunction` to create this function',
		},
		schema: [
			{
				type: 'object',
	...
	create(context) {
		...
		return {
			[selector](functionNode) {
				...
				if (ignoreSingleType && types.length === 1 && types[0].type === 'single') {
					return;
				}
				...
				const commentsInFunction = sourceCode.getCommentsInside(functionNode).length;
				const commentsInTypes = commentsInFunction === 0 ? 0 : types.reduce((count, {node}) => count + sourceCode.getCommentsInside(node).length, 0);
					...
					problem.fix = (fixer) => {
						const typesText =
							types.length === 1 && types[0].type === 'multiple'
								...
								: `[${types.map(({type, node}) => `${type === 'single' ? '' : '...'}${sourceCode.getText(node)}`).join(', ')}]`;

						let text = `createTypeCheckFunction(${typesText})`;

						if (functionNode.type === 'FunctionDeclaration') {
							const functionName = functionNode.id?.name ?? '__please_name_this_function';
							text = `const ${functionName} = ${text};`;

							if (functionNode.parent.type === 'ExportDefaultDeclaration' && functionNode.parent.declaration === functionNode) {
								return fixer.replaceText(functionNode.parent, `${text}\nexport default ${functionName};`);
							}
						}
