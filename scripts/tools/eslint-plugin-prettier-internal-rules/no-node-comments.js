...
// `node.comments`
const memberExpressionSelector = ['MemberExpression[computed=false]', '', '>', 'Identifier.property', '[name="comments"]'].join('');

// `const {comments} = node`
// `const {comments: nodeComments} = node`
const objectPatternSelector = ['ObjectPattern', '>', 'Property.properties', '>', 'Identifier.key', '[name="comments"]'].join('');
	...
	create(context) {
		...
		const ignored = new Map(
			context.options.map((option) => {
				...
				return [path.join(__dirname, '../../..', file), functions ? new Set(functions) : true];
			}),
		);
		...
		return {
			[selector](node) {
				...
				if (ignored.has(fileName)) {
					...
					while (currentNode) {
						if (currentNode.type === 'FunctionDeclaration' && currentNode.id && currentNode.id.type === 'Identifier' && functionNames.has(currentNode.id.name)) {
							isIgnored = true;
							break;
						}
