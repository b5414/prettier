...
const getLengthSelector = (path)=>`[${path}.type="MemberExpression"][${path}.property.type="Identifier"][${path}.property.name="length"]`;
const selector = [
	'LogicalExpression',
	...
	`:matches(${[
		...
		// `&& foo.length !== 0`
		// `&& foo.length > 0`
		['[right.type="BinaryExpression"]', ':matches([right.operator="!=="], [right.operator=">"])', getLengthSelector('right.left'), '[right.right.type="Literal"]', '[right.right.raw="0"]'].join(
			'',
		),
	].join(', ')})`,
].join('');
...
const negativeSelector = [
	...
	`:matches(${[
		// `|| !foo.length`
		['[right.type="UnaryExpression"]', '[right.operator="!"]', getLengthSelector('right.argument')].join(''),
		// `|| foo.length === 0`
		['[right.type="BinaryExpression"]', '[right.operator="==="]', getLengthSelector('right.left'), '[right.right.type="Literal"]', '[right.right.raw="0"]'].join(''),
	].join(', ')})`,
].join('');
	...
	create(context) {
		...
		return {
			[selector](node) {
				...
				const rightObject = right.type === 'BinaryExpression' ? right.left.object : right.object;
				...
				context.report({
					...
					fix(fixer) {
						return fixer.replaceTextRange([start, end], `isNonEmptyArray(${objectText})`);
					},
				});
			},
			[negativeSelector](node) {
				...
				const rightObject = right.type === 'UnaryExpression' ? right.argument.object : right.left.object;
