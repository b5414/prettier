...
const selector = ['Identifier', '[name="n"]', `:not(${['MemberExpression[computed=false] > .property', 'Property[shorthand=false][computed=false] > .key'].join(', ')})`].join('');
			...
			'Program:exit'() {
				for(const [variable, {fixable}] of variables.entries()){
					...
					const fix = function* (fixer) {
						const identifiers = new Set([...variable.identifiers, ...variable.references.map((reference)=>reference.identifier)]);

						for(const identifier of identifiers){
							const {parent} = identifier;
							if(parent && parent.type === 'Property' && parent.shorthand && parent.key === identifier){
