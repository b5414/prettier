...
const selector = ['CallExpression', '[callee.type="Identifier"]', '[callee.name="runCli"]'].join('');
	...
	create(context) {
		return {
			[selector](callExpression) {
				...
				if (parent.type === 'AwaitExpression' && parent.argument === callExpression) {
					return;
				}
				...
				if ((parent.type === 'ArrowFunctionExpression' && parent.body === callExpression) || (parent.type === 'ReturnStatement' && parent.argument === callExpression)) {
					return;
				}
				...
				if (
					...
					if (memberExpression.parent.type === 'AwaitExpression' && memberExpression.parent.argument === memberExpression) {
						return;
					}
