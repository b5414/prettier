...
test('await-cli-tests', {
	...
	invalid: [
		{
			code: 'runCli()',
			errors: [{message: "'runCli()' should be awaited or calling `.test()`."}],
		},
		{
			code: 'runCli().stderr',
...
test('better-parent-property-check-in-needs-parens', {
	...
	invalid: [
		...
		},
		{
			...
			errors: [{message: 'Prefer `key === "test"` over `parent.test === node`.'}],
		},
		{
			code: 'return parent.test !== node;',
			output: 'return key !== "test";',
		...
		},
		{
			...
			errors: [
				{
					message: 'Prefer `key === "property"` over `parent."property" === node`.',
				},
			],
		},
	].map((testCase) => ({
		...
		output: testCase.output ? `function needsParens() {${testCase.output}}` : null,
	...
	invalid: [
		{
			...
			errors: [{message: 'Please import `locStart` function and use it directly.'}],
		},
		{
			code: 'options.locEnd(node)',
			output: 'locEnd(node)',
			errors: [{message: 'Please import `locEnd` function and use it directly.'}],
		},
	],
});
	...
	invalid: [
		{
			...
			errors: [
				{
					message: "Do not use 'CommentCheckFlags.Leading', 'CommentCheckFlags.Trailing' together.",
				},
			],
		},
		{
			...
			errors: [
				{
					message: "Do not use 'CommentCheckFlags.Leading', 'CommentCheckFlags.Trailing', 'CommentCheckFlags.Dangling' together.",
				},
			],
		},
		{
		...
		},
		{
			...
			errors: [
				{
					message: "Do not use 'CommentCheckFlags.Block', 'CommentCheckFlags.Line' together.",
				},
			],
		},
		{
...
test('no-legacy-format-test', {
	valid: ["runFormatTest(import.meta, ['babel'])", "runFormatTest({importMeta: import.meta}, ['babel'])"],
	invalid: [
		{
		...
		},
		{
			...
			errors: [
				{
					message: 'Use `importMeta: import.meta` instead of `dirname: __dirname`.',
				},
			],
			output: "runFormatTest({snippets: ['x'], importMeta: import.meta}, ['babel'])",
		},
	],
});
...
test('no-node-comments', {
	...
	invalid: [
		...['function functionName() {return node.comments;}', 'const {comments} = node', 'const {comments: nodeComments} = node'].map((code) => ({
			code,
			output: null,
	...
	invalid: [
      ...
      `,
        ...
        const foo = node =>
          node.type === "Identifier" || node.type === "FunctionExpression";
      `,
			output: 'const foo = createTypeCheckFunction(["Identifier", "FunctionExpression"]);',
			errors: 1,
		},
		{
			code: outdent`
      ...
      `,
			output: 'const foo = createTypeCheckFunction([a.complex.way.to.get.type()]);',
			errors: 1,
		},
		{
			code: outdent`
      ...
      `,
        ...
        const foo = ({type}) =>
          ...
          type === "Identifier";
      `,
			output: 'const foo = createTypeCheckFunction([...a.complex.way.to.get.types(), ...another.complex.way.to.get.types(), "Identifier"]);',
			errors: 1,
		},
		{
			code: outdent`
      ...
      `,
			output: 'const isClassProperty = createTypeCheckFunction(["ClassProperty", "PropertyDefinition"]);',
			errors: 1,
		},
	],
});
...
test('prefer-is-non-empty-array', {
	...
	invalid: [
		...
		...['!a || !a.b || !a.b.length', '!a || !a.b || a.b.length === 0', '!a || !Array.isArray(a.b) || !a.b.length', '!a || !Array.isArray(a.b) || a.b.length === 0'].map(
			(code) => ({
				code,
				output: '!a || !isNonEmptyArray(a.b)',
				errors: 1,
...
test('no-empty-flat-contents-for-if-break', {
	valid: ["ifBreak('foo', 'bar')", 'ifBreak(doc1, doc2)', "ifBreak(',')", 'ifBreak(doc)', "ifBreak('foo', '', { groupId })", 'ifBreak(...foo, { groupId })'],
	invalid: [
		{
			code: "ifBreak('foo', '')",
			output: "ifBreak('foo')",
			errors: [
				{
					message: "Please don't pass an empty string to second parameter of ifBreak.",
				},
			],
		},
		{
...
test('no-unnecessary-ast-path-call', {
	valid: ['call(foo)', 'foo["call"](bar)', 'foo.call?.(bar)', 'foo?.call(bar)', 'foo.call(bar, name)', 'foo.notCall(bar)', 'foo.call(...bar)', 'foo.call()'],
	invalid: [
		{
			code: 'foo.call(bar)',
			output: 'bar(foo)',
...
test('prefer-ast-path-getters', {
	...
	invalid: [
		...
		// path.getParentNode(1)
		{
			...
			errors: [
				{
					message: 'Prefer `AstPath#grandparent` over `AstPath#getParentNode(1)`.',
				},
			],
		},
		{
		...
		},
		{
			...
			errors: [
				{
					message: 'Prefer `AstPath#key` or `AstPath#index` over `AstPath#getName()`.',
					suggestions: [
...
test('massage-ast-parameter-names', {
	valid: ['function notNamedClean(a, b) {}', 'function clean(original, cloned) {}'],
	invalid: [
		{
