...
const test = (ruleId, tests)=>{
	new RuleTester({parserOptions: {ecmaVersion: 2021}}).run(ruleId, rules[ruleId], tests);
};
...
test('better-parent-property-check-in-needs-parens', {
	...
	invalid: [
		...
		},
		{
			...
			errors: [{message: 'Prefer `name === "test"` over `parent.test === node`.'}],
		},
		{
			code: 'return parent.test !== node;',
			output: 'return name !== "test";',
		...
		},
		{
			...
			errors: [
				{
					message: 'Prefer `name === "property"` over `parent."property" === node`.',
				},
			],
		},
	].map((testCase)=>({
...
test('prefer-is-non-empty-array', {
	...
	invalid: [
		...
		...['!a || !a.b || !a.b.length', '!a || !a.b || a.b.length === 0', '!a || !Array.isArray(a.b) || !a.b.length', '!a || !Array.isArray(a.b) || a.b.length === 0'].map((code)=>({
			code,
			output: '!a || !isNonEmptyArray(a.b)',
			errors: 1,
		})),
