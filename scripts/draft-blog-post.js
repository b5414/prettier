...
import {changelogUnreleasedDirPath, changelogUnreleasedDirs, getEntries, printEntries, replaceVersions} from './utils/changelog.js';

const {__dirname, require} = createEsmUtils(import.meta);
const blogDir = path.join(__dirname, '../website/blog');
const introTemplateFile = path.join(changelogUnreleasedDirPath, 'BLOG_POST_INTRO_TEMPLATE.md');
...
const nextVersion = `${semver.major(version)}.${semver.minor(version)}.${semver.patch(version)}`;

const postGlob = path.join(blogDir, `????-??-??-${nextVersion}.md`);
const postFile = path.join(blogDir, `${new Date().toISOString().replace(/T.+/u, '')}-${nextVersion}.md`);
...
const categoriesByDir = new Map(categories.map((category) => [category.dir, category]));
