...
async function buildFile({file, files, cliOptions, results}) {
	...
	if ((file.platform === 'universal' && file.output.format !== 'esm') || (file.output.file.startsWith('index.') && file.output.format !== 'esm') || file.kind === 'types') {
		displayName = ` ${displayName}`;
	}

	process.stdout.write(fitTerminal(displayName));

	if ((cliOptions.files && !cliOptions.files.has(file.output.file)) || (cliOptions.playground && (file.output.format !== 'umd' || file.output.file === 'doc.js'))) {
	...
	if (cliOptions.compareSize) {
		...
		if (stableSize) {
			...
			const message = chalk[sizeDiff > 0 ? 'yellow' : 'green'](prettyBytes(sizeDiff));
	...
	if (sizeMessages.length > 0) {
		// Clear previous line
		clear();
		process.stdout.write(fitTerminal(displayName, `${sizeMessages.join(', ')} `));
	}
