...
const {dirname, readJsonSync, require, resolve: importMetaResolve} = createEsmUtils(import.meta);
...
function getEsbuildOptions({file, files, cliOptions}) {
    ...
    */
		...
		},
		{
			...
			process(text) {
				const exports = [...text.matchAll(/(?<=\n)exports\.(?<specifier>\w+) = \k<specifier>;/gu)].map((match) => match.groups.specifier);

				const lines = text.split('\n');
				const startMarkLine = lines.findIndex((line) => line.includes('function _interopRequireDefault'));
				const endMarkLine = lines.indexOf('module.exports = __webpack_exports__;');

				if (lines[startMarkLine + 1] !== '/**' || lines[endMarkLine - 2] !== '})();') {
				...
				text = text
					.replace('const line = (0, _detectNewline().default)(docblock) ?? _os().EOL;', String.raw`const line = "\n"`)
	...
	if (file.platform === 'node') {
		...
		replaceModule.push(
			...files
				.filter((bundle) => bundle.input === 'package.json' || (file.input !== bundle.input && bundle.output.format === 'esm'))
				.map((bundle) => {
					let output = bundle.output.file;
					if (file.output.file === 'index.cjs' && bundle.output.file === 'doc.mjs') {
						output = 'doc.js';
					}
	...
	const shouldMinify = cliOptions.minify ?? buildOptions.minify ?? file.platform === 'universal';
	...
	const esbuildOptions = {
		...
		plugins: [
			...
			cliOptions.reports && esbuildPluginVisualizer({formats: cliOptions.reports}),
