...
async function buildPluginTypes({file: {input, output}}) {
	const pluginModule = await import(url.pathToFileURL(path.join(PROJECT_ROOT, input)));
	...
	const code =
        ...
        ${parserNames.map((parserName) => `${' '.repeat(2)}${toPropertyKey(parserName)}: Parser;`).join('\n')}
        };
      `;
...
function buildTypes(options) {
	return options.file.isPlugin ? buildPluginTypes(options) : typesFileBuilder(options);
}
