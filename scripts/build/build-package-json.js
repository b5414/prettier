...
async function buildPackageJson({file, files}) {
	...
	const bin = files.find((file) => path.join(PROJECT_ROOT, packageJson.bin) === path.join(PROJECT_ROOT, file.input)).output.file;
	...
	const overrides = {
		...
		exports: {
			...
			...Object.fromEntries(
				files
					.filter((file) => file.isPlugin && file.output.format === 'umd' && file.output.file !== 'plugins/estree.js')
					.flatMap((file) => {
						...
						return [
							...
							[`./esm/parser-${basename}.mjs`, `./${file.output.file.replace(/\.js$/u, '.mjs')}`],
						];
					}),
			),
		},
	...
	await writeJson(path.join(DIST_DIR, file.output.file), Object.assign(pick(packageJson, keysToKeep), overrides));
}
