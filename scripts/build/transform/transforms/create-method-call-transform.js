...
function createMethodCallTransform({methodName, argumentsLength, functionName = `__${methodName}`, functionImplementationUrl}) {
	const functionImplementationPath = fileURLToPath(functionImplementationUrl);

	return {
		shouldSkip: (text, file) => !text.includes(`.${methodName}(`) || file === functionImplementationPath,
		test: (node) => isMethodCall(node, {methodName, argumentsLength}),
		transform: (node) => transformMethodCallToFunctionCall(node, functionName),
		inject: `import ${functionName} from ${JSON.stringify(functionImplementationPath)};`,
	};
}
