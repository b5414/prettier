...
const {require, dirname, resolve: importMetaResolve} = createEsmUtils(import.meta);
const resolveEsmModulePath = (specifier) => url.fileURLToPath(importMetaResolve(specifier));
const copyFileBuilder = ({file}) => copyFile(path.join(PROJECT_ROOT, file.input), path.join(DIST_DIR, file.output.file));
...
const pluginFiles = [
		...
		replaceModule: [
			{
				...
				process: (text) => text.replaceAll('const entity = entities[desc];', 'const entity = undefined'),
			},
		],
	},
	{
		...
		replaceModule: [
			{
				...
				process(text) {
					const {fsModuleNameVariableName} = text.match(/,(?<fsModuleNameVariableName>[\p{ID_Start}_$][\p{ID_Continue}$]*)="fs",/u).groups;

					return text.replaceAll(`require(${fsModuleNameVariableName})`, '{}').replaceAll('require("fs")', '{}').replaceAll('require("constants")', '{}');
				},
			},
		],
	},
	{
		...
		replaceModule: [
			...
			{
				module: getPackageFile('@typescript-eslint/typescript-estree/dist/index.js'),
				path: getPackageFile('@typescript-eslint/typescript-estree/dist/parser.js'),
			},
			{
				module: getPackageFile('@typescript-eslint/typescript-estree/dist/create-program/getScriptKind.js'),
				process: (text) => text.replace('require("path")', '{extname: file => "." + file.split(".").pop()}'),
			},
			{
				module: getPackageFile('@typescript-eslint/typescript-estree/dist/parseSettings/createParseSettings.js'),
				process(text) {
					return text
						.replace('process.cwd()', JSON.stringify('/prettier-security-dirname-placeholder'))
						.replace('parseSettings.projects = ', 'parseSettings.projects = true ? new Map() : ');
				},
			},
			{
				module: '*',
          ...
          `,
			...
			},
			{
				...
				process() {
					throw new Error("Please replace the CJS version of 'ts-api-utils' with ESM version.");
				},
			},
			{
				module: getPackageFile('@typescript-eslint/types/dist/generated/ast-spec.js'),
				text: outdent`
        ...
        `,
			...
			// Use named import from `typescript`
			{
				...
				process(text) {
					const typescriptVariables = [...text.matchAll(/import (?<variable>\w+) from ["']typescript["']/gu)].map((match) => match.groups.variable);

					// Remove `'property' in typescript` check
					text = text.replaceAll(new RegExp(`".*?" in (?:${typescriptVariables.join('|')})(?=\\W)`, 'gu'), 'true');

					text = text.replaceAll(/(?<=import )(?=\w+ from ["']typescript["'])/gu, '* as ');
					return text;
				},
			},
		],
		...
		replaceModule: [
			{
				...
				process(text) {
					...
					lineIndex = lines.findIndex((line) => line.endsWith(' from "eslint-visitor-keys";'));
	...
	},
	{
        ...
        `,
			...
			...['expression_parser/lexer.mjs', 'expression_parser/parser.mjs', 'ml_parser/defaults.mjs'].map((file) => ({
				module: getPackageFile(`@angular/compiler/esm2022/src/${file}`),
				process: (text) => text.replaceAll(/(?<=import .*? from )'(.{1,2}\/.*)'/gu, "'$1.mjs'"),
			})),
		],
	},
	{
		...
		replaceModule: [
			...
			},
			{
				...
				process: (text) => text.replace("require('url')", '{}').replace("require('path')", '{}'),
			},
	...
	},
	{
		...
		replaceModule: [
			...['@glimmer/util', '@glimmer/wire-format', '@glimmer/syntax'].map((packageName) => ({
			...
			})),
			{
				...
				process(text) {
					...
					text = text.replace('const ARGUMENT_RESOLUTION = ', 'const ARGUMENT_RESOLUTION = undefined &&');

					text = text.replace('const HTML_RESOLUTION = ', 'const HTML_RESOLUTION = undefined &&');

					text = text.replace('const LOCAL_DEBUG = ', 'const LOCAL_DEBUG = false &&');
	...
	outputBaseName ??= input.match(/\/plugins\/(?<outputBaseName>.*?)\.js$/u).groups.outputBaseName;
...
const nonPluginUniversalFiles = [
	...
	const {input, outputBaseName = path.basename(input, '.js'), umdVariableName, ...buildOptions} = file;
...
const universalFiles = [...nonPluginUniversalFiles, ...pluginFiles].flatMap((file) => {
	let {input, outputBaseName, umdVariableName, buildOptions, isPlugin} = file;
...
const nodejsFiles = [
	{
		...
		replaceModule: [
			...
			// `editorconfig` use a older version of `semver` and only uses `semver.gte`
			{
				...
				replacement: `
          var semver = {
            gte: require(${JSON.stringify(require.resolve('semver/functions/gte'))})
          };
        `,
			},
			{
		...
		replaceModule: [
			{
				...
				process: (text) => text.replace('../src/cli/index.js', '../internal/cli.mjs'),
			},
		],
	},
	{
