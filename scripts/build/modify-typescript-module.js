...
class TypeScriptModuleSource {
	...
	replaceAlignedCode({start, end, replacement = ''}) {
		const regexp = new RegExp(
			['(?<=\n)', String.raw`(?<indentString>\s*)`, escapeStringRegexp(start), '.*?', '(?<=\n)', String.raw`\k<indentString>`, escapeStringRegexp(end), '(?=\n)'].join(''),
			'gsu',
		);
...
function getExports(entry) {
	...
	const exports = lines
		.map((line) => {
			const match = line.match(/^\s*(?<specifier>.*?): \(\) => (?<variable>.*?),?$/u);
...
function modifyTypescriptModule(text) {
	...
	for (const module of source.modules) {
		...
		if (module.path === 'src/services/utilities.ts') {
			source.replaceModule(module, 'var scanner = createScanner(ScriptTarget.Latest, /*skipTrivia*/ true);');
			continue;
		}
	...
	// perfLogger
	source.replaceModule('src/compiler/perfLogger.ts', 'var perfLogger = new Proxy(() => {}, {get: () => perfLogger});');
...
function createExports(exports) {
    ...
    export {
      ${exports
			.map(({specifier, variable}) => (variable === specifier ? specifier : `${variable} as ${specifier}`))
			.map((line) => `  ${line},`)
			.join('\n')}
    };
  `;
