		...
		setup(build) {
			...
			build.onLoad({filter: /\.evaluate\.c?js$/u}, async ({path}) => {
				...
				const text = Object.entries(module)
					.map(([specifier, value]) => {
						const code = value instanceof RegExp ? `/${value.source}/${value.flags}` : serialize(value, {space: 2});

						if (specifier === 'default') {
							return format === 'cjs' ? `module.exports = ${code};` : `export default ${code};`;
						}
						...
						return format === 'cjs' ? `exports.${specifier} = ${code};` : `export const ${specifier} = ${code};`;
					})
					.join('\n');
