...
function getUmdWrapper({name}, build) {
	...
	globalObjectText = globalObjectText.map((text) => `${' '.repeat(4)}${text}`).join('\n');
  ...
  `;
	...
	if (minify) {
		wrapper = build.esbuild.transformSync(wrapper, {loader: 'js', minify}).code.trim();
	...
	return {
		...
		outro,
		expectedOutput: {
			start: minify ? `var ${temporaryName}=(()=>{` : `var ${temporaryName} = (() => {`,
			end: '})();',
		},
	};
}
...
export default function esbuildPluginUmd(options) {
	return {
		...
		setup(build) {
			...
			const {name: temporaryName, intro, outro, expectedOutput} = getUmdWrapper(options, build);
            ...
            `);
				...
				fs.writeFileSync(outfile, intro + text.slice(expectedOutput.start.length, -expectedOutput.end.length).trimEnd() + outro);
			});
		},
	};
}
