	...
	return {
		...
		setup(build) {
			...
			if (esbuildConfig.define) {
				throw new Error("Use `esbuildPluginPrimitiveDefine(define)` instead of 'define' option");
			}

			esbuildConfig.define = Object.fromEntries(Object.entries(define).map(([key, value]) => [key, stringify(value)]));
		},
	};
}
