...
function processReplacements(replacements) {
	...
	const checkTextReplaced = (module) => {
		...
		throw new Error(`module: '${module}' already replaced with another content.`);
	};
	...
	for (const replacement of replacements) {
		...
		if (Object.hasOwn(replacement, 'external') || Object.hasOwn(replacement, 'path')) {
			if (module === '*') {
			...
			onResolveReplacements.set(module, Object.hasOwn(replacement, 'external') ? {external: true, path: replacement.external} : {path: replacement.path});
		...
		} else if (Object.hasOwn(replacement, 'find') && Object.hasOwn(replacement, 'replacement')) {
			process = (text) => text.replaceAll(replacement.find, replacement.replacement);
		} else {
...
function setupOnResolveListener(build, {concepts, replacements}) {
	...
	build.onResolve(concepts, async (args) => {
		if (!(args.kind === 'require-call' || args.kind === 'import-statement' || args.kind === 'dynamic-import')) {
			return;
		}
...
function setupOnLoadListener(build, {concepts, replacements, processors}) {
	...
	build.onLoad(concepts, async ({path: file}) => {
		...
		const processFunctions = processors.filter(({module}) => module === '*' || module === file).map((processor) => processor.process);
...
export default function esbuildPluginReplaceModule({onLoadConcepts = DEFAULT_ON_LOAD_CONCEPTS, onResolveConcepts = DEFAULT_ON_RESOLVE_CONCEPTS, replacements}) {
	const {onResolveReplacements, onLoadReplacements, onLoadProcessors} = processReplacements(replacements);
