...
const logOverride = Object.fromEntries(ESBUILD_MESSAGE_IDS.map((id) => [id, 'warning']));
		...
		setup(build) {
			...
			build.onEnd((result) => {
				...
				for (const warning of result.warnings) {
					if (allowDynamicRequire && ['unsupported-require-call', 'indirect-require'].includes(warning.id)) {
						continue;
					}

					if (allowDynamicImport && warning.id === 'unsupported-dynamic-import') {
						continue;
					}

					if (
						['node_modules/flow-parser/flow_parser.js', 'dist/_parser-flow.js.umd.js', 'dist/_parser-flow.js.esm.mjs'].includes(warning.location.file) &&
