...
function getDependencies(results) {
	// A fake rollup chunk
	const chunk = {
		modules: Object.fromEntries(results.flatMap((result) => Object.keys(result.esbuildResult?.metafile.inputs ?? {})).map((file) => [file, {renderedLength: 1}])),
	};
...
async function getLicenseText(dependencies) {
	dependencies = dependencies.filter(
		(dependency, index) =>
			...
			index === dependencies.findIndex(({name, version}) => dependency.name === name && dependency.version === version),
	);

	dependencies.sort((dependencyA, dependencyB) => dependencyA.name.localeCompare(dependencyB.name) || dependencyA.version.localeCompare(dependencyB.version));

	const prettierLicense = await fs.readFile(PROJECT_LICENSE_FILE, 'utf8');

	const licenses = [...new Set(dependencies.filter(({license}) => license).map(({license}) => license))];
	...
	const content = dependencies
		.map((dependency) => {
			...
			if (dependency.contributors?.length > 0) {
				const contributors = dependency.contributors.map((contributor) => ` - ${contributor.text()}`).join('\n');
...
async function buildLicense({file, files, results, cliOptions}) {
	...
	const shouldBuildLicense = !cliOptions.playground && !cliOptions.files && typeof cliOptions.minify !== 'boolean';
