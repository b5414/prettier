...
const EXTERNALS = ['assert', 'buffer', 'constants', 'crypto', 'events', 'fs', 'module', 'os', 'path', 'stream', 'url', 'util', 'readline', 'tty'];
...
const entries = [
	...
	},
	{
		...
		replacement: path.resolve(`${PROJECT_ROOT}/node_modules/@angular/compiler/esm2015/src`),
	},
...
function getBabelConfig(bundle) {
	...
	if(bundle.type === 'core'){
		config.plugins.push(require.resolve('./babel-plugins/transform-custom-require'));
	}
	const targets = {node: '10'};
	if(bundle.target === 'universal'){
		targets.browsers = ['>0.5%', 'not ie 11', 'not safari 5.1', 'not op_mini all'];
	}
	config.presets = [
		[
	...
	config.plugins.push([require.resolve('@babel/plugin-proposal-object-rest-spread'), {loose: true, useBuiltIns: true}], require.resolve('@babel/plugin-proposal-optional-catch-binding'));
	return config;
}
...
function getRollupConfig(bundle) {
	const config = {
		...
		onwarn(warning) {
			if(
				...
				(warning.code === 'CIRCULAR_DEPENDENCY' && (warning.importer.startsWith('node_modules') || warning.importer.startsWith('\x00polyfill-node:'))) ||
			...
			if(warning.code === 'UNRESOLVED_IMPORT' && bundle.target === 'universal'){
				throw new Error(`Unresolved dependency in universal bundle: ${warning.source}`);
			}
	...
	if(bundle.target === 'universal'){
		...
		// See #8268
		replaceStrings.__filename = JSON.stringify('/prettier-security-filename-placeholder.js');
		replaceStrings.__dirname = JSON.stringify('/prettier-security-dirname-placeholder');
	}
...
function getRollupOutputOptions(bundle, buildOptions) {
	...
	if(bundle.target === 'node'){
		...
		options.name = bundle.type === 'plugin' ? `prettierPlugins.${bundle.name}` : bundle.name;
...
module.exports = async function createBundle(bundle, cache, options) {
	...
	if(!options['purge-cache'] && (await Promise.all(outputOptions.map((outputOption)=>cache.isCached(inputOptions, outputOption)))).every((cached)=>cached)){
		return {cached: true};
	}
