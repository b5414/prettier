...
async function getRemovedSpecifiers(code, exports) {
	...
	for (const {text} of errors) {
		const match = text.match(/^"(?<variable>.*?)" is not declared in this file$/u);

		if (match) {
			const {specifier} = exports.find(({variable}) => variable === match.groups.variable);
...
async function main() {
	const original = await fs.readFile(new URL(importMetaResolve.resolve('typescript', import.meta.url)), 'utf8');
	const {code, exports} = modifyTypescriptModule(original);

	let specifiers = (await getRemovedSpecifiers(code, exports)) ?? [];

	specifiers = [...new Set([...UNUSED_SPECIFIERS, ...specifiers])].filter((specifier) => Object.hasOwn(typescript, specifier)).sort();
