...
const {browserslist: targets} = JSON.parse(await fs.readFile(new URL('../package.json', import.meta.url)));

const browserFiles = ['doc.js', 'doc.mjs', 'standalone.js', 'standalone.mjs', 'plugins/*'];
...
const browserRestrictedSyntaxes = [
	...
	{
		selector: ':matches(ExportAllDeclaration, ExportDefaultDeclaration, ExportNamedDeclaration)[source]',
...
const nodejsRestrictedSyntaxes = [
	...
	{
		selector: 'CallExpression:not(:function *)[callee.name="require"][arguments.0.value=/plugins/]',
	...
	{
		selector: ':matches(ImportDeclaration, ExportAllDeclaration, ExportDefaultDeclaration, ExportNamedDeclaration)[source.value=/plugins/]',
		message: 'Parsers should be inline `import()`ed.',
	},
];
...
export default [
...

	{
		files: browserFiles,
		rules: {
			'no-restricted-syntax': ['error', ...restrictedSyntaxes, ...browserRestrictedSyntaxes],
		},
	},

	{
		files: nodejsFiles,
		rules: {
			'no-restricted-syntax': ['error', ...restrictedSyntaxes, ...nodejsRestrictedSyntaxes],
		},
	},
