...
async function update() {
	const npmPage = await logPromise(
		...
		fetch('https://www.npmjs.com/package/prettier').then((response)=>response.text()),
	);
	const dependentsCountNpm = Number(npmPage.match(/"dependentsCount":(\d+),/)[1]);
	...
	const githubPage = await logPromise(
		...
		fetch('https://github.com/prettier/prettier/network/dependents').then((response)=>response.text()),
	);
	const dependentsCountGithub = Number(githubPage.replace(/\n/g, '').match(/<svg.*?octicon-code-square.*?>.*?<\/svg>\s*([\d,]+?)\s*Repositories\s*<\/a>/)[1].replace(/,/g, ''));
	if(Number.isNaN(dependentsCountNpm)){
	...
	processFile('website/pages/en/index.js', (content)=>
		content.replace(/(<strong data-placeholder="dependent-npm">)(.*?)(<\/strong>)/, `$1${formatNumber(dependentsCountNpm)}$3`).replace(
	...
	const isUpdated = await logPromise(
		...
		execa('git', ['diff', '--name-only']).then(({stdout})=>stdout === 'website/pages/en/index.js'),
	);
