...
export default async function updateVersion({version, next}) {
	...
	processFile('.github/ISSUE_TEMPLATE/formatting.md', (content) => content.replace(/^(\*\*Prettier ).*?(\*\*)$/mu, `$1${version}$2`));
	processFile('.github/ISSUE_TEMPLATE/integration.md', (content) => content.replace(/^(- Prettier Version: ).*$/mu, `$1${version}`));
