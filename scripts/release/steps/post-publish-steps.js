...
async function checkSchema() {
	...
	const remoteSchema = await logPromise('Checking current schema in SchemaStore', fetchText(RAW_URL));

	if (schema.trim() === remoteSchema.trim()) {
		return;
	}

	writeFile(new URL('../../../.tmp/schema/prettierrc.json', import.meta.url), schema);

	return outdent`
    ${chalk.bold.underline('The schema in {yellow SchemaStore')} needs an update.}
...
export default async function postPublishSteps({dry, next}) {
	...
	console.log(
		outdent`
      ${chalk.yellow.bold(`The following ${steps.length === 1 ? 'step is' : 'steps are'} optional.`)}
