...
import {getBlogPostInfo, getChangelogContent, waitForEnter} from '../utils.js';
...
export function getReleaseUrl(version, previousVersion) {
	...
	let body;
	if (isPatch) {
		const urlToChangelog = 'https://github.com/prettier/prettier/blob/main/CHANGELOG.md#' + version.split('.').join('');
...
export default async function showInstructionsAfterNpmPublish({version, previousVersion, next}) {
	if (next) {
