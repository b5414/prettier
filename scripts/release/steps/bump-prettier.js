...
async function commit(version) {
	await execa('git', ['commit', '-am', `Bump Prettier dependency to ${version}`]);
...
async function bump({version, previousVersion, previousVersionOnDefaultBranch}) {
...
module.exports = async function (params) {
	...
	await logPromise('Installing Prettier', execa('yarn', ['add', '--dev', `prettier@${version}`]));
