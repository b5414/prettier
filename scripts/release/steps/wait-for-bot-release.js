...
export default async function waitForBotRelease({dry, version, next}) {
	...
	if (!(await checkBotPermission())) {
		console.log(
			outdent`
        1. Go to ${chalk.green.underline('https://www.npmjs.com/package/prettier/access')}
	...
	console.log(
		outdent`
      1. Go to ${chalk.green.underline('https://www.npmjs.com/package/prettier/access')}
      2. Make sure "${chalk.yellow('Publishing access')}" section is set to "${chalk.yellow('Require two-factor authentication or automation tokens')}".
    `,
	);
	...
	console.log(
		outdent`
      1. Go to ${chalk.green.underline('https://github.com/prettier/release-workflow/actions/workflows/release.yml')}
      2. Click "${chalk.green('Run workflow')}" button, type "${chalk.yellow.underline(version)}" in "Version to release", ${
	...
	while (!released) {
		try {
			released = await logPromise('Checking release status', isVersionReleased(version));
		} catch {
			// No op
		}
