...
async function logPromise(name, promiseOrAsyncFunction, shouldSkip = false) {
	...
	try {
		const result = await (typeof promiseOrAsyncFunction === 'function' ? promiseOrAsyncFunction() : promiseOrAsyncFunction);
...
export {fetchText, getBlogPostInfo, getChangelogContent, logPromise, processFile, readJson, runGit, runYarn, waitForEnter, writeFile, writeJson};
