...
const getDateParts = () => [new Date().getFullYear(), String(new Date().getMonth() + 1).padStart(2, '0'), String(new Date().getDate()).padStart(2, '0')];
			...
			assert.equal(
				...
				getExpectedReleaseUrl({
					...
					body: [
						...
						`🔗 [Release note](https://prettier.io/blog/${getDateParts().join('/')}/2.4.0.html)`,
					].join('\n\n'),
				}),
			);
		});
			...
			assert.equal(
				...
				getExpectedReleaseUrl({
					...
					body: [
						...
						`🔗 [Release note](https://prettier.io/blog/${getDateParts().join('/')}/2.3.0.html)`,
					].join('\n\n'),
				}),
			);
		});
