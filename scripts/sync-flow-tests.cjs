...
function syncTests(syncDir) {
	const specFiles = fastGlob.sync(path.join(FLOW_TESTS_DIR, '**', SPEC_FILE_NAME));
	const filesToCopy = fastGlob.sync(path.join(syncDir, '**/*.js'));

	if (filesToCopy.length === 0) {
		throw new Error(["Couldn't find any files to copy.", `Please make sure that \`${syncDir}\` exists and contains the flow tests.`].join('\n'));
	}
...
function run(argv) {
	if (argv.length !== 1) {
		console.error(['You must provide the path to a flow tests directory to sync from!', 'Example: node scripts/sync-flow-tests.cjs ../flow/tests/'].join('\n'));
		return 1;
	}
