	...
	for (const prFile of files) {
		...
		for (const comment of templateComments) {
			if (comment !== '<!-- prettier-ignore -->' && content.includes(comment)) {
				showErrorMessage(`[${displayPath}]: Please remove ${getCommentDescription(content, comment)}`);
			}
		}
...
function getCommentDescription(content, comment) {
	...
	const [startLine, endLine] = [start, end].map((index) => linesAndColumns.locationForIndex(index).line + 1);
