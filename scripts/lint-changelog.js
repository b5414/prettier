...
const CHANGELOG_CATEGORIES = ['angular', 'api', 'cli', 'css', 'flow', 'graphql', 'handlebars', 'html', 'javascript', 'json', 'less', 'lwc', 'markdown', 'mdx', 'scss', 'typescript', 'vue', 'yaml'];
...
for(const file of files){
	if(file !== TEMPLATE_FILE && file !== BLOG_POST_INTRO_FILE && file !== BLOG_POST_INTRO_TEMPLATE_FILE && !CHANGELOG_CATEGORIES.includes(file)){
		showErrorMessage(`Please remove "${file}" from "${CHANGELOG_DIR}".`);
	}
}
for(const file of [TEMPLATE_FILE, BLOG_POST_INTRO_TEMPLATE_FILE, ...CHANGELOG_CATEGORIES]){
...
const template = fs.readFileSync(path.join(CHANGELOG_ROOT, TEMPLATE_FILE), 'utf8');
...
for(const category of CHANGELOG_CATEGORIES){
	...
	if(!files.includes('.gitkeep')){
		showErrorMessage(`Please don't remove ".gitkeep" from "${CHANGELOG_DIR}/${category}".`);
	}
	...
	for(const prFile of files){
		...
		if(!match){
			showErrorMessage(`[${displayPath}]: Filename is not in form of "{PR_NUMBER}.md".`);
			continue;
		}
		...
		const content = fs.readFileSync(path.join(CHANGELOG_DIR, category, prFile), 'utf8');
		...
		if(content.includes(templateComment)){
			showErrorMessage(`[${displayPath}]: Please remove template comments at top.`);
		}
		if(content.includes(templateAuthorLink)){
			showErrorMessage(`[${displayPath}]: Please change author link to your github account.`);
		}
		...
		if([...CHANGELOG_CATEGORIES, 'js'].includes(categoryInTitle.toLowerCase())){
			showErrorMessage(`[${displayPath}]: Please remove "${categoryInTitle}:" in title.`);
		}

		if(!title.endsWith(' ') || title.length - title.trimEnd().length !== 1){
			showErrorMessage(`[${displayPath}]: Please put one space between title and PR link.`);
		}

		if(/prettier master/i.test(content)){
			showErrorMessage(`[${displayPath}]: Please use "main" instead of "master".`);
		}
	}
}
