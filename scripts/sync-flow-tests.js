...
function syncTests(syncDir) {
	const specFiles = globby.sync(path.join(FLOW_TESTS_DIR, '**', SPEC_FILE_NAME));
...
function run(argv) {
	if(argv.length !== 1){
		console.error(['You must provide the path to a flow tests directory to sync from!', 'Example: node scripts/sync-flow-tests.js ../flow/tests/'].join('\n'));
		return 1;
	}
